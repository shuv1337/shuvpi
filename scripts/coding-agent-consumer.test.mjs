...
const codingAgentName = "@shuv1337/shuvpi-coding-agent";
const devPackages = ["shuvpi-client", "shuvpi-protocol", "shuvpi-server"].map((name) => `@shuv1337/${name}`);

function createFixture(t, { importServer = false, declareServer = false } = {}) {
	const root = mkdtempSync(join(tmpdir(), "shuvpi-consumer-test-"));
	t.after(() => rmSync(root, { recursive: true, force: true }));
	const packages = [codingAgentName, "@shuv1337/shuvpi-chord", ...devPackages].map((name) => ({
		name,
	...
	for (const pkg of packages) {
		...
		const manifest = {
			...
			...(isAgent ? {
				bin: { shuvpi: "dist/bundle/cli.js" },
				dependencies: {
					"@shuv1337/shuvpi-chord": "1.0.0",
					...(declareServer ? { "@shuv1337/shuvpi-server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@shuv1337/shuvpi-server";' : ""}
import { marker } from "@shuv1337/shuvpi-chord";
...
test("installs only coding-agent directly and uses overrides only for declared runtime dependencies", (t) => {
	...
	const nested = join(directory, "node_modules", codingAgentName, "node_modules/@shuv1337/shuvpi-server");
	mkdirSync(nested, { recursive: true });
	writeFileSync(join(nested, "package.json"), JSON.stringify({ name: "@shuv1337/shuvpi-server", version: "1.0.0" }));
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /shuvpi-server must not be installed/);
...
test("fails when the SDK imports an undeclared server despite a working CLI", (t) => {
	...
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /Cannot find package '@shuv1337\/shuvpi-server'/);
});
