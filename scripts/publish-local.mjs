...
const otpArgIndex = process.argv.indexOf("--otp");
const otp = otpArgIndex === -1 ? undefined : process.argv[otpArgIndex + 1];
const unknownArgs = process.argv.slice(2).filter((arg, index, args) => {
	if (arg === "--dry-run" || arg === "--otp") {
		return false;
	}
	return args[index - 1] !== "--otp";
});

if (otpArgIndex !== -1 && !otp) {
	console.error("Usage: node scripts/publish-local.mjs [--dry-run] [--otp <code>]");
	process.exit(1);
}
