...
const developmentPackages = new Set(["shuvpi-client", "shuvpi-protocol", "shuvpi-server"].map((name) => `@shuv1337/${name}`));
...
export function smokeTestCodingAgentConsumer(directory, runtime = process.execPath) {
	...
	const env = {
		...
		XDG_CACHE_HOME: home,
		SHUVPI_CODING_AGENT_DIR: join(home, ".shuvpi", "agent"),
		SHUVPI_OFFLINE: "1",
		SHUVPI_TELEMETRY: "0",
	};
...
for (const name of ["shuvpi-client", "shuvpi-protocol", "shuvpi-server"]) {
...
`);
		...
		for (const cli of new Set([manifest.bin.shuvpi, "dist/cli.js"])) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	...
	const root = mkdtempSync(join(tmpdir(), "shuvpi-package-consumer-"));
	try {
