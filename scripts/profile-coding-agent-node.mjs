...
const agentDirEnvName = "SHUVPI_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "SHUVPI_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific SHUVPI_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force SHUVPI_OFFLINE=1 / SHUVPI_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		...
		env.SHUVPI_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "shuvpi-startup-benchmark-")) : undefined;
