#!/usr/bin/env bash
#
# Build shuvpi binaries for all platforms locally.
...
#     shuvpi-darwin-arm64.tar.gz
#     shuvpi-darwin-x64.tar.gz
#     shuvpi-linux-x64.tar.gz
#     shuvpi-linux-arm64.tar.gz
#     shuvpi-windows-x64.zip
#     shuvpi-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before shuvpi starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/shuvpi.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/shuvpi"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating shuvpi-$platform.zip..."
        (cd "$platform" && zip -r ../shuvpi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating shuvpi-$platform.tar.gz..."
        mv "$platform" shuvpi && tar -czf shuvpi-$platform.tar.gz shuvpi && mv shuvpi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../shuvpi-$platform.zip)
    else
        tar -xzf shuvpi-$platform.tar.gz && mv shuvpi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/shuvpi.exe"
    else
        echo "  $OUTPUT_DIR/$platform/shuvpi"
    fi
done
