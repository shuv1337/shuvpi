...
const INSTALLER_PACKAGE_NAME = "@shuv1337/shuvpi-coding-agent-install";
...
async function verifyPackagesAreAvailable(packages) {
	...
	do {
		...
		if (failures.length === 0) {
			console.log(`All ${packages.length} Shuvpi packages are available from npm (attempt ${attempt}).`);
			return results.map((result) => result.value);
		}

		console.log(`Waiting for ${failures.length} Shuvpi package${failures.length === 1 ? "" : "s"} on npm (attempt ${attempt}):`);
		for (const failure of failures) console.log(`  ${failure}`);
		if (Date.now() < deadline) await sleep(RETRY_DELAY_MS);
	} while (Date.now() < deadline);

	throw new Error(`Timed out waiting for Shuvpi packages to become available from npm:\n${failures.map((failure) => `  ${failure}`).join("\n")}`);
}
...
function readLatestRelease(bucket, endpoint, key, outputPath) {
	...
	if (typeof metadata.ETag !== "string") {
		throw new Error("Latest Shuvpi release marker has no ETag.");
	}
	runAws([
		"s3api",
		"get-object",
	...
	if (
		...
		throw new Error("Latest Shuvpi release marker has an invalid version.");
	}
...
function validateInstallerArtifacts(packageJsonPath, packageLockPath, version) {
	...
	if (
		...
		root.dependencies?.["@shuv1337/shuvpi-coding-agent"] !== version
	) {
...
export async function advanceLatestRelease(version, readLatest, writeLatest) {
	...
	throw new Error(`Could not advance the Shuvpi release marker to ${version} after ${MAX_POINTER_UPDATE_ATTEMPTS} attempts.`);
}
...
async function main() {
	...
	const temporaryDirectory = mkdtempSync(join(tmpdir(), "shuvpi-release-announcement-"));
	try {
		...
		console.log(
			result.advanced
				? `Announced Shuvpi ${options.version} through s3://${options.bucket}/${RELEASES_PREFIX}/latest.json`
				: `Shuvpi ${result.version} is already the latest announced release.`,
		);
	} finally {
