 ...
 *   --analyze      Spawn shuvpi subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("shuvpi", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn shuvpi: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".shuvpi/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn shuvpi subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".shuvpi/agent/AGENTS.md");
