...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "shuvpi-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `shuvpi-${platform}.zip` : `shuvpi-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createShuvpiShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "shuvpi.cmd"))) {
			writeFileSync(join(installDirectory, "shuvpi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\shuvpi.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "shuvpi.ps1"), '& "$PSScriptRoot/node_modules/.bin/shuvpi.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "shuvpi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\shuvpi.exe" %*\r\n');
		writeFileSync(join(installDirectory, "shuvpi.ps1"), '& "$PSScriptRoot/node_modules/.bin/shuvpi.exe" @args\n');
		return;
	}
...
if (rootPackageJson.name !== "shuvpi-monorepo") {
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `shuvpi-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "shuvpi.exe" : "shuvpi")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "shuvpi.cmd" : "shuvpi")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "shuvpi.cmd" : "shuvpi")} --help`);
	}
}
