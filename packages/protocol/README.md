# @shuv1337/shuvpi-protocol

Runtime-neutral routed envelopes, CBOR encoding, and byte-stream framing for the experimental Shuvpi protocol.
...
Chord owns the payload semantics carried inside these envelopes: `{ serviceId, instance?, member, args }` calls, the `$chord.service` control vocabulary, service catalogues, subscription snapshots and updates, service error codes, and the independent Delta path codecs for replicated states. `shuvpi-protocol` validates that each opaque payload is strict JSON but does not validate or export its Chord grammar. Clients and servers parse those values through `@shuv1337/shuvpi-chord` at the service adapter boundary.
