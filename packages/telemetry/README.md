# @shuv1337/shuvpi-telemetry

Vendor-neutral telemetry contracts and typed schema utilities for shuvpi packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Shuvpi packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Shuvpi Package Integration](#shuvpi-package-integration)
...
```bash
npm install @shuv1337/shuvpi-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@shuv1337/shuvpi-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@shuv1337/shuvpi-telemetry';
...
`@shuv1337/shuvpi-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@shuv1337/shuvpi-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@shuv1337/shuvpi-agent-core';
...
## Shuvpi Package Integration

Package ownership is intentionally split:

- `@shuv1337/shuvpi-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@shuv1337/shuvpi-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@shuv1337/shuvpi-agent-core` owns and exports the shuvpi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
The shuvpi schemas use shuvpi-owned `shuvpi.ai.*`, `shuvpi.harness.*`, and `shuvpi.session.*` names. Adapters may translate them to backend conventions without changing the emitted shuvpi vocabulary.
