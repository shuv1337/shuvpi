...
import type {
  ...
  JsonValue,
  ReplicatedState,
} from "@shuv1337/shuvpi-chord";
import { applyImmutable, type Op } from "@shuv1337/shuvpi-chord/delta";
import type {
  Message,
  Models,
  ModelThinkingLevel,
...
Pico5 targets the transcript `SystemMessage` contract from shuvpi-ai PR
...
The referenced shuvpi-ai member is:
...
type HarnessOptions = {
  readonly models: Models; // the shuvpi-ai Models interface
...
persistence; non-text content is retained as declared by its shuvpi-ai type.
...
positional `shuvpi.system` baseline or delta.
...
section order differ, one commit appends two `shuvpi.system` entries: the first
...
`shuvpi.system` entry adds `ContextEdit` omissions for every earlier `shuvpi.system`
...
changes. Shuvpi-ai decides whether to send the messages positionally to a capable
...
resolves `models.getModel(ref.provider, ref.modelId)`, builds a shuvpi-ai `Context`
...
that same model and signal. Missing models and synchronous/streamed shuvpi-ai errors
...
-> view op ["s", ["docs", "shuvpi.live", "message"], value]
```
