...
import { MemorySessionRepo, type Session } from "@shuv1337/shuvpi-agent-core";
import {
...
async function startServer(
  ...
  const server = createUnixServer(host, {
    serverId,
    path: getUnixSocketPath(serverId, "/run/user/1000/shuvpi"),
  });
  await server.start();
  return server;
}
