...
import type { Command, EventEnvelope, SessionMetadata, SessionSnapshot } from "@shuv1337/shuvpi-protocol";
...
export class LiveSessionManager {
	...
	private async acquire(id: string, acquireRuntime: () => Promise<ShuvpiSessionRuntime>): Promise<LiveSession> {
		for (;;) {
			const existing = this.liveSessions.get(id);
			if (existing) {
				if (existing.terminal)
					throw new ShuvpiServerError("session_locked", `Session runtime is terminating: ${id}`);
	...
	private async create(id: string, acquireRuntime: () => Promise<ShuvpiSessionRuntime>): Promise<LiveSession> {
		const runtime = await acquireRuntime();
		if (this.options.isClosing()) {
			await runtime.dispose();
			throw new Error("ShuvpiServer closed while acquiring a session runtime");
		}
		let live: LiveSession | undefined;
		try {
			const snapshot = await runtime.snapshot();
			if (snapshot.id !== id) {
				throw new ShuvpiServerError(
					"invalid_request",
	...
	private handleRuntimeEvent(live: LiveSession, event: ShuvpiSessionRuntimeEvent): void {
		if (event.type === "error") {
	...
	private async terminate(live: LiveSession, error: ShuvpiServerError): Promise<void> {
	...
	private async normalizedSnapshot(live: LiveSession): Promise<SessionSnapshot> {
		...
		if (snapshot.id !== live.id) {
			throw new ShuvpiServerError("invalid_request", `Runtime session ID changed from ${live.id} to ${snapshot.id}`);
		}
		return {
			...snapshot,
	...
	private async attach(connection: ConnectionState, live: LiveSession): Promise<void> {
		if (connection.disconnected || connection.stage !== "ready" || connection.connection.closed) {
			await this.maybeDispose(live);
			throw new ShuvpiServerError("invalid_request", "Connection closed while attaching to a session");
		}
	...
	private requireAttached(connection: ConnectionState, sessionId: string): LiveSession {
		if (!connection.sessionIds.has(sessionId)) {
			throw new ShuvpiServerError("invalid_request", `Connection is not attached to session ${sessionId}`);
		}
		const live = this.liveSessions.get(sessionId);
		if (!live || live.terminal || live.disposing) {
			throw new ShuvpiServerError("not_found", `Session is not live: ${sessionId}`);
		}
		return live;
	}
