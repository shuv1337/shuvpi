...
import { ShuvpiServerError } from "../errors.ts";
import type {
	CreateSessionOptions,
	PromptInput,
	ShuvpiServerService,
	ShuvpiSessionRuntime,
	ShuvpiSessionRuntimeEvent,
} from "../types.ts";
...
export class TestSessionRuntime implements ShuvpiSessionRuntime {
	readonly disposed = new Deferred<void>();
	disposeCount = 0;
	readonly steers: PromptInput[] = [];
	private readonly stored: StoredSession;
	private readonly onDispose: () => void;
	private readonly listeners = new Set<(event: ShuvpiSessionRuntimeEvent) => void>();
	...
	async prompt(input: PromptInput): Promise<void> {
		if (this.getPhase() !== "idle") throw new ShuvpiServerError("busy", "A prompt is already running");
	...
	async steer(input: PromptInput): Promise<void> {
		if (this.getPhase() === "idle") throw new ShuvpiServerError("busy", "There is no active prompt to steer");
	...
	async abort(): Promise<void> {
		if (!this.pendingPrompt) throw new ShuvpiServerError("busy", "There is no active prompt to abort");
		this.pendingPrompt.done.resolve("aborted");
	}

	async setModel(model: ModelRef): Promise<void> {
		if (this.getPhase() !== "idle") throw new ShuvpiServerError("busy", "Session is busy");
		this.update({ model });
	}
	...
	emitError(error: ShuvpiServerError): void {
...
export class TestServerService implements ShuvpiServerService {
	...
	async createSession(options: CreateSessionOptions): Promise<ShuvpiSessionRuntime> {
		this.lastCreatedId = options.id;
		if (this.sessions.has(options.id)) throw new ShuvpiServerError("session_locked", "Session already exists");
		this.seed(options.id, options.name, options.cwd, options.model, options.thinkingLevel);
		return this.acquire(options.id);
	}

	async openSession(sessionId: string): Promise<ShuvpiSessionRuntime> {
		if (!this.sessions.has(sessionId)) throw new ShuvpiServerError("not_found", `Unknown session: ${sessionId}`);
		if (this.locked.has(sessionId)) throw new ShuvpiServerError("session_locked", `Session is locked: ${sessionId}`);
	...
	seed(
		id = "session-1",
		name = `Session ${id}`,
		cwd = "/tmp/shuvpi-server-conformance",
