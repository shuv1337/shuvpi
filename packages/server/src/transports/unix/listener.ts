...
import { DEFAULT_MAX_FRAME_LENGTH } from "@shuv1337/shuvpi-protocol";
...
class UnixListener implements ShuvpiServerListener {
	...
	async start(accept: ByteConnectionAcceptor): Promise<void> {
		...
		validateUnixSocketPath(ownedBindPath, "ShuvpiServer private Unix bind path");
...
export function createUnixListener(options: UnixListenerOptions): ShuvpiServerListener {
...
function resolveUnixListenerOptions(options: UnixListenerOptions): ResolvedUnixListenerOptions {
	validateUnixSocketPath(options.path, "ShuvpiServer Unix socket path");
	const mode = options.mode ?? DEFAULT_SOCKET_MODE;
	if (!Number.isInteger(mode) || mode < 0 || mode > 0o777) {
		throw new TypeError("ShuvpiServer Unix socket mode must be an integer between 0 and 0o777");
	}
	...
	if (!Number.isSafeInteger(maxPendingBytes) || maxPendingBytes < maxFrameLength + 4) {
		throw new TypeError("ShuvpiServer maxPendingBytes must be a safe integer at least maxFrameLength + 4");
	}
	...
	if (
		...
		throw new TypeError(`ShuvpiServer gracefulCloseTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
	return {
		path: options.path,
		mode,
