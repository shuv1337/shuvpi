...
export class ShuvpiServer {
	readonly id: string;

	private readonly listeners: readonly ShuvpiServerListener[];
	...
	constructor(service: ShuvpiServerService, options: ShuvpiServerOptions) {
	...
	start(): Promise<this> {
		if (this.started) return Promise.reject(new Error("ShuvpiServer is already started"));
		if (this.startPromise) return Promise.reject(new Error("ShuvpiServer is already starting"));
		if (this.closing) return Promise.reject(new Error("ShuvpiServer is closing or closed"));
		this.startPromise = this.startInternal();
		return this.startPromise;
	}

	private async startInternal(): Promise<this> {
		const started: ShuvpiServerListener[] = [];
		try {
	...
	private toProtocolError(error: unknown): ProtocolError {
		...
		if (error instanceof ShuvpiServerError) {
...
function resolveOptions(options: ShuvpiServerOptions): { maxFrameLength: number; handshakeTimeoutMs: number } {
	if (!Array.isArray(options.listeners)) throw new TypeError("ShuvpiServer listeners must be an array");
	if (options.serverId === "") throw new TypeError("ShuvpiServer serverId must not be empty");
	const maxFrameLength = options.maxFrameLength ?? DEFAULT_MAX_FRAME_LENGTH;
	if (!Number.isSafeInteger(maxFrameLength) || maxFrameLength <= 0 || maxFrameLength > MAX_UINT32) {
		throw new TypeError(`ShuvpiServer maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
	}
	...
	if (
		...
		throw new TypeError(`ShuvpiServer handshakeTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
