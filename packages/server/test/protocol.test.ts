import { encodeCbor, encodeClientMessage, encodeFrame, PROTOCOL_VERSION } from "@shuv1337/shuvpi-protocol";
...
test("requires hello as the first message", async () => {
	...
	await client.sendMessage({
		...
		call: { serviceId: "shuvpi.session-directory", member: "list", args: [] },
	});
...
test("accepts fragmented hello and request frames", async () => {
	...
	const request = {
		...
		id: "request-1",
		target: { serverId: "00000000-0000-4000-8000-000000000001" },
		call: { serviceId: "shuvpi.session-directory" as const, member: "list" as const, args: [] as [] },
	};
