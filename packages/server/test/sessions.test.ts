...
import type { ModelMetadata, SessionSnapshot, TranscriptProgress } from "@shuv1337/shuvpi-protocol";
import { afterEach, describe, expect, test } from "vitest";
import type { CreateSessionOptions, ShuvpiServer, ShuvpiSessionRuntime } from "../src/index.ts";
import {
...
const servers = new Set<ShuvpiServer>();
...
async function connect(server: ShuvpiServer): Promise<Client> {
...
describe("ShuvpiServer Unix integration", () => {
	...
	test("rejects and disposes a service runtime with the wrong server-assigned ID", async () => {
		class WrongIdService extends MemoryService {
			override async createSession(options: CreateSessionOptions): Promise<ShuvpiSessionRuntime> {
