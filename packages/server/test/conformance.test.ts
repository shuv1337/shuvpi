import { BACKGROUND_CONTEXT, type SessionMetadata } from "@shuv1337/shuvpi-agent-core";
import type { ServiceCall } from "@shuv1337/shuvpi-chord";
...
describe("Session protocol", () => {
	...
	test("routes opaque server services and publishes attachment changes out of band", async () => {
				...
				attachClient(presentation) {
					return {
						async invokeService(call, _publish, context) {
							if (call.serviceId !== "shuvpi.session-management") throw new Error("Unexpected service");
		...
		await expect(
			client.requestService({ serverId }, { serviceId: "shuvpi.session-management", member: "detach", args: [] }),
