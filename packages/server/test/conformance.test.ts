...
import { encodeClientMessage, encodeFrame, PROTOCOL_VERSION } from "@shuv1337/shuvpi-protocol";
import { afterEach, describe, expect, test } from "vitest";
import { InternalServerError, NotImplementedError, type ShuvpiServer, ShuvpiServerError } from "../src/index.ts";
...
): Promise<{ server: ShuvpiServer; service: TestServerService }> {
...
async function connect(server: ShuvpiServer): Promise<ProtocolTestClient> {
...
describe("Unix transport conformance", () => {
	...
	test("disconnects attached clients when a runtime reports a terminal error", async () => {
		...
		runtime.emitError(new ShuvpiServerError("session_locked", "lock ownership lost"));
		await client.waitForClose();
