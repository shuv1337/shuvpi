...
import { ServerMessageDecoder } from "@shuv1337/shuvpi-protocol";
import { afterEach, expect, test, vi } from "vitest";
import type { ByteConnection } from "../src/connection.ts";
import { ShuvpiServer } from "../src/index.ts";
...
let server: ShuvpiServer | undefined;
...
test("requires explicit listeners", () => {
	expect(() => Reflect.construct(ShuvpiServer, [service, {}])).toThrow(/listeners/);
});
...
test("handshake timeout cleanup does not wait for a blocked output queue", async () => {
	...
	const core = new ShuvpiServer(service, {
		listeners: [],
		maxFrameLength: 1024,
...
test("rejects timeout values above Node's maximum timer delay", () => {
	const unix = { path: "/tmp/shuvpi-server-timeout-test.sock" };
