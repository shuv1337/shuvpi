...
const SHUVPI_USER_AGENT = `shuvpi (${platform()} ${release()}; ${arch()})`;
...
describe("xAI Responses provider", () => {
	...
	it("uses /responses with bearer auth and xAI-compatible request fields", async () => {
		...
		expect(captured.headers.get("user-agent")).toBe(SHUVPI_USER_AGENT);
	...
	it("keeps the SDK User-Agent for non-xAI Responses requests", async () => {
		...
		expect(userAgent).not.toBe(SHUVPI_USER_AGENT);
	});

	it("forces Shuvpi's User-Agent on custom xAI Completions models over caller headers", async () => {
		let userAgent: string | null = null;
		vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
			userAgent = new Request(input, init).headers.get("user-agent");
			const chunks = [
		...
		expect(result.stopReason, result.errorMessage).toBe("stop");
		expect(userAgent).toBe(SHUVPI_USER_AGENT);
	});
});
