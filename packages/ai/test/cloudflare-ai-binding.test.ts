...
/** Collect headers without iterating: the checked TS lib set does not mark Headers iterable. */
...
describe("createAiBindingFetch", () => {
	it("passes requests to the binding untouched", async () => {
		...
		expect(headersToObject(requests[0].headers)).toEqual({
			"content-type": "application/json",
	...
	it("keeps SDK placeholder auth off the wire when paired with null auth headers", async () => {
		...
		const headerNames = Object.keys(headersToObject(requests[0].headers));
