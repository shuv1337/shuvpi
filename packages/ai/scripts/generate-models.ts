...
const GITHUB_COPILOT_THINKING_LEVEL_OVERRIDES = {
	...
	// models.dev lists only xhigh/max; keep the levels from the 2026-09-22 Copilot catalog fallback.
	"claude-opus-5.5": { off: null, minimal: null, low: "low", medium: "medium", high: "high" },
...
function supportsOpenAiMax(model: Model<Api>): boolean {
	return (
		(model.id.includes("gpt-5.6") || model.id.includes("gpt-6") || OPENAI_DAYBREAK_BLUE_MODEL_IDS.has(model.id)) &&
		(model.api === "openai-responses" ||
			model.api === "azure-openai-responses" ||
			model.api === "openai-codex-responses" ||
...
function processGoogleModels(data: ModelsDevCatalog): Model<Api>[] {
	...
	if (vertexModels) {
		for (const [modelId, model] of Object.entries(vertexModels)) {
			...
			// do not match the official Gemini API standard pricing table. shuvpi only accounts
