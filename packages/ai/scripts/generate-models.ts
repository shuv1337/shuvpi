...
function getBasetenCompat(modelId: string): OpenAICompletionsCompat {
	...
	if (BASETEN_CHAT_TEMPLATE_ARGS_MODELS.has(modelId)) {
		return {
			...
			chatTemplateKwargs: { ...BASETEN_CHAT_TEMPLATE_ARGS_COMPAT.chatTemplateKwargs },
		};
	}
...
function applyThinkingLevelMetadata(model: Model<any>): void {
	...
	if (model.provider === "openrouter" && model.id.startsWith("inception/mercury-2")) {
		...
		// Shuvpi's low/medium/high pass through verbatim; OpenRouter normalizes to Mercury's vocabulary.
		mergeThinkingLevelMap(model, { off: null });
	}
...
async function generateModels() {
	...
	// Context windows follow the Codex backend catalog; GPT-5.6 uses its 372k limit.
