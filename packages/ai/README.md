...
- **Fireworks AI** (Fire Pass, uses Anthropic-compatible API)
...
```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@shuv1337/pi-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@shuv1337/pi-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@shuv1337/pi-ai';
...
```typescript
import { getImageModel, generateImages } from '@shuv1337/pi-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@shuv1337/pi-ai';
...
```typescript
import { getModel, stream } from '@shuv1337/pi-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@shuv1337/pi-ai';
...
```typescript
import { Model, stream } from '@shuv1337/pi-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@shuv1337/pi-ai';
...
```typescript
import { getModel, complete, Context } from '@shuv1337/pi-ai';
...
```typescript
import { Context, getModel, complete } from '@shuv1337/pi-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@shuv1337/pi-ai/oauth` entry point in Node.js.
...
| Fireworks (Fire Pass) | `FIREWORKS_API_KEY` |
...
```typescript
import { getEnvApiKey } from '@shuv1337/pi-ai';
...
The library provides login and token refresh functions via the `@shuv1337/pi-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@shuv1337/pi-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@shuv1337/pi-ai/oauth';
...
```typescript
import { getModel, complete } from '@shuv1337/pi-ai';
import { getOAuthApiKey } from '@shuv1337/pi-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@shuv1337/pi-ai`
