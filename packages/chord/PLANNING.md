...
The current Shuvpi experiments prove many required behaviors, but Chord will be implemented from scratch. Existing source may be used as test and design evidence, not copied into this package. Compatibility with experimental APIs or wire messages is not a requirement.
...
```text
@shuv1337/shuvpi-chord
        ↑
Shuvpi agent, protocol, server, coding agent, TUI, and future applications
```

Chord must:

- have no dependency on another Shuvpi workspace package;
- contain no imports from `@shuv1337/shuvpi-*` or relative paths outside `packages/chord`;
...
- be buildable, testable, packable, and usable without resolving another Shuvpi package.
...
Current experiments depend on Shuvpi's Harness `Context`, which Chord cannot import. Chord therefore needs a small neutral invocation context.
...
Applications may define their own context keys. A Shuvpi adapter can carry telemetry and authenticated identity through such keys without Chord knowing their types.
...
The wire protocol contains no server ID, session ID, attachment ID, route, user identity, or host kind. A Shuvpi router may wrap or forward Chord envelopes using its own control fields without parsing service business payloads.
...
- `@shuv1337/shuvpi-chord` must be externalized so a plugin uses the host's one runtime and branding symbols.
...
- Bundle output must not depend on Shuvpi's repository path aliases.
...
|---|---|
| `packages/coding-agent/src/experimental/services/connection.ts` | Shuvpi connection state, selected-session attachment, route rebinding, Shuvpi client adapter |
| `packages/coding-agent/src/experimental/services/server.ts` | server-wide session directory and management implementations |
| `packages/coding-agent/src/experimental/services/worker.ts` | Session worker host construction and Shuvpi protocol publication adapter |
...
| `source-resolver.ts` and Shuvpi internal process entrypoints | Shuvpi source execution and process policy |

The Pico5 Chord usage guide, experimental service tests, and remote plugin fixture are behavioral input. They are not normative Chord APIs. Once migration finishes, generic semantics should be documented in Chord and Shuvpi documents should cover only their host-specific contracts and adapters.

Migration should happen only after Chord passes its standalone conformance suite:

1. implement Chord without changing Shuvpi callers;
2. add thin Shuvpi adapters and migrate generic service imports;
3. migrate the experimental plugin host and loaders;
4. adapt Shuvpi's routed protocol to carry Chord envelopes;
...
If Node-only APIs are exported, they should use a separate package export such as `@shuv1337/shuvpi-chord/node` or `@shuv1337/shuvpi-chord/bundler`; importing the main runtime must not load Node-only modules.
...
- add package-boundary checks preventing Shuvpi imports; and
...
Tests bundle an application-neutral fixture with two opaque entries and a third-party dependency, load each independently, activate it, reload changed source, prove unchanged source hashes are stable, reject corrupt manifests/integrity, and prove output resolves no Shuvpi packages.
...
- Shuvpi adapters for context metadata and routed RPC transport;
...
Exit condition: Shuvpi depends on Chord, while Chord remains independently packable and contains no Shuvpi imports.
...
- Shuvpi host APIs or built-in Shuvpi services;
...
- backward compatibility with experimental Shuvpi APIs or wire formats.
...
None of these decisions should introduce Shuvpi concepts into Chord.

## 17. Definition of done

Chord's initial scope is complete when:

- a standalone application unrelated to Shuvpi can define local and remote singleton/keyed services;
...
- the packed package works outside the Shuvpi monorepo; and
- an automated boundary check proves that Chord has no imports or dependencies on the rest of Shuvpi.
