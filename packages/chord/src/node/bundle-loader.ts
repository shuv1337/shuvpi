...
function resolveExternalTarget(specifier: string, resolver?: FacetBundleExternalResolver): string {
	...
	if (specifier === "@shuv1337/shuvpi-chord") return new URL(`../index.${extension}`, import.meta.url).href;
	if (specifier === "@shuv1337/shuvpi-chord/context") {
		return new URL(`../context/index.${extension}`, import.meta.url).href;
	}
	if (specifier === "@shuv1337/shuvpi-chord/node") return new URL(`../node.${extension}`, import.meta.url).href;
