...
describe("service wire protocol", () => {
	test("encodes control calls and validates service values", () => {
		...
		expect(
			parseServiceCatalogue([
				{ serviceId: "shuvpi.models", mode: "singleton" },
				{ serviceId: "shuvpi.dialogs", mode: "keyed" },
			]),
		).toEqual([
			{ serviceId: "shuvpi.models", mode: "singleton" },
			{ serviceId: "shuvpi.dialogs", mode: "keyed" },
		]);
		const subscribe = createServiceSubscribeCall("subscription-1", "shuvpi.models", "singleton");
		expect(decodeServiceControlCall(subscribe)).toEqual({
			type: "subscribe",
			subscriptionId: "subscription-1",
			serviceId: "shuvpi.models",
			mode: "singleton",
		});
		...
		expect(
			parseServiceCall({
				serviceId: "shuvpi.question-dialog",
	...
	test("rejects malformed service values", () => {
		expect(() => parseServiceCall({ serviceId: "shuvpi.models", member: "list", args: [], extra: true })).toThrow(
			"Invalid service call",
		);
		expect(() => parseServiceCatalogue([{ serviceId: "shuvpi.models", mode: "unknown" }])).toThrow(
