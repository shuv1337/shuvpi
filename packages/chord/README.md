...
the Shuvpi monorepo, but it is not a Shuvpi package: it does not depend on any other Shuvpi
...
`@shuv1337/shuvpi-chord`. Import public types and general runtime APIs from the
package root. Context constants and functions live in
`@shuv1337/shuvpi-chord/context` because their generic names should not pollute
the root API.
...
Import the standalone delta primitive from `@shuv1337/shuvpi-chord/delta`:

```ts
import { apply, track } from "@shuv1337/shuvpi-chord/delta";
...
`@shuv1337/shuvpi-chord/bundler` uses esbuild to turn ESM or TypeScript application
...
```ts
import { bundleFacetPackage } from "@shuv1337/shuvpi-chord/bundler";
...
```ts
import { createFacetBundleLoader } from "@shuv1337/shuvpi-chord/node";
