...
```bash
npm install @shuv1337/shuvpi-mom
```
...
# Option 2: use /login command in shuvpi agent, then copy/link auth.json to ~/.shuvpi/mom/
...
- run interactive coding agent session: `npx @shuv1337/shuvpi-coding-agent`
- enter `/login` command
  - choose "Anthropic" provider
  - follow instructions in the browser
- link `auth.json` to mom: `ln -s ~/.shuvpi/agent/auth.json ~/.shuvpi/mom/auth.json`
...
You can find a set of basic skills at [github.com/badlogic/shuvpi-skills](https://github.com/badlogic/shuvpi-skills). Just tell mom to clone this repository into `/workspace/skills/shuvpi-skills` and she'll help you set up the rest.
...
Update mom anytime with `npm install -g @shuv1337/shuvpi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
