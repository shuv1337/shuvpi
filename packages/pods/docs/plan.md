...
Shuvpi automates vLLM deployment on GPU pods from DataCrunch, Vast.ai, Prime Intellect, RunPod (or any Ubuntu machine with NVIDIA GPUs). It manages multiple concurrent model deployments via separate vLLM instances, each accessible through the OpenAI API protocol with API key authentication.

Pods are treated as ephemeral - spin up when needed, tear down when done. To avoid re-downloading models (30+ minutes for 100GB+ models), shuvpi uses persistent network volumes for model storage that can be shared across pods on the same provider. This minimizes both cost (only pay for active compute) and setup time (models already cached).

## Usage

### Pods
```bash
shuvpi pods setup dc1 "ssh root@1.2.3.4" --mount "mount -t nfs..."  # Setup pod (requires HF_TOKEN, SHUVPI_API_KEY env vars)
shuvpi pods                              # List all pods (* = active)
shuvpi pods active dc2                   # Switch active pod
shuvpi pods remove dc1                   # Remove pod
```

### Models
```bash
shuvpi start Qwen/Qwen2.5-72B-Instruct --name qwen72b          # Known model - shuvpi handles vLLM args
shuvpi start some/unknown-model --name mymodel --vllm --tensor-parallel-size 4 --max-model-len 32768  # Custom vLLM args
shuvpi list                              # List running models with ports
shuvpi stop qwen72b                      # Stop model
shuvpi logs qwen72b                      # View model logs
```

For known models, shuvpi automatically configures appropriate vLLM arguments from model documentation based on the hardware of the pod. For unknown models or custom configurations, pass vLLM args after `--vllm`.

## Pod management

Shuvpi manages GPU pods from various providers (DataCrunch, Vast.ai, Prime Intellect, RunPod) as ephemeral compute resources. Users manually create pods via provider dashboards, then register them with shuvpi for automated setup and management.
...
When a user creates a fresh pod on a provider, they register it with shuvpi using the SSH command from the provider:

```bash
shuvpi pods setup dc1 "ssh root@1.2.3.4" --mount "mount -t nfs..."
```
...
- `SHUVPI_API_KEY`: API key for securing vLLM endpoints
...
shuvpi pods                    # List all pods (* = active)
shuvpi pods active dc2         # Switch active pod
shuvpi pods remove dc1         # Remove pod from local config but doesn't destroy pod remotely.
```

All model commands (`shuvpi start`, `shuvpi stop`, etc.) target the active pod, unless `--pod <podname>` is given, which overrides the active pod for that command.

## Model deployment

Shuvpi uses direct SSH commands to manage vLLM instances on pods. No remote manager component is needed - everything is controlled from the local shuvpi CLI.

### Architecture
The shuvpi CLI maintains all state locally in `~/.shuvpi/pods.json`:
```json
{
  "pods": {
    "dc1": {
...
The location of the shuvpi config dir can also be specified via the `SHUVPI_CONFIG_DIR` env var, e.g. for testing.

Pods are assumed to be fully managed by shuvpi - no other processes compete for ports or GPUs.

### Starting models
When user runs `shuvpi start Qwen/Qwen2.5-72B --name qwen`:
...
4. Builds vLLM command with appropriate args and SHUVPI_API_KEY
...
### Testing models
The `shuvpi prompt` command provides a quick way to test deployed models:
```bash
shuvpi prompt qwen "What is 2+2?"                    # Simple prompt
shuvpi prompt qwen "Read file.txt and summarize"     # Uses built-in tools
```
...
See [models.md](./models.md) for a list of models, their HW reqs, vLLM args and notes, we want to support out of the box with a simple `shuvpi start <model-name> --name <local-name>`