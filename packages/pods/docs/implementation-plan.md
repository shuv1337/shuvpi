...
- [ ] `src/setup/generate-setup-script.ts` - Generate bash script as string
  ...
  - [ ] Generate env var setup (HF_TOKEN, SHUVPI_API_KEY)
...
- [ ] `src/config/store.ts` - Read/write ~/.shuvpi/pods.json
...
- [ ] `src/commands/pods-setup.ts` - shuvpi pods setup
  - [ ] Parse args (name, ssh, mount)
  - [ ] Check env vars (HF_TOKEN, SHUVPI_API_KEY)
  - [ ] Call setup executor
  - [ ] Save pod to config

- [ ] `src/commands/pods-list.ts` - shuvpi pods
  - [ ] Load config
  - [ ] Display all pods with active marker

- [ ] `src/commands/pods-active.ts` - shuvpi pods active
  - [ ] Switch active pod
  - [ ] Update config

- [ ] `src/commands/pods-remove.ts` - shuvpi pods remove
...
- [ ] `src/commands/start.ts` - shuvpi start
...
- [ ] `src/commands/stop.ts` - shuvpi stop
  - [ ] Find model in config
  - [ ] Kill process via PID
  - [ ] Clean up config

- [ ] `src/commands/list.ts` - shuvpi list
  - [ ] Show models from config
  - [ ] Optionally verify PIDs

- [ ] `src/commands/logs.ts` - shuvpi logs
...
- [ ] `src/commands/prompt.ts` - shuvpi prompt
