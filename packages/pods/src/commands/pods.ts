...
export const listPods = () => {
	...
	if (podNames.length === 0) {
		console.log("No pods configured. Use 'shuvpi pods setup' to add a pod.");
		return;
	}
...
export const setupPod = async (
	...
	const vllmApiKey = process.env.SHUVPI_API_KEY;
	...
	if (!vllmApiKey) {
		console.error(chalk.red("ERROR: SHUVPI_API_KEY environment variable is required"));
		console.error("Set an API key: export SHUVPI_API_KEY=your_api_key_here");
		process.exit(1);
	}
	...
	console.log(chalk.cyan(`  shuvpi start <model> --name <name>`));
};
