...
chmod +x /tmp/model_run_${name}.sh`,
	...
	const env = [
		...
		`SHUVPI_API_KEY='${process.env.SHUVPI_API_KEY}'`,
...
WRAPPER
	...
	if (startupFailed) {
		...
		console.log(`\n${chalk.cyan(`Check full logs: shuvpi ssh "tail -100 ~/.vllm_logs/${name}.log"`)}`);
		process.exit(1);
		...
		console.log(chalk.white("API Key:     ") + chalk.yellow(process.env.SHUVPI_API_KEY || "(not set)"));
		console.log(chalk.cyan("─".repeat(50)));

		console.log(`\n${chalk.bold("Export for shell:")}`);
		console.log(chalk.gray(`export OPENAI_BASE_URL="http://${host}:${port}/v1"`));
		console.log(chalk.gray(`export OPENAI_API_KEY="${process.env.SHUVPI_API_KEY || "your-api-key"}"`));
  ...
  curl $OPENAI_BASE_URL/chat/completions \\
    ...
    -d '{"model":"${modelId}","messages":[{"role":"user","content":"Hi"}]}'`),
		...
		console.log("");
		console.log(chalk.cyan(`Chat with model:  shuvpi agent ${name} "Your message"`));
		console.log(chalk.cyan(`Interactive mode: shuvpi agent ${name} -i`));
		console.log(chalk.cyan(`Monitor logs:     shuvpi logs ${name}`));
		console.log(chalk.cyan(`Stop model:       shuvpi stop ${name}`));
	} else if (interrupted) {
		console.log(chalk.yellow("\n\nStopped monitoring. Model deployment continues in background."));
		console.log(chalk.cyan(`Chat with model: shuvpi agent ${name} "Your message"`));
		console.log(chalk.cyan(`Check status: shuvpi logs ${name}`));
		console.log(chalk.cyan(`Stop model: shuvpi stop ${name}`));
	} else {
...
export const listModels = async (options: { pod?: string }) => {
	...
	for (const name of modelNames) {
		...
		if (status === "dead") {
			...
			console.log(chalk.red(`  ${name}: vLLM crashed (check logs with 'shuvpi logs ${name}')`));
			anyDead = true;
	...
	if (anyDead) {
		...
		console.log(chalk.cyan("  shuvpi stop <name>"));
	} else {
...
export const showKnownModels = async () => {
	...
	if (activePod) {
		...
		console.log(chalk.yellow("No active pod. Use 'shuvpi pods active <name>' to filter compatible models.\n"));
	}

	console.log("Usage: shuvpi start <model> --name <name> [options]\n");
