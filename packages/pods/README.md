...
```bash
npm install -g @shuv1337/shuvpi-pods
```

## What is shuvpi?

`shuvpi` simplifies running large language models on remote GPU pods. It automatically:
...
export SHUVPI_API_KEY=your_api_key              # Any string you want for API authentication

# Setup a DataCrunch pod with NFS storage (models path auto-extracted)
shuvpi pods setup dc1 "ssh root@1.2.3.4" \
...
shuvpi start Qwen/Qwen2.5-Coder-32B-Instruct --name qwen

# Send a single message to the model
shuvpi agent qwen "What is the Fibonacci sequence?"
...
export OPENAI_API_KEY=$SHUVPI_API_KEY
```
...
```bash
shuvpi pods setup <name> "<ssh>" [options]        # Setup new pod
...
shuvpi pods                                       # List all configured pods
shuvpi pods active <name>                         # Switch active pod
shuvpi pods remove <name>                         # Remove pod from local config
shuvpi shell [<name>]                             # SSH into pod
shuvpi ssh [<name>] "<command>"                   # Run command on pod
```
...
```bash
shuvpi start <model> --name <name> [options]  # Start a model
...
shuvpi stop [<name>]          # Stop model (or all if no name given)
shuvpi list                   # List running models with status
shuvpi logs <name>            # Stream model logs (tail -f)
```

### Agent & Chat Interface

```bash
shuvpi agent <name> "<message>"               # Single message to model
shuvpi agent <name> "<msg1>" "<msg2>"         # Multiple messages in sequence
shuvpi agent <name> -i                        # Interactive chat mode
shuvpi agent <name> -i -c                     # Continue previous session

# Standalone OpenAI-compatible agent (works with any API)
shuvpi-agent --base-url http://localhost:8000/v1 --model llama-3.1 "Hello"
shuvpi-agent --api-key sk-... "What is 2+2?"  # Uses OpenAI by default
shuvpi-agent --json "What is 2+2?"            # Output event stream as JSONL
shuvpi-agent -i                                # Interactive mode
```
...
`shuvpi` includes predefined configurations for popular agentic models, so you do not have to specify `--vllm` arguments manually. `shuvpi` will also check if the model you selected can actually run on your pod with respect to the number of GPUs and available VRAM. Run `shuvpi start` without additional arguments to see a list of predefined models that can run on the active pod.
...
shuvpi start Qwen/Qwen3-Coder-30B-A3B-Instruct --name qwen3

# Qwen3-Coder-480B - State-of-the-art on 8xH200 (data-parallel mode)
shuvpi start Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8 --name qwen-480b
```

### GPT-OSS Models
```bash
# Requires special vLLM build during setup
shuvpi pods setup gpt-pod "ssh root@1.2.3.4" --models-path /workspace --vllm gpt-oss

# GPT-OSS-20B - Fits on 16GB+ VRAM
shuvpi start openai/gpt-oss-20b --name gpt20

# GPT-OSS-120B - Needs 60GB+ VRAM
shuvpi start openai/gpt-oss-120b --name gpt120
```

### GLM Models
```bash
# GLM-4.5 - Requires 8-16 GPUs, includes thinking mode
shuvpi start zai-org/GLM-4.5 --name glm

# GLM-4.5-Air - Smaller version, 1-2 GPUs
shuvpi start zai-org/GLM-4.5-Air --name glm-air
```
...
shuvpi start deepseek-ai/DeepSeek-V3 --name deepseek --vllm \
  --tensor-parallel-size 4 --trust-remote-code

# Mistral with pipeline parallelism
shuvpi start mistralai/Mixtral-8x22B-Instruct-v0.1 --name mixtral --vllm \
  --tensor-parallel-size 8 --pipeline-parallel-size 2

# Any model with specific tool parser
shuvpi start some/model --name mymodel --vllm \
...
shuvpi pods setup dc1 "ssh root@instance.datacrunch.io" \
  --mount "sudo mount -t nfs -o nconnect=16 nfs.fin-02.datacrunch.io:/your-pseudo /mnt/hf-models"
...
```bash
# With network volume
shuvpi pods setup runpod "ssh root@pod.runpod.io" --models-path /runpod-volume

# Or use workspace (persists with pod but not shareable)
shuvpi pods setup runpod "ssh root@pod.runpod.io" --models-path /workspace
```
...
When running multiple models, shuvpi automatically assigns them to different GPUs:
```bash
shuvpi start model1 --name m1  # Auto-assigns to GPU 0
shuvpi start model2 --name m2  # Auto-assigns to GPU 1
shuvpi start model3 --name m3  # Auto-assigns to GPU 2
```
...
shuvpi start Qwen/Qwen2.5-Coder-32B-Instruct --name qwen --gpus 1

# Run GLM-4.5 on 8 GPUs (if it has an 8-GPU config)
shuvpi start zai-org/GLM-4.5 --name glm --gpus 8
```
...
shuvpi start meta-llama/Llama-3.1-70B-Instruct --name llama70b --vllm \
  --tensor-parallel-size 4

# Specific GPU count
shuvpi start Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8 --name qwen480 --vllm \
...
`shuvpi` includes a standalone OpenAI-compatible agent that can work with any API:

```bash
# Install globally to get shuvpi-agent command
npm install -g @shuv1337/shuvpi-pods

# Use with OpenAI
shuvpi-agent --api-key sk-... "What is machine learning?"

# Use with local vLLM
shuvpi-agent --base-url http://localhost:8000/v1 \
...
shuvpi-agent --continue "Follow up question"

# Custom system prompt
shuvpi-agent --system-prompt "You are a Python expert" "Write a web scraper"

# Use responses API (for GPT-OSS models)
shuvpi-agent --api responses --model openai/gpt-oss-20b "Hello"
```
...
`shuvpi` automatically configures appropriate tool calling parsers for known models:
...
```bash
shuvpi start model --name mymodel --vllm --disable-tool-call-parser
```
...
shuvpi start Qwen/Qwen2.5-Coder-32B-Instruct --name coder \
...
shuvpi agent qwen -i -c
```

Sessions are stored in `~/.shuvpi/sessions/` organized by project path and include:
...
```bash
shuvpi-agent --api-key sk-... --json "What is 2+2?"
```
...
- Check CUDA compatibility with `shuvpi ssh "nvidia-smi"`
...
```bash
# Good
shuvpi agent qwen "What is this file about?"
...
shuvpi start model --name test --pod dev-pod
shuvpi list --pod prod-pod
shuvpi stop test --pod dev-pod
```

### Custom vLLM Arguments
```bash
# Pass any vLLM argument after --vllm
shuvpi start model --name custom --vllm \
  --quantization awq \
...
shuvpi ssh "watch -n 1 nvidia-smi"

# Check model downloads
shuvpi ssh "du -sh ~/.cache/huggingface/hub/*"

# View all logs
shuvpi ssh "ls -la ~/.vllm_logs/"

# Check agent session history
ls -la ~/.shuvpi/sessions/
```

## Environment Variables

- `HF_TOKEN` - HuggingFace token for model downloads
- `SHUVPI_API_KEY` - API key for vLLM endpoints
- `SHUVPI_CONFIG_DIR` - Config directory (default: `~/.shuvpi`)
- `OPENAI_API_KEY` - Used by `shuvpi-agent` when no `--api-key` provided
