...
import type { AssistantMessage, Context, Model, ProviderStreamOptions } from "@shuv1337/shuvpi-ai";
...
async function getCompleteFunction(): Promise<CompleteFunction> {
  completeFunctionPromise ??= (async () => {
    const piAi = await import("@shuv1337/shuvpi-ai");
    const complete = Reflect.get(piAi, "complete");
    if (typeof complete === "function") {
      return complete as CompleteFunction;
    }

    const compat = await import("@shuv1337/shuvpi-ai/compat");
