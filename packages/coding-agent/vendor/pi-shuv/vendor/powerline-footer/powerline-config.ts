...
export function parsePowerlineConfig(value: unknown, presets: readonly StatusLinePreset[]): PowerlineConfig {
  const defaultConfig: PowerlineConfig = {
    // Match pre-0.7 pack defaults: classic p10k footer, no fixed-editor takeover.
    preset: "p10k",
    customItems: [],
    disabledSegments: [],
  ...
  return {
    ...
    // Prefer explicit booleans; fall back to pack defaults (fixedEditor/welcome off).
    mouseScroll: typeof value.mouseScroll === "boolean" ? value.mouseScroll : defaultConfig.mouseScroll,
    fixedEditor: typeof value.fixedEditor === "boolean" ? value.fixedEditor : defaultConfig.fixedEditor,
    placement,
    invalidPlacement,
    welcome: typeof value.welcome === "boolean" ? value.welcome : defaultConfig.welcome,
