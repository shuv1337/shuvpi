import type { EditToolDetails, ExtensionAPI, Theme } from "@shuv1337/shuvpi-coding-agent";
import { createEditTool, getLanguageFromPath, highlightCode } from "@shuv1337/shuvpi-coding-agent";
import { Text, truncateToWidth, type Component, visibleWidth } from "@shuv1337/shuvpi-tui";
...
function extractEditedPath(message: string): string | undefined {
	const patterns = [
		/Successfully replaced \d+ block\(s\) in (.+)\.$/,
...
function isLightTheme(theme: Theme, background?: RgbColor | null): boolean {
	if (background) return getRelativeLuminance(background) >= 0.55;
	return theme.name?.toLowerCase().includes("light") === true;
}

function resolveDiffPalette(theme: Theme): DiffPalette {
	const resolvedBaseBg =
		parseAnsiColorCode(theme.getBgAnsi("toolSuccessBg")) ??
		parseAnsiColorCode(theme.getBgAnsi("toolPendingBg"));
	const light = isLightTheme(theme, resolvedBaseBg);
	const baseBg = resolvedBaseBg ?? (light ? { r: 250, g: 250, b: 250 } : { r: 32, g: 35, b: 42 });
	const addFg =
		parseAnsiColorCode(theme.getFgAnsi("toolDiffAdded")) ??
		(light ? { r: 34, g: 139, b: 34 } : { r: 88, g: 173, b: 88 });
	const removeFg =
		parseAnsiColorCode(theme.getFgAnsi("toolDiffRemoved")) ??
		(light ? { r: 178, g: 34, b: 34 } : { r: 196, g: 98, b: 98 });

	// Light backgrounds need stronger tint ratios to remain visible
	const addRowMix = light ? 0.35 : ADD_ROW_BACKGROUND_MIX_RATIO;
	const removeRowMix = light ? 0.2 : REMOVE_ROW_BACKGROUND_MIX_RATIO;
	const addEmphasisMix = light ? 0.55 : ADD_INLINE_EMPHASIS_MIX_RATIO;
	const removeEmphasisMix = light ? 0.35 : REMOVE_INLINE_EMPHASIS_MIX_RATIO;

	const addRowBg = mixRgb(baseBg, addFg, addRowMix);
	const removeRowBg = mixRgb(baseBg, removeFg, removeRowMix);
	const addEmphasisBg = mixRgb(baseBg, addFg, addEmphasisMix);
	const removeEmphasisBg = mixRgb(baseBg, removeFg, removeEmphasisMix);
...
class SplitDiffComponent implements Component {
	...
	constructor(
		...
		const bgColor = parseAnsiColorCode(bg);
		this.containerBgAnsi =
			bg === "\x1b[49m"
				? isLightTheme(theme, bgColor)
...
export default function diffRendererExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			const sourcePath = args.path || extractEditedPath(message);
