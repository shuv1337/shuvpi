...
import { getBuiltinModel } from "@shuv1337/shuvpi-ai/providers/all";
...
describe("cache warming", () => {
	it("derives eligibility and timing from retention and provider behavior", () => {
		expect([
			...
			getPromptCacheTtlMs(adaptiveModel, { env: { SHUVPI_CACHE_RETENTION: "long" } }),
...
describe("ExtensionRunner.emitCacheWarmingDecision", () => {
	it("uses the last extension override", async () => {
		...
		const factories: ExtensionFactory[] = [
			(shuvpi) => {
				shuvpi.on("cache_warming_decision", () => ({ action: "warm" }));
			},
			(shuvpi) => {
				shuvpi.on("cache_warming_decision", () => ({ action: "stop" }));
			},
		];
		const extensions = [];
