import type { NativeClipboard } from "@shuv1337/shuvpi-tui";
...
vi.mock("@shuv1337/shuvpi-tui", () => ({ getNativeClipboard: mocks.getNativeClipboard }));
	...
	test("WSL: tries PowerShell before a broken native X11 bridge", async () => {
		...
		mocks.command.mockImplementation(async (command, args, options) => {
			...
			if (command === "powershell.exe") {
				...
				expect(spawnOptions.env?.SHUVPI_WSL_CLIPBOARD_IMAGE_PATH).toBeUndefined();
