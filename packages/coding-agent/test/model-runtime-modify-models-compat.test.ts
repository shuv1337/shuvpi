...
describe("extension provider model lifecycle", () => {
	it("registers native shuvpi-ai providers with their auth implementation", async () => {
	...
	it("preserves native deferred methods through provider overlays", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "shuvpi-native-provider-deferred-"));
	...
	it("applies models.json overrides above native providers", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "shuvpi-native-provider-"));
		const modelsPath = join(tempDir, "models.json");
		writeFileSync(
			modelsPath,
			JSON.stringify({
