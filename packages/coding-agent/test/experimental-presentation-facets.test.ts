...
import { readFacetBundleManifest } from "@shuv1337/shuvpi-chord/node";
...
describe("server-selected presentation facets", () => {
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		...
		await writeFile(
			...
			`${JSON.stringify({
				...
				version: "1.0.0",
				peerDependencies: {
					"@shuv1337/shuvpi-chord": "^0.84.4",
					"@shuv1337/shuvpi-coding-agent": "^0.84.4",
				},
			})}\n`,
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@shuv1337/shuvpi-chord"; import { SlashCommands } from "@shuv1337/shuvpi-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@shuv1337/shuvpi-chord"; import { SlashCommands } from "@shuv1337/shuvpi-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					...
					version: "1.0.0",
					peerDependencies: { "@shuv1337/shuvpi-chord": "^0.84.4" },
				})}\n`,
			),
			writeFile(
				join(secondPackagePath, "src", "tui.ts"),
				'import { defineFacet } from "@shuv1337/shuvpi-chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@shuv1337/shuvpi-chord"; import { SlashCommands } from "@shuv1337/shuvpi-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
	...
	test("builds the example plugin package without a package-owned build script", async () => {
		...
		expect(manifest.plugin).toEqual({ id: "@shuv1337/shuvpi-example-plugin", version: "1.0.0" });
		...
		expect(loaded.facets.map(({ id }) => id)).toEqual(["@shuv1337/shuvpi-example-plugin/tui"]);
