...
} from "@shuv1337/shuvpi-chord/node";
import { ProcessTerminal, TuiMainScreen } from "@shuv1337/shuvpi-tui";
...
describe("experimental client TUI", () => {
	...
	test.each([
		...
		async (kind, command, sessionId, creates) => {
			...
			const reloadSource =
				'"use strict";\nconst { defineFacet, defineService } = require("@shuv1337/shuvpi-chord");\nconst Models = defineService("shuvpi.models");\nmodule.exports = { __esModule: true, default: defineFacet({ id: "test-tui-facet", setup(env) { env.use(Models); } }) };\n';
			const reloadArtifact: FacetBundleArtifact = {
				...
				entry: {
					...
					externalImports: ["@shuv1337/shuvpi-chord"],
				},
				source: reloadSource,
			};
