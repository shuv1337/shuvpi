...
describe("ExtensionRunner", () => {
	...
	describe("error handling", () => {
		...
		it.each([
			...
			const extCode = `
				export default function(shuvpi) {
					shuvpi.on("user_bash", async () => (${handlerResult}));
				}
			`;
		...
		it("accepts valid user_bash operations and result overrides", async () => {
			const extCode = `
				export default function(shuvpi) {
					shuvpi.on("user_bash", async (event) => {
	...
	describe("event subscriptions", () => {
		...
		it("allows self-removal without skipping neighboring handlers", async () => {
			const calls: string[] = [];
			const { runner } = await loadSubscriptionExtension((shuvpi) => {
				const unsubscribe = shuvpi.on("agent_end", () => {
					calls.push("A");
					unsubscribe();
				});
				shuvpi.on("agent_end", () => {
		...
		it("removes duplicate registrations independently and cleans up the last handler", async () => {
			...
			const { extension, runner } = await loadSubscriptionExtension((shuvpi) => {
				const shared = () => {
					calls.push("shared");
				};
				unsubscribers.push(shuvpi.on("agent_end", shared));
				unsubscribers.push(
					shuvpi.on("agent_end", () => {
						calls.push("B");
					}),
		...
		it("keeps removed pending handlers in the current dispatch", async () => {
			...
			const { runner } = await loadSubscriptionExtension((shuvpi) => {
				...
				const stopB = shuvpi.on("agent_end", () => {
					calls.push("B");
				});
				shuvpi.on("agent_end", () => {
					calls.push("C");
		...
		it("uses a fresh handler list for nested dispatches", async () => {
			const calls: string[] = [];
			const { runner } = await loadSubscriptionExtension((shuvpi) => {
				const stopA = shuvpi.on("agent_end", async () => {
					calls.push("A");
					stopA();
					stopB();
					shuvpi.on("agent_end", () => {
