...
import { NodeExecutionEnv } from "@shuv1337/shuvpi-agent-core/node";
...
const defaultPromptTools = [
	...
	createPromptTool("bash", "Execute bash commands (ls, grep, find, etc.)", [
		"You can inspect SHUVPI_* environment variables for current model and session details.",
	]),
...
describe("coding-agent Harness construction", () => {
	...
	test("preserves coding-agent prompt snippets and guideline order", () => {
		...
		expect(prompt).toContain("You can inspect SHUVPI_* environment variables for current model and session details.");
		expect(prompt.indexOf("Use read to examine files")).toBeLessThan(
			prompt.indexOf("You can inspect SHUVPI_* environment variables"),
		);
	});
	...
	test("sets the optional session file in the default bash tool environment", async () => {
		...
		const env = new CapturingExecutionEnv({
			cwd: process.cwd(),
			shellEnv: { SHUVPI_SESSION_FILE: "/stale/parent.jsonl", SHUVPI_CODING_AGENT: "true" },
		});
		const created = await createCodingAgentHarness({
			session,
			models: createModels(),
		...
		try {
			...
			const result = await bash.execute("bash-call", {
				command: `printf '%s' "$SHUVPI_SESSION_ID|$SHUVPI_SESSION_FILE|$SHUVPI_PROVIDER|$SHUVPI_MODEL|$SHUVPI_REASONING_LEVEL|$SHUVPI_CODING_AGENT"`,
			});

			expect(env.executionOverrides).toEqual({
				SHUVPI_SESSION_ID: "session-file-harness",
				SHUVPI_SESSION_FILE: "/sessions/current.jsonl",
				SHUVPI_PROVIDER: "google",
				SHUVPI_MODEL: "gemini-2.5-flash",
				SHUVPI_REASONING_LEVEL: "high",
			});
	...
	test("keeps bash PI model variables synchronized with Harness state", async () => {
		...
		try {
			...
			const result = await bash.execute("bash-call", {
				command: `printf '%s:%s' "\${SHUVPI_SESSION_FILE+x}" "$SHUVPI_SESSION_ID|$SHUVPI_PROVIDER|$SHUVPI_MODEL|$SHUVPI_REASONING_LEVEL|$SHUVPI_CODING_AGENT"`,
			});

			expect(env.executionOverrides).toEqual({
				SHUVPI_SESSION_ID: "dynamic-bash-session",
				SHUVPI_SESSION_FILE: "",
				SHUVPI_PROVIDER: "anthropic",
				SHUVPI_MODEL: "claude-sonnet-4-5",
				SHUVPI_REASONING_LEVEL: "low",
			});
			expect(Object.hasOwn(env.executionOverrides ?? {}, "SHUVPI_SESSION_FILE")).toBe(true);
			expect(env.executionOverrides?.SHUVPI_SESSION_FILE).toBe("");
			expect(result.content).toEqual([
				{
					type: "text",
	...
	test("builds the default prompt from active tools and resolved prompt resources", () => {
		...
		expect(prompt).not.toContain("You can inspect SHUVPI_* environment variables");
