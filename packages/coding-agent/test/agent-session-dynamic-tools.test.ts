...
describe("AgentSession dynamic tool registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `shuvpi-dynamic-tool-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(sessionEnv).toMatchObject({
			SHUVPI_SESSION_ID: session.sessionId,
			SHUVPI_SESSION_FILE: session.sessionFile,
			SHUVPI_PROVIDER: model.provider,
			SHUVPI_MODEL: model.id,
			SHUVPI_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("SHUVPI_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("SHUVPI_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("SHUVPI_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("SHUVPI_MODEL");
		expect(optedOutEnv).not.toHaveProperty("SHUVPI_REASONING_LEVEL");
