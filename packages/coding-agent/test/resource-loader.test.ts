...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "shuvpi-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".shuvpi", "prompts");
			...
			const projectSkillDir = join(cwd, ".shuvpi", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".shuvpi", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".shuvpi", "themes"), { recursive: true });
	...
	shuvpi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".shuvpi"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".shuvpi", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".shuvpi", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".shuvpi", "extensions");
...
export default function(shuvpi) {
	...
	shuvpi.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(shuvpi) {
	shuvpi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".shuvpi", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	shuvpi.registerCommand("project-only", {
	...
	shuvpi.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.shuvpi", async () => {
			const shuvpiDir = join(cwd, ".shuvpi");
			mkdirSync(shuvpiDir, { recursive: true });
			writeFileSync(join(shuvpiDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			const shuvpiDir = join(cwd, ".shuvpi");
			const extensionsDir = join(shuvpiDir, "extensions");
			const skillDir = join(shuvpiDir, "skills", "project-skill");
			const promptsDir = join(shuvpiDir, "prompts");
			const themesDir = join(shuvpiDir, "themes");
			mkdirSync(extensionsDir, { recursive: true });
			mkdirSync(skillDir, { recursive: true });
			mkdirSync(promptsDir, { recursive: true });
			mkdirSync(themesDir, { recursive: true });
			writeFileSync(join(shuvpiDir, "SYSTEM.md"), "Project system prompt.");
...
Project skill content`,
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const shuvpiDir = join(cwd, ".shuvpi");
			mkdirSync(shuvpiDir, { recursive: true });
			writeFileSync(join(shuvpiDir, "APPEND_SYSTEM.md"), "Additional instructions.");
	...
	describe("system prompt sources", () => {
		it("exposes discovered project SYSTEM.md as the system prompt source", async () => {
			const shuvpiDir = join(cwd, ".shuvpi");
			const systemPromptPath = join(shuvpiDir, "SYSTEM.md");
			mkdirSync(shuvpiDir, { recursive: true });
		...
		it("exposes discovered APPEND_SYSTEM.md as an append system prompt source", async () => {
			const shuvpiDir = join(cwd, ".shuvpi");
			const appendSystemPromptPath = join(shuvpiDir, "APPEND_SYSTEM.md");
			mkdirSync(shuvpiDir, { recursive: true });
