...
import type { AssistantMessage, ToolResultMessage } from "@shuv1337/shuvpi-ai/compat";
...
describe("JSONL share export", () => {
	...
	it("adds presentation data without changing conversation IDs or links", async () => {
		...
		try {
			...
			expect(normalRecords.some((record) => record.type === "custom" && record.customType === "shuvpi.share")).toBe(
				false,
			);
