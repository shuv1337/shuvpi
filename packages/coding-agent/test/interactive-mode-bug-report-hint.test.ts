import { type AssistantMessage, fauxAssistantMessage } from "@shuv1337/shuvpi-ai";
...
describe("InteractiveMode bug report hints", () => {
	test("identifies extensions with frames in a crash stack", () => {
		expect(formatCrashExtensionHint(["npm:shuvpi-observational-memory"])).toBe(
			"A stack frame came from loaded extension `npm:shuvpi-observational-memory`, which may be involved. Try disabling it with `shuvpi config`, or run `shuvpi -ne` to confirm.",
		);
