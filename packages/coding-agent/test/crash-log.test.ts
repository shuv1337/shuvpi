...
describe("extension crash attribution", () => {
	test("matches stack frames beneath loaded package roots", () => {
		const memory = packageExtension(
			"npm:shuvpi-observational-memory",
			"/home/fedora/.shuvpi/agent/npm/node_modules/shuvpi-observational-memory",
		);
		const unrelated = packageExtension("npm:unrelated", "/home/fedora/.shuvpi/agent/npm/node_modules/unrelated");
		const stack =
			"TypeError: Cannot read properties of undefined (reading 'runtime')\n" +
			"    at streamSimple (file:///home/fedora/.local/lib/node_modules/@shuv1337/shuvpi-coding-agent/dist/bundle/chunks/chunk-CMRUVXTE.js:1093:16944)\n" +
			"    at /home/fedora/.shuvpi/agent/npm/node_modules/shuvpi-observational-memory/src/agents/worker-stream.ts:43:45";

		expect(findExtensionStackMatches(stack, [memory, unrelated])).toEqual(["npm:shuvpi-observational-memory"]);
	});

	test("normalizes Windows paths and deduplicates package extensions", () => {
		const root = "C:\\Users\\reporter\\.shuvpi\\agent\\npm\\node_modules\\@scope\\memory";
	...
	test("decodes frame paths independently from malformed error text", () => {
		const extension: StackExtension = {
			path: "/Users/reporter/.shuvpi/agent/extensions/local memory/index.ts",
			resolvedPath: "/Users/reporter/.shuvpi/agent/extensions/local memory/index.ts",
			sourceInfo: {
				path: "/Users/reporter/.shuvpi/agent/extensions/local memory/index.ts",
				source: "local",
				scope: "user",
				origin: "top-level",
				baseDir: "/Users/reporter/.shuvpi/agent/extensions",
			},
		};
		const stack =
			"Error: progress 100%\n    at run (file:///Users/reporter/.shuvpi/agent/extensions/local%20memory/worker.ts:4:2)";
