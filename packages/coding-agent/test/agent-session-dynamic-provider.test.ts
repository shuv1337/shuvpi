...
describe("AgentSession dynamic provider registration", () => {
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(shuvpi) => {
				shuvpi.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(shuvpi) => {
				shuvpi.on("session_start", () => {
					shuvpi.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(shuvpi) => {
				shuvpi.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						shuvpi.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
