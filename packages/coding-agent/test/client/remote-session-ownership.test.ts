import { ShuvpiSessionOwnershipError } from "@shuv1337/shuvpi-client";
...
describe("RemoteSession ownership", () => {
	...
	test("rejects an exclusive coordinator while a direct shared lease is active", async () => {
		...
		await expect(RemoteSession.open(client, "session-1")).rejects.toBeInstanceOf(ShuvpiSessionOwnershipError);
