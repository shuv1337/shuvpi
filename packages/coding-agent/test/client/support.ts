import { type ByteTransport, type ByteTransportHandlers, ShuvpiClient } from "@shuv1337/shuvpi-client";
import {
	type ClientMessage,
...
export async function connectClient(server: MemoryServer): Promise<ShuvpiClient> {
	const client = new ShuvpiClient({ transportFactory: (handlers) => server.connect(handlers) });
