import { type AssistantMessage, fauxAssistantMessage, fauxProvider } from "@shuv1337/shuvpi-ai";
import { getApiProvider } from "@shuv1337/shuvpi-ai/compat";
import { expect, it } from "vitest";
import { createHarness } from "../harness.ts";

// Regression for #8964: extensions can stream responses from providers registered with shuvpi.registerProvider().
it.each(["stream", "streamSimple"] as const)(
	...
	async (method) => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(shuvpi) => {
					shuvpi.registerProvider(faux.provider.id, {
						api: faux.api,
				...
				(shuvpi) => {
					shuvpi.registerCommand("stream-custom", {
