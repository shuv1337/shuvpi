...
async function startRpcHarness(extension: (shuvpi: ExtensionAPI) => void): Promise<{
	harness: Harness;
...
const rpcCases: Array<{
	name: string;
	extension: (shuvpi: ExtensionAPI) => void;
	error?: string;
	executeCount: number;
}> = [
	{
	...
	},
	{
		...
		extension: (shuvpi) => {
			shuvpi.on("user_bash", async () => ({}) as unknown as UserBashEventResult);
		},
	...
	},
	{
		...
		extension: (shuvpi) => {
			shuvpi.on("user_bash", async () => undefined);
		},
		executeCount: 1,
	},
];
