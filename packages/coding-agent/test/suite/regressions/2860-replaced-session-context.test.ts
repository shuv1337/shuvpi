...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `shuvpi-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale shuvpi/ctx", async () => {
		const events: string[] = [];
		let oldCtx: ExtensionCommandContext | undefined;
		let oldShuvpi: ExtensionAPI | undefined;
		let oldSessionFile: string | undefined;
		let staleCtxThrows = false;
		let staleShuvpiThrows = false;
					...
					handler: async (_args, ctx) => {
						...
						await ctx.newSession({
							...
							withSession: async (replacedCtx) => {
								...
								try {
									oldShuvpi?.sendUserMessage("stale message");
								} catch {
									staleShuvpiThrows = true;
								}
		...
		expect(staleShuvpiThrows).toBe(true);
	...
	it("supports withSession for fork", async () => {
		const { runtime } = await createRuntimeForTest(
			(shuvpi) => {
				shuvpi.registerCommand("fork-it", {
					description: "fork-it",
					handler: async (_args, ctx) => {
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(shuvpi) => {
				shuvpi.registerCommand("switch-it", {
