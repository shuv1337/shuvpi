...
describe("AgentSession actionable boundaries", () => {
					...
					(shuvpi) => {
						shuvpi.on("turn_end", () => {
							...
							if (queueKind === "steering" || queueKind === "both") {
								shuvpi.sendUserMessage("queued steering", { deliverAs: "steer" });
							}
							if (queueKind === "follow-up" || queueKind === "both") {
								shuvpi.sendUserMessage("queued follow-up", { deliverAs: "followUp" });
							}
							return {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(shuvpi) => {
					...
					shuvpi.on("session_before_compact", (event) => ({
						compaction: {
		...
		const harness = await createHarness({
			extensionFactories: [
				(shuvpi) => {
					...
					shuvpi.on("agent_before_settle", (event) => {
						if (continued) return;
						continued = true;
		...
		const harness = await createHarness({
			extensionFactories: [
				(shuvpi) => {
					...
					shuvpi.on("agent_settled", (_event, ctx) => {
	...
	it("does not consume queued input when pre-settlement drafts leave system-only context", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(shuvpi) => {
					shuvpi.on("agent_before_settle", (_event, ctx) => {
						if (handled) return;
						handled = true;
						shuvpi.sendUserMessage("still queued", { deliverAs: "followUp" });
	...
	it("commits pre-settlement drafts but suppresses continuation when aborted during the hook", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(shuvpi) => {
					shuvpi.on("agent_before_settle", async () => {
						started.resolve();
...
describe("durable length recovery", () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(shuvpi) => {
					...
					shuvpi.on("agent_end", (event) => {
						if (queued || !event.messages.some((message) => getMessageText(message) === "first recovered"))
							return;
						queued = true;
						shuvpi.sendUserMessage("distinct follow-up", { deliverAs: "followUp" });
					});
				},
			],
		});
	...
	it("omits a recoverable projected replacement by its source entry ID", async () => {
		const harness = await createHarness({
			...
			extensionFactories: [
				(shuvpi) => {
					shuvpi.on("session_before_compact", () => ({ cancel: true }));
				},
			],
		});
		harnesses.push(harness);
