...
vi.mock("@shuv1337/shuvpi-coding-agent", () => ({
...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (shuvpi: ExtensionAPI) => {
			shuvpi.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (shuvpi: ExtensionAPI) => {
			shuvpi.on("session_compact", async (event: SessionCompactEvent) => {
