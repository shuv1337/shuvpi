...
describe("experimental CLI command composition", () => {
	test("composes shuvpi command options with the existing parser", () => {
		const result = experimentalCli.parse([
			"--listen",
			"unix:///tmp/shuvpi.sock",
			"--auth-token",
			"secret",
			"--provider",
			"anthropic",
	...
	test.each(["--help", "--version"] as const)("keeps Shuvpi %s handling in existing CLI options", (option) => {
		expect(experimentalCli.parse([option])).toMatchObject({
			ok: true,
			command: { command: "shuvpi", options: { [option === "--help" ? "help" : "version"]: true } },
		});
	});
	...
	test.each(["shuvpi", "server", "client"] as const)("executes the parsed %s command", async (name) => {
		const context = {
			runShuvpi: vi.fn(() => undefined),
			runServer: vi.fn(() => undefined),
			runClient: vi.fn(() => undefined),
		};
		const result = await experimentalCli.execute(name === "shuvpi" ? [] : [name], context);

		expect(result).toMatchObject({ ok: true, command: { command: name } });
		expect(context.runShuvpi).toHaveBeenCalledTimes(name === "shuvpi" ? 1 : 0);
