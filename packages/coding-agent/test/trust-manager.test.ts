...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".shuvpi", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".shuvpi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".shuvpi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".shuvpi"), { recursive: true });
			writeFileSync(join(cwd, ".shuvpi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".shuvpi"), { recursive: true, force: true });
