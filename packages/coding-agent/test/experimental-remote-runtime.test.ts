...
import { type Context, createFacetHost, defineFacet, defineService } from "@shuv1337/shuvpi-chord";
import { BACKGROUND_CONTEXT } from "@shuv1337/shuvpi-chord/context";
import { Client, ServerError as ClientServerError } from "@shuv1337/shuvpi-client";
...
beforeEach(async () => {
	...
	vi.stubEnv("SHUVPI_CODING_AGENT_DIR", agentDir);
...
describe("experimental durable server composition", () => {
	test("uses SHUVPI_SERVER_DIR and SHUVPI_SERVER_ID", async () => {
		const directory = await mkdtemp(join("/tmp", "pi-server-dir-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("SHUVPI_SERVER_DIR", directory);
		vi.stubEnv("SHUVPI_SERVER_ID", serverId);
	...
	test("loads conventional Session facets from multiple configured plugin packages", async () => {
		...
		await Promise.all([
			...
			writeFile(
				...
				'import { defineFacet, defineService } from "@shuv1337/shuvpi-chord"; const Service = defineService("test.second-plugin"); export default defineFacet({ id: "second-session-plugin", setup(env) { env.provide(Service, { async read() { return "second"; } }); } });\n',
			),
		]);
		const runtime = await startServer({ ...sessionWorkerModel, directory });
		servers.add(runtime);
