...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.SHUVPI_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.SHUVPI_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .shuvpi skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .shuvpi", async () => {
			const extDir = join(tempDir, ".shuvpi", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".shuvpi"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".shuvpi", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".shuvpi", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".shuvpi", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".shuvpi", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".shuvpi", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json shuvpi.extensions in extensions setting", async () => {
			// Create a package with shuvpi.extensions in package.json
			...
			// Should find the extensions declared in package.json shuvpi.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .shuvpi/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-shuvpi", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-shuvpi"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-shuvpi\ndescription: user shuvpi\n---\n");
		...
		it("should use the project .shuvpi dir as baseDir for project .shuvpi skills", async () => {
			const projectBaseDir = join(tempDir, ".shuvpi");
			const skillPath = join(projectBaseDir, "skills", "project-shuvpi", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-shuvpi"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-shuvpi\ndescription: project shuvpi\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".shuvpi", "agent");
		...
		it("should dedupe user skill entries when ~/.shuvpi/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .shuvpi auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".shuvpi\n");

			const skillDir = join(tempDir, ".shuvpi", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with shuvpi manifest", async () => {
		...
		it("should keep shuvpi manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.shuvpi\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".shuvpi", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the shuvpi-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "shuvpi-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .shuvpi settings base", () => {
			...
			const rel = relative(join(tempDir, ".shuvpi"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in shuvpi manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "shuvpi-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "shuvpi-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:shuvpi-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:shuvpi-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".shuvpi"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);

			const result = await packageManager.resolve();

			expect(result.extensions.map((resource) => resource.path)).toEqual([join(pkgDir, "extensions", "foo.ts")]);
			expect(result.skills.some((resource) => resource.path === join(pkgDir, "skills", "foo", "SKILL.md"))).toBe(
				false,
			);
		});
	});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json shuvpi.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".shuvpi", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".shuvpi", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".shuvpi", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".shuvpi", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".shuvpi", "npm", "node_modules", "example");
