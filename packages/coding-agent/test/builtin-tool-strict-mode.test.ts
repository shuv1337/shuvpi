...
describe("strict built-in tools", () => {
	...
	it.each([undefined, "0", "1"])("prefers strict sampling with SHUVPI_EXPERIMENTAL=%s", (experimental) => {
		vi.stubEnv("SHUVPI_EXPERIMENTAL", experimental);
	...
	it.each([{ activeTools: [] }, { activeTools: ["read"] }, { activeTools: [...strictToolNames] }])(
		...
		async ({ activeTools }) => {
			const cwd = mkdtempSync(join(tmpdir(), "shuvpi-non-strict-tools-"));
				...
				extensionFactories: [
					(shuvpi) => {
						shuvpi.on("session_start", () => {
							...
							for (const name of strictToolNames) {
								shuvpi.registerTool({ ...definitions[name], constrainedSampling: false });
							}
							shuvpi.setActiveTools(activeTools);
						});
					},
				],
			});
