...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".shuvpi"), { recursive: true });
	});
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates shuvpi starting up)
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".shuvpi", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".shuvpi", "settings.json"), JSON.stringify({ theme: "project" }));
		...
		it("should read default project trust from global settings only", () => {
			...
			writeFileSync(join(projectDir, ".shuvpi", "settings.json"), JSON.stringify({ defaultProjectTrust: "never" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .shuvpi folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .shuvpi folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .shuvpi folder that beforeEach created
			rmSync(join(projectDir, ".shuvpi"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .shuvpi folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".shuvpi"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .shuvpi folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .shuvpi folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .shuvpi folder should NOT exist yet
			expect(existsSync(join(projectDir, ".shuvpi"))).toBe(false);
			...
			// Now .shuvpi folder should exist
			expect(existsSync(join(projectDir, ".shuvpi"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".shuvpi", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".shuvpi", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("cacheWarming", () => {
		it("defaults to streaming and ignores project settings", () => {
			...
			writeFileSync(join(projectDir, ".shuvpi", "settings.json"), JSON.stringify({ cacheWarming: "idle" }));
			expect(SettingsManager.create(projectDir, agentDir).getCacheWarmingMode()).toBe("streaming");
	...
	describe("defaultTools", () => {
		it("loads global defaults and lets project settings replace them", () => {
			...
			writeFileSync(join(projectDir, ".shuvpi", "settings.json"), JSON.stringify({ defaultTools: ["grep"] }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".shuvpi", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
