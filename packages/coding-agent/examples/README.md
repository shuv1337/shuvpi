...
Example code for the shuvpi-coding-agent SDK, process integration, and extensions.

## CLI integration

[`rpc-client.ts`](rpc-client.ts) uses the typed `RpcClient` to run Shuvpi in a child process, stream events, and wait for the run to settle.
...
### [plugins/shuvpi-example-plugin/](plugins/shuvpi-example-plugin/)
An experimental plugin package that Shuvpi automatically builds into separate Session-worker and TUI Chord facets.
