 ...
 *
 * Usage: shuvpi --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@shuv1337/shuvpi-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@shuv1337/shuvpi-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@shuv1337/shuvpi-tui";
...
export default function (shuvpi: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like shuvpi-doom would need)
	shuvpi.registerCommand("overlay-animation", {
	...
	shuvpi.registerCommand("overlay-anchors", {
	...
	shuvpi.registerCommand("overlay-margins", {
	...
	shuvpi.registerCommand("overlay-stack", {
	...
	shuvpi.registerCommand("overlay-overflow", {
	...
	shuvpi.registerCommand("overlay-edge", {
	...
	shuvpi.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	shuvpi.registerCommand("overlay-maxheight", {
	...
	shuvpi.registerCommand("overlay-sidepanel", {
	...
	shuvpi.registerCommand("overlay-toggle", {
	...
	shuvpi.registerCommand("overlay-passive", {
	...
	shuvpi.registerCommand("overlay-focus", {
	...
	shuvpi.registerCommand("overlay-streaming", {
...
// Animation demo - proves overlays can handle real-time updates like shuvpi-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (shuvpi-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
