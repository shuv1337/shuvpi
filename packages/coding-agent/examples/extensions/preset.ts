 ...
 * - ~/.shuvpi/agent/presets.json (global)
 * - <cwd>/.shuvpi/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `shuvpi --preset plan` - start with plan preset
...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@shuv1337/shuvpi-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@shuv1337/shuvpi-tui";
...
export default function presetExtension(shuvpi: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: shuvpi.getThinkingLevel(),
				tools: shuvpi.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			shuvpi.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = shuvpi.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				shuvpi.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await shuvpi.setModel(originalState.model);
				}
				shuvpi.setThinkingLevel(originalState.thinkingLevel);
				shuvpi.setActiveTools(originalState.tools);
			} else {
				shuvpi.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	shuvpi.registerShortcut(Key.ctrlShift("u"), {
	...
	shuvpi.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = shuvpi.getFlag("preset");
	...
	shuvpi.on("turn_start", async () => {
		if (activePresetName) {
			shuvpi.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
