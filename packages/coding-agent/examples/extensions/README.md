...
Example extensions for shuvpi-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
shuvpi --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.shuvpi/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `shuvpi.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs shuvpi theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `shuvpi.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using shuvpi-ai's built-in Anthropic/OpenAI streaming via proxy |
