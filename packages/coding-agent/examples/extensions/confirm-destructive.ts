...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@shuv1337/shuvpi-coding-agent";

export default function (shuvpi: ExtensionAPI) {
	shuvpi.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
