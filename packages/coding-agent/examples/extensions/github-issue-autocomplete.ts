...
async function resolveGitHubRepo(shuvpi: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await shuvpi.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (shuvpi: ExtensionAPI): void {
	shuvpi.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(shuvpi, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await shuvpi.exec(
					"gh",
					[
						"issue",
						"list",
