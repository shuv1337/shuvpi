...
export default function (shuvpi: ExtensionAPI) {
	shuvpi.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await shuvpi.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[shuvpi] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await shuvpi.exec("git", ["add", "-A"]);
		const { code: commitCode } = await shuvpi.exec("git", ["commit", "-m", commitMessage]);
