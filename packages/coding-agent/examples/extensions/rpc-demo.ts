...
export default function (shuvpi: ExtensionAPI) {
	...
	shuvpi.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "shuvpi RPC Demo (new session)" : "shuvpi RPC Demo");
	...
	shuvpi.registerCommand("rpc-input", {
	...
	shuvpi.registerCommand("rpc-editor", {
	...
	shuvpi.registerCommand("rpc-prefill", {
