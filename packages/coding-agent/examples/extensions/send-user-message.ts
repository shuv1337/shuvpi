 ...
 *
 * Demonstrates shuvpi.sendUserMessage() for sending user messages from extensions.
 * Unlike shuvpi.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (shuvpi: ExtensionAPI) {
	...
	shuvpi.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			shuvpi.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	shuvpi.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				shuvpi.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	shuvpi.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				shuvpi.sendUserMessage(args, { deliverAs: "followUp" });
	...
	shuvpi.registerCommand("askwith", {
