 ...
 *
 * Runs shuvpi's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   shuvpi -e /path/to/shuvpi/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (shuvpi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `shuvpi ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	shuvpi.registerCommand("gondolin", {
	...
	shuvpi.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
