 ...
 * (logo + keybinding hints) with a custom component showing the shuvpi mascot.
 */

import type { ExtensionAPI, Theme } from "@shuv1337/shuvpi-coding-agent";
import { VERSION } from "@shuv1337/shuvpi-coding-agent";

// --- SHUVPI MASCOT ---
// Based on shuvpi_mascot.ts - the shuvpi agent character
function getShuvpiMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const shuvpiBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${shuvpiBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${shuvpiBlue(BLOCK.repeat(2))}    ${shuvpiBlue(BLOCK.repeat(2))}`;
...
export default function (shuvpi: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getShuvpiMascot(theme);
	...
	shuvpi.registerCommand("builtin-header", {
