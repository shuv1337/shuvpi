 ...
 *
 * Usage:
 *   shuvpi -e ./ssh.ts --ssh user@host
 *   shuvpi -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (shuvpi: ExtensionAPI) {
	shuvpi.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	shuvpi.on("session_start", async (_event, ctx) => {
		...
		const arg = shuvpi.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
