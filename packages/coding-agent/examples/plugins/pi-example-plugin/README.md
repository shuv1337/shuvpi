# `@shuv1337/shuvpi-example-plugin`
...
The package needs no build script. Shuvpi asks Chord to discover `src/session.ts` and `src/tui.ts`, builds both entries into its server-owned plugin cache, and sends the TUI artifact to clients.

From the repository root:

```bash
SHUVPI_EXPERIMENTAL=1 ./shuvpi-test.sh server \
  -e "$PWD/packages/coding-agent/examples/plugins/shuvpi-example-plugin"
```

Alternatively, a client can select the plugin for the Session it creates or resumes on one local server:

```bash
SHUVPI_EXPERIMENTAL=1 ./shuvpi-test.sh client \
  -e "$PWD/packages/coding-agent/examples/plugins/shuvpi-example-plugin"
```
