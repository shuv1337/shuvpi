...
Shuvpi runs in four modes: interactive, print or JSON, RPC for process integration, and an SDK for embedding in your own apps.
...
Shuvpi also supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage downloads and loaded models with `/llama`, then select a loaded model with `/model`. See [docs/llama-cpp.md](docs/llama-cpp.md) for setup and usage.

See [docs/providers.md](docs/providers.md) for other provider setup instructions.

**Custom providers & models:** Add providers via `~/.shuvpi/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.shuvpi/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
| Location | Scope |
|----------|-------|
| `~/.shuvpi/agent/settings.json` | Global (all projects) |
| `.shuvpi/settings.json` | Project (overrides global) |
...
If a directory contains `AGENTS.override.md`, Shuvpi loads it instead of `AGENTS.md` or `CLAUDE.md` from that directory. Context files from other directories are still concatenated.
...
Replace the default system prompt with `.shuvpi/SYSTEM.md` (project) or `~/.shuvpi/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.shuvpi/agent/prompts/review.md -->
...
```markdown
<!-- ~/.shuvpi/agent/skills/my-skill/SKILL.md -->
# My Skill
...
shuvpi update --models                      # refresh model catalogs only
...
Read the [blog post](https://mariozechner.at/posts/2025-11-30-shuvpi-coding-agent/) for the full rationale.
...
shuvpi -p -- "- Summarize these points"
...
|----------|-------------|
| `AI_AGENT` | Set to `shuvpi` by the CLI and RPC entry points so generic tooling can attribute child processes to Shuvpi |
| `SHUVPI_CODING_AGENT` | Set to `true` by the CLI and RPC entry points so child processes can detect that they run inside Shuvpi |
| `SHUVPI_CODING_AGENT_DIR` | Override config directory (default: `~/.shuvpi/agent`) |
...
| `SHUVPI_SERVER_DIR` | Override experimental server profile and socket directory (default: `~/.shuvpi/server`) |
...
| `SHUVPI_SESSION_FILE` | Absolute session JSONL path; unset for ephemeral sessions |
| `SHUVPI_PROVIDER` | Currently selected model provider |
| `SHUVPI_MODEL` | Currently selected model ID |
| `SHUVPI_REASONING_LEVEL` | Current effective reasoning level |
