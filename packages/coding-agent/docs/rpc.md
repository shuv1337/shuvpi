...
**Note for Node.js/TypeScript users**: If you're building a Node.js application, consider using `AgentSession` directly from `@shuv1337/shuvpi-coding-agent` instead of spawning a subprocess. See [`src/core/agent-session.ts`](../src/core/agent-session.ts) for the API. For a subprocess-based TypeScript client, see [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts).

## Starting RPC Mode

```bash
shuvpi --mode rpc [options]
```
...
**Extension commands**: If the message is an extension command (e.g., `/mycommand`), it executes immediately even during streaming. Extension commands manage their own LLM interaction via `shuvpi.sendMessage()`.
...
`"xhigh"`, `"max"`, and `"ultra"` are exposed only when supported by the selected model. GPT-5.6 models can expose `"xhigh"` and `"max"`; the ChatGPT Codex Daybreak Blue alias also exposes `"ultra"`.
...
```json
{
  ...
  "data": {
    ...
    "cancelled": false,
    "truncated": true,
    "fullOutputPath": "/tmp/shuvpi-bash-abc123.log"
  }
}
```
...
The current session name is available via `get_state` in the `sessionName` field. To set the initial name when starting RPC mode, pass `--name <name>` or `-n <name>` to the `shuvpi --mode rpc` process.
...
```json
{
  ...
  "data": {
    "commands": [
      {"name": "session-name", "description": "Set or clear session name", "source": "extension", "path": "/home/user/.shuvpi/agent/extensions/session.ts"},
      {"name": "fix-tests", "description": "Fix failing tests", "source": "prompt", "location": "project", "path": "/home/user/myproject/.shuvpi/agent/prompts/fix-tests.md"},
      {"name": "skill:brave-search", "description": "Web search via Brave API", "source": "skill", "location": "user", "path": "/home/user/.shuvpi/agent/skills/brave-search/SKILL.md"}
    ]
  }
}
```
...
- `source`: What kind of command:
  - `"extension"`: Registered via `shuvpi.registerCommand()` in an extension
  - `"prompt"`: Loaded from a prompt template `.md` file
  - `"skill"`: Loaded from a skill directory (name is prefixed with `skill:`)
- `location`: Where it was loaded from (optional, not present for extensions):
  - `"user"`: User-level (`~/.shuvpi/agent/`)
  - `"project"`: Project-level (`./.shuvpi/agent/`)
...
Emitted after the full session-level run settles. At this point Shuvpi will not continue automatically through retry, compaction retry, or queued follow-up messages.
...
const agent = spawn("shuvpi", ["--mode", "rpc", "--no-session"]);
