...
RPC mode runs Shuvpi as a long-lived subprocess controlled through JSON records on stdin and stdout. Use it for language-independent integrations, process isolation, IDEs, and custom user interfaces.

For an in-process Node.js or Bun integration, prefer the [SDK](sdk.md). For a subprocess-based TypeScript integration, prefer the exported `RpcClient`, which starts Shuvpi, correlates responses, exposes typed command methods, and delivers events to listeners.
...
Normal CLI options still select the working folder, model, tools, resources, and session behavior. Common choices include `--provider`, `--model`, `--name`, `--no-session`, and `--session-dir`. See [Command Line](cli.md) for the complete, version-specific interface; `shuvpi --help` is authoritative for the installed version.
...
|---|---|---|
| stdin | Command | Ask Shuvpi to prompt, inspect state, change configuration, or manage the session |
...
| Both | Extension UI record | Forward supported extension interactions between Shuvpi and the client |
...
Read stdout continuously. Shuvpi honors stdout backpressure, but a client that stops reading can stall the process. Honor stdin backpressure when writing commands. Stdout is reserved for protocol records; diagnostics and application logging go to stderr.
...
Continue consuming [events](json.md) after that response. `agent_end` marks the end of one low-level agent run, but retries, overflow recovery, compaction, steering, or follow-up work can still follow. Wait for `agent_settled` when the client needs to know Shuvpi will not continue automatically.
...
Close the child's stdin to request an orderly shutdown. Shuvpi disposes the active runtime before exiting. Clients should still handle process signals and unexpected exits.

An extension can also request shutdown through its extension context. Shuvpi completes shutdown after the current command or after the active run emits `agent_settled`.
...
For maintained TypeScript clients, use the checked [RPC client example](../examples/rpc-client.ts). It requires a built Shuvpi CLI because the repository example points to `dist/cli.js`.
...
- [`rpc-types.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/modes/rpc/rpc-types.ts): exported TypeScript protocol definitions
- [`RpcClient`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/modes/rpc/rpc-client.ts): subprocess client implementation
