...
| Setting | Type | Default | Description |
|---|---|---|---|
| `sessionDir` | string | Agent session directory | Session storage directory. Relative paths resolve from the working directory. `SHUVPI_CODING_AGENT_SESSION_DIR` and `--session-dir` override this setting. |
...
| `showHardwareCursor` | boolean | `false` | Show the terminal cursor while Shuvpi positions it for input methods. |
...
| `httpProxy` | string | None | Proxy URL applied as `HTTP_PROXY` and `HTTPS_PROXY` for Shuvpi-managed HTTP clients. **Can only be set in agent-directory settings.** |
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are required. Provider retries can delay Shuvpi from handling quota and usage-limit errors itself.
...
See [Shell aliases](shell-aliases.md) for shell setup and [Shuvpi Packages](packages.md) for package-manager behavior.

## Resources

Resource paths in user settings resolve from the agent directory. Paths in project settings resolve from the project `.shuvpi` directory. Absolute paths and `~` are supported.

| Setting | Type | Default | Description |
|---|---|---|---|
| `packages` | array | `[]` | npm, git, or local Shuvpi package sources. See [Shuvpi Packages](packages.md). |
...
Resource arrays support glob exclusions with `!pattern`, exact inclusion with `+path`, and exact exclusion with `-path`. Shuvpi loads resources listed in both user-level and project settings.
