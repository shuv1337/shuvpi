...
Shuvpi uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.shuvpi/agent/settings.json` | Global (all projects) |
| `.shuvpi/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`SHUVPI_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so shuvpi resumes after the editor exits:
...
`enableInstallTelemetry` controls the anonymous install/update ping to `https://pi.dev/api/report-install` and Shuvpi attribution headers for OpenRouter, NVIDIA NIM, and Cloudflare provider requests. Opting out disables both. It does not disable update checks; Shuvpi can still fetch `https://pi.dev/api/latest-version` to look for the latest version.

Set `SHUVPI_SKIP_VERSION_CHECK=1` to disable the Shuvpi version update check. Use `--offline` or `SHUVPI_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Shuvpi sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.shuvpi/agent/npm/`; project-scoped npm packages install under `.shuvpi/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, Shuvpi uses its standard defaults |
...
```json
{ "sessionDir": ".shuvpi/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `SHUVPI_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.shuvpi/agent/settings.json` resolve relative to `~/.shuvpi/agent`. Paths in `.shuvpi/settings.json` resolve relative to `.shuvpi`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["shuvpi-skills", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["shuvpi-skills"]
}
```

## Project Overrides

Project settings (`.shuvpi/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.shuvpi/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .shuvpi/settings.json (project)
{
