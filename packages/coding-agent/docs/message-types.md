...
Shuvpi uses `AgentMessage` values in SDK state, lifecycle events, RPC responses, and persisted session message entries. This page defines those shared messages and their content blocks.
...
- [`packages/ai/src/types.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/ai/src/types.ts) defines provider-facing messages and content blocks.
- [`packages/agent/src/types.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/agent/src/types.ts) defines the extensible `AgentMessage` union.
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/core/messages.ts) adds coding-agent message roles.
...
`"pending"` is used for a partial assistant message while it streams. The completed message in `message_end` has a terminal stop reason, and Shuvpi does not persist `"pending"` assistant messages in session JSONL.
...
Unless `excludeFromContext` is true, Shuvpi converts this message to user-role text before the next model request.
...
Shuvpi converts its content to a user message for model requests. `display` controls terminal rendering; `details` is not sent to the model.
...
Shuvpi creates this context message from a persisted `branch_summary` entry.
...
Shuvpi creates this context message from a persisted `compaction` entry.
