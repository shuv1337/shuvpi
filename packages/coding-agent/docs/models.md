...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.shuvpi/agent/models.json`.
...
The `apiKey` value is a placeholder because Ollama ignores it. shuvpi still treats models as requiring auth before they appear in `/model`, so keyless local servers should keep a dummy value, save a key for that provider with `/login`, or pass `--api-key` when selecting the model.

Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so shuvpi sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
For `models.json`, shell commands are resolved at request time. shuvpi intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and shuvpi cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps shuvpi thinking levels to provider values and marks unsupported levels (see below) |
...
`samplingParams` is a free-form object merged verbatim into every request body for the model, after the fields shuvpi sets itself, so its keys win. Use it to send sampling parameters shuvpi does not model — including server-specific ones like llama.cpp's `min_p` or vLLM's `top_k`:
...
Only OpenAI-compatible APIs apply it (`openai-completions`, `openai-responses`, `azure-openai-responses`); other APIs ignore it. Keys override shuvpi's named request fields (for example a `temperature` key here beats the request-level temperature), so prefer it as the single source of sampling truth for a model. In `modelOverrides`, `samplingParams` merges per key with the base model's value.
...
| `supportsMidConvoEffort` | Whether the exact Claude model transport supports per-turn effort system messages and thinking binding controls. Shuvpi persists native effort levels and always sends `drop_block` when enabled. Default: `false`. |
...
| `supportsFinishReason` | Whether streamed responses include `finish_reason`. When `false`, shuvpi infers `stop` or `toolUse` when the stream ends. Default: `true`. |
...
| `chatTemplateKwargs` | `chat_template_kwargs` values for `thinkingFormat: "chat-template"`; use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for shuvpi-controlled thinking values |
| `chatTemplateArgs` | `chat_template_args` values for `thinkingFormat: "baseten"`; use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for shuvpi-controlled thinking values |
