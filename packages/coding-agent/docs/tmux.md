...
Shuvpi works inside tmux, but tmux can report `Shift+Enter`, `Ctrl+Enter`, and plain `Enter` as the same key. Enable extended keys so Shuvpi can distinguish them.
...
Shuvpi requests extended-key reporting when the terminal does not provide the Kitty keyboard protocol directly. CSI-u is the most reliable format for forwarding modified keys through tmux.
...
Start Shuvpi inside the new tmux session and check that:
...
Shuvpi supports the xterm `modifyOtherKeys` format used by these versions. Restart tmux and repeat the verification steps.

For older versions, upgrade tmux or use Shuvpi outside tmux rather than relying on modified Enter shortcuts.
