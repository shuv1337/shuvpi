...
Use `/logout` to clear credentials. Tokens are stored in `~/.shuvpi/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Store credentials in `~/.shuvpi/agent/auth.json`:
...
API key credentials can also include provider-scoped environment values. These values are used before process environment variables when resolving the credential key, provider/model headers, and provider configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `SHUVPI_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
