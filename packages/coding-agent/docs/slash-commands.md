...
Type `/` in Shuvpi's terminal editor to search the commands available in the current session. This page lists the built-in commands in the current Shuvpi release.

Extensions, prompt templates, and skills can add commands. The command menu in Shuvpi is therefore the exact reference for the resources loaded in your session.
...
| `/bug [description]` | Prepare a private bug report for the Shuvpi developers |
...
| Command | Description |
|---|---|
| `/trust` | Save a project trust decision for future Shuvpi processes |
