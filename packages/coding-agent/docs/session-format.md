...
```
~/.shuvpi/agent/sessions/--<path>--/<timestamp>_<session-id>.jsonl
```

By default, `<session-id>` is a UUID. Callers can supply a custom ID through the SDK or `--session-id`. For `<path>`, Shuvpi removes the leading path separator and replaces `/`, `\\`, and `:` with `-`.
...
Source on GitHub ([shuvpi](https://github.com/shuv1337/shuvpi)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [Message Types](message-types.md) - Shared message and content-block reference
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types
- [`packages/ai/src/types.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/ai/src/types.ts) - Base message and content-block types
- [`packages/agent/src/types.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/agent/src/types.ts) - Extensible `AgentMessage` union

For TypeScript definitions in your project, inspect `node_modules/@shuv1337/shuvpi-coding-agent/dist/` and `node_modules/@shuv1337/shuvpi-ai/dist/`.
...
Usage entries contribute to session token and cost totals. Shuvpi hides them from the conversation tree. Consumers should treat unknown `kind` values as normal usage rather than rejecting them.
...
`firstKeptEntryId` is required. It identifies the first entry retained from before the compaction entry. When rebuilding context, Shuvpi replaces older summarized entries with the compaction summary and keeps the range beginning at this entry. A retain-none compaction stores its own ID in this field, so no preceding entries are retained.
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if shuvpi-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `shuvpi.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `shuvpi.setSessionName()` in extensions.
