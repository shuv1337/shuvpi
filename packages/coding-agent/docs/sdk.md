...
`@shuv1337/shuvpi-coding-agent` embeds Shuvpi in a Node.js or Bun process. It provides direct TypeScript access to the agent, sessions, tools, models, and resources used by the command-line application.
...
Sessions are persistent by default. `SessionManager` owns the persisted or in-memory entry tree and tracks its active leaf. Branching changes that leaf without deleting abandoned branches. When Shuvpi reconstructs model context, the manager selects the active branch and applies compaction.
...
Use `agent_settled` when the host needs to know that Shuvpi will not continue automatically.
