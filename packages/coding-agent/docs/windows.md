# Run Shuvpi on Windows

Run Shuvpi either as a native Windows process or inside Windows Subsystem for Linux (WSL). Native Windows uses Git Bash by default for Bash commands and can optionally expose PowerShell to the model. Shuvpi inside WSL uses the Linux environment and its Bash installation.

Follow the main [Quickstart](quickstart.md) to install and authenticate Shuvpi. Use this page to choose and configure its command environment.
...
Shuvpi resolves Bash in this order:

1. `shellPath` from `~/.shuvpi/agent/settings.json`
2. Git Bash under `Program Files` or `Program Files (x86)`
3. `bash.exe` on `PATH`, including Cygwin, MSYS2, or legacy WSL Bash

Start Shuvpi and enter this command to verify the shell:

```text
!printf 'Bash is working\n'
```

If Shuvpi cannot find Bash, it reports the locations it checked. Install Git for Windows, put another Bash executable on `PATH`, or configure `shellPath`.
...
To replace the model-facing `bash` tool with `powershell`, add this to `~/.shuvpi/agent/settings.json`:
...
Restart Shuvpi, then ask it to run a harmless PowerShell command. The `!` and `!!` editor commands continue to use Bash. The `powershell` tool is available only when Shuvpi runs as a native Windows process.

See [Settings](settings.md#tools) for other tool combinations.

## Use a custom Bash executable

Set `shellPath` when Bash is installed somewhere Shuvpi does not discover automatically:
...
Windows Terminal reserves or rewrites some modified keys. See [Windows Terminal](terminal-setup.md#windows-terminal) to configure `Shift+Enter` and `Alt+Enter`, and [Keybindings](keybindings.md) for Shuvpi's Windows and WSL shortcut defaults.
