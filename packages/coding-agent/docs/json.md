...
```bash
shuvpi --mode json "Review this repository"
```

Shuvpi writes one session header followed by session events, then exits after the supplied prompts finish. RPC mode emits the same session-event shapes but has no session header because it is a bidirectional, long-lived protocol. See [RPC Mode](rpc.md).
...
Read stdout continuously. A reader that stops consuming records can stall Shuvpi when the pipe buffer fills. Stdout is reserved for JSONL; diagnostics and application logging go to stderr.
...
`agent_end` closes one low-level agent run. Automatic retry, overflow recovery, compaction retry, steering, or follow-up work can still continue. `agent_settled` means Shuvpi has no remaining automatic work for that session-level run.
...
| `agent_settled` | None | Shuvpi will not continue automatically through retries, compaction recovery, or queued messages. |
...
| `entry_appended` | `entry` | An extension appended a custom session entry through `shuvpi.appendEntry()`. |
...
If compaction was aborted, `result` is absent and `aborted` is true. If it failed, `result` is absent, `aborted` is false, and `errorMessage` describes the failure. Successful overflow recovery sets `willRetry` to true before Shuvpi retries the prompt.
...
Use the exported `JsonAgentSessionEvent` type from `@shuv1337/shuvpi-coding-agent`. Its implementation is in [`json-event.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/modes/json-event.ts).
