...
`@shuv1337/shuvpi-tui` provides the terminal component system used by Shuvpi. Extensions use it when built-in dialogs, notifications, status text, and widgets are not enough for the interaction they need.
...
These APIs receive Shuvpi’s active theme and keybindings where needed. Do not create a second terminal renderer inside an extension.
...
Use `visibleWidth()`, `truncateToWidth()`, `sliceByColumn()`, and `wrapTextWithAnsi()` instead of implementing terminal-width handling yourself. Shuvpi resets styling and hyperlinks after every line, so reapply styles on each rendered line.
...
Prefer these components over rebuilding selection, scrolling, text editing, or width handling. The extension examples show how to combine them with Shuvpi’s borders and themes.
...
Extend Shuvpi’s `CustomEditor` when replacing the main editor. It preserves application shortcuts and agent controls.
...
Use [Themes](themes.md) to create terminal palettes. Use Shuvpi’s `getMarkdownTheme()` when rendering Markdown that should match the active application theme.
...
Use `SHUVPI_TUI_WRITE_LOG` to capture the raw ANSI stream when diagnosing rendering problems. Test narrow widths, wide characters, resize events, theme changes, focus transitions, and both regular and fullscreen modes.
...
The public exports are defined in [`packages/tui/src/index.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/tui/src/index.ts). See [Extensions](extensions.md) for extension lifecycle, state, tools, events, and mode behavior.
