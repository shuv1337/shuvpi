...
Extensions are TypeScript modules that add executable behavior to Shuvpi. Use one when a workflow needs tools, commands, event handlers, model providers, session state, or terminal UI rather than instructions alone.

An extension runs inside the Shuvpi process with the same operating-system permissions. It can inspect prompts, tool calls, files, credentials, and session history, so load extensions only from sources you trust.
...
Create `~/.shuvpi/agent/extensions/hello.ts`:
...
Start Shuvpi and run `/hello`. During development, load a file directly:

```bash
shuvpi --extension ./hello.ts
```

Shuvpi uses `jiti`, so local TypeScript extensions do not need a separate compilation step. Use [Shuvpi packages](packages.md) for distributed extensions and dependencies.
...
Place the extension in your user or project extensions directory. Shuvpi loads direct TypeScript or JavaScript files and subdirectories containing an `index.ts` or `index.js` entry point.
...
The factory can be synchronous or asynchronous. Shuvpi waits for an asynchronous factory before startup continues, allowing it to fetch configuration or register providers needed during startup.
...
`agent_settled` is final and notification-only; use it when an integration needs to know Shuvpi will not continue automatically.
...
| Capability | Main API |
|---|---|
| Observe or modify lifecycle behavior | `shuvpi.on()` |
| Add a model-callable operation | `shuvpi.registerTool()` |
| Add a `/` command | `shuvpi.registerCommand()` |
| Add a shortcut or CLI flag | `shuvpi.registerShortcut()` or `shuvpi.registerFlag()` |
| Send user or custom messages | `shuvpi.sendUserMessage()` or `shuvpi.sendMessage()` |
| Persist non-context session data | `shuvpi.appendEntry()` |
| Change active tools, model, or thinking level | Session control methods on `shuvpi` |
| Add a model provider | `shuvpi.registerProvider()` |
| Add terminal rendering | Renderer registration and `ctx.ui` |
| Communicate with another extension | `shuvpi.events` |

Use the exported declarations in [`extensions/types.ts`](https://github.com/shuv1337/shuvpi/blob/main/packages/coding-agent/src/core/extensions/types.ts) for exact event, context, tool, and result types.
...
Handlers run in extension load and registration order. `shuvpi.on()` returns a function that unsubscribes that registration; changes do not affect a dispatch already in progress.
...
`before_agent_start` exposes both the current prompt and its structured `systemPromptOptions`. Prefer changing prompt sections, selected tools, or guidelines so Shuvpi can append a transcript delta. Returning `systemPrompt`, or setting `forceSystemPrompt`, replaces the whole prompt for that run while the transcript continues recording the structured sections. Providers receive the forced text as their leading system prompt.
...
`context` transforms conversation messages without prompt and tool system messages; Shuvpi restores that state afterward. Use `context_with_system` only when a request-local transformation must own the complete transcript, and keep a system message at index zero.
...
Register every tool first, keep optional tools inactive, and use `shuvpi.setActiveTools()` from a loader tool to select the desired active tools. Names must already be registered; unknown names are ignored.

Shuvpi records the initial prompt and tool set in the transcript's first system message, then appends tool and prompt changes before the next model request. Providers that cannot represent the transition receive a complete transcript checkpoint, which can invalidate the cached prefix.
...
| Durable data excluded from model context | `shuvpi.appendEntry()` |
| Custom content stored and sent to the model | `shuvpi.sendMessage()` |
...
Shuvpi reports handler errors and continues where possible. A `tool_call` handler failure blocks the tool as a fail-safe; a tool execution failure becomes an error result for the model.
...
Use [Custom Providers](custom-provider.md) for model-service integrations, [Terminal UI](tui.md) for custom components, and [Shuvpi Packages](packages.md) to install or distribute extensions with other resources.
