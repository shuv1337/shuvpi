> shuvpi can create themes. Ask it to build one for your setup.
...
Shuvpi loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.shuvpi/agent/themes/*.json`
- Project: `.shuvpi/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `shuvpi.themes` entries in `package.json`
...
On first run, shuvpi detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.shuvpi/agent/themes
vim ~/.shuvpi/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/shuv1337/pi-mono/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, shuvpi reloads it automatically for immediate visual feedback.
...
Shuvpi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, shuvpi falls back to the nearest approximation.
