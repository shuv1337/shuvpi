...
When the next `prompt` command is sent, Shuvpi transforms context messages before sending them to the model. Unless `excludeFromContext` is true, the `BashExecutionMessage` becomes a `UserMessage` with this format:
...
2. Multiple bash commands can run before a prompt; Shuvpi includes each output that does not set `excludeFromContext`.
    ...
    "commands": [
      {
        ...
        "source": "prompt",
        "sourceInfo": {
          "path": "/home/user/myproject/.shuvpi/agent/prompts/fix-tests.md",
          "source": "local",
          "scope": "project",
          "origin": "top-level"
        }
...
- `sourceInfo`: Metadata for the resource that registered the command:
  ...
  - `source`: How Shuvpi discovered it, such as `"local"`, `"auto"`, or `"cli"`
...
For model configuration, see [Configure a compatible endpoint](models.md#configure-a-compatible-endpoint). For TypeScript, use the exported `Model` type from `@shuv1337/shuvpi-ai`.
