...
By default, running `shuvpi` opens the interactive terminal interface. When input or output is piped or redirected, Shuvpi uses print mode instead. You can also select print, JSON, or RPC mode explicitly for scripts and applications.

All four modes use the same agent, sessions, resources, and tools. The mode determines how input enters Shuvpi, how output is exposed, and whether the process remains available for more commands.
...
|---|---|---|---|
| Interactive | Terminal UI | Until the user exits | A person is working with Shuvpi directly |
...
```bash
shuvpi --print "Summarize the changes in this repository"
```
...
```bash
shuvpi --mode json "Review this repository" > events.jsonl
```
...
A failed or aborted assistant response appears in the event stream but does not by itself produce a nonzero exit status. Inspect the events when success or failure matters. Shuvpi still exits nonzero if the invocation throws an error.
...
## Control Shuvpi with RPC

RPC mode keeps Shuvpi running while another process sends commands and receives responses and events:
...
For Node.js or TypeScript integrations, prefer `RpcClient` from `@shuv1337/shuvpi-coding-agent`. It starts a Shuvpi RPC child process, correlates requests, exposes typed command methods, and delivers session events to listeners.
...
The client requires a path to a runnable Shuvpi CLI. The repository example points at `dist/cli.js`, so the package must be built before that example runs from a checkout.
