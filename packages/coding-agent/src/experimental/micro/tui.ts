import type { AssistantMessage, AuthEvent, ToolResultMessage, UserMessage } from "@shuv1337/shuvpi-ai";
import {
	type Component,
	Container,
	type Focusable,
...
class MicroTui {
	...
	#addEntry(entry: MicroView["conversation"]["entries"][number]): void {
		...
		if (entry.kind === "shuvpi.user" && message?.role === "user") this.#addUser(message as UserMessage);
		else if (entry.kind === "shuvpi.assistant") {
			const assistant =
				message?.role === "assistant"
					? (message as AssistantMessage)
					: (entry.data?.display as unknown as AssistantMessage | undefined);
			if (assistant) this.#addAssistant(assistant);
		} else if (entry.kind === "shuvpi.tool_result" && message?.role === "toolResult") {
			this.#tool(message.toolName, message.toolCallId).updateResult({
				...(message as ToolResultMessage),
				details: entry.data?.details,
			});
		} else if (entry.kind === "shuvpi.summary") {
			this.#addText("[compaction summary]");
			if (message?.role === "user") this.#addText(userContent(message.content));
		} else if (entry.kind === "shuvpi.notice" && message?.role === "user")
			this.#addText(userContent(message.content));
		else if (entry.kind === "shuvpi.handoff") this.#addText("[handoff]");
		else if (entry.kind === "shuvpi.reset") this.#addText("[reset]");
	}
