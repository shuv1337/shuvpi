...
import {
	...
	type ViewEvent,
	type Watch,
} from "@shuv1337/shuvpi-agent-core/experimental/pico3";
			...
			login: (providerId, authType) =>
				command(async () => {
					...
					if (!account.interactive)
						throw new Error(`${account.methodName ?? "Authentication"} is configured outside shuvpi`);
...
function usageView(
	...
	const newestSummary = conversation.entries
		.filter((entry) => entry.kind === "shuvpi.summary")
		.map((entry) => entry.id)
		.at(-1);
...
function assistantMessage(entry: Entry): AssistantMessage | undefined {
	...
	if (entry.kind !== "shuvpi.assistant" || !entry.data?.display) return undefined;
