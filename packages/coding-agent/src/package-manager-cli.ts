...
import { Markdown, type MarkdownTheme } from "@shuv1337/shuvpi-tui";
...
import { formatVersionCheckError, getLatestShuvpiRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getActiveManagedInstallRoot(): string | undefined {
	const configuredRoot = process.env.SHUVPI_MANAGED_INSTALL_ROOT?.trim();
	...
	// source checkout or another shuvpi installation launched from managed shuvpi as managed.
	...
	try {
		...
		if (marker.kind !== "shuvpi-managed-install" || marker.schemaVersion !== 1 || marker.layout !== "releases-v1") {
			throw new Error();
		}
	} catch {
...
async function fetchInstallerArtifact(url: string, label: string): Promise<string> {
	const response = await fetch(url, { headers: { "User-Agent": getShuvpiUserAgent(VERSION) } });
	if (!response.ok) {
...
function verifyManagedRelease(releaseDir: string, expectedVersion: string): void {
	...
	if (result.error || result.status !== 0) {
		...
		throw new Error(`Could not verify managed shuvpi ${expectedVersion}: ${reason}`);
	}
	const installedVersion = result.stdout.trim();
	if (installedVersion !== expectedVersion) {
		throw new Error(`Managed shuvpi smoke test returned version ${installedVersion}; expected ${expectedVersion}.`);
	}
}
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		...
		if (error instanceof Error && "code" in error && error.code === "ELOCKED") {
			throw new Error("Another managed shuvpi update is already running.");
		}
		throw error;
	}
	...
	try {
		cleanupManagedStaging(managedRoot);
		const installerApiBase = (process.env.SHUVPI_INSTALLER_API_BASE?.trim() || DEFAULT_INSTALLER_API_BASE).replace(
			/\/+$/,
			"",
		);
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "shuvpi";
			if (sourceIsSelf) {
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestShuvpiRelease>>;
	try {
		latestRelease = await getLatestShuvpiRelease(VERSION, { retry: true });
	} catch (error: unknown) {
