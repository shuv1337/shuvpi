import type { AuthResult } from "@shuv1337/shuvpi-ai";
...
const AUTH_COMMAND_USAGE: Record<AuthCommandKind, string> = {
	check: "shuvpi auth check --provider <provider> [--json] [--credentials] [--no-refresh]",
	api_key: "shuvpi auth print-api-key --provider <provider> [--model <model>]",
	bearer_token: "shuvpi auth print-bearer-token --provider <provider> [--model <model>] [--min-expiry <duration>]",
};
...
export function printAuthCommandHelp(): void {
	console.log(`Usage:
  shuvpi auth print-api-key [--provider <provider>] [--model <model>]
  shuvpi auth print-bearer-token [--provider <provider>] [--model <model>] [--min-expiry <duration>]
  shuvpi auth check [--provider <provider>] [--model <model>] [--json] [--credentials] [--no-refresh]
...
export function parseAuthCommand(args: string[]): AuthCommand | undefined {
	...
	if (!kind) {
		throw new AuthCommandError(
			`Unknown auth command "${args[1] ?? ""}". Use "shuvpi auth print-api-key", "shuvpi auth print-bearer-token", or "shuvpi auth check".`,
		);
	}
