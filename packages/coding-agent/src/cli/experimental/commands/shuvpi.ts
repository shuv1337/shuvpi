...
export interface ShuvpiCommand {
	readonly command: "shuvpi";
	readonly auth?: AuthInput;
	readonly options: Args;
	readonly listen?: readonly TransportAddress[];
}

export interface ShuvpiCommandContext {
	runShuvpi(command: ShuvpiCommand): void | Promise<void>;
}

const listenOption = transportOption("--listen");

export const shuvpiCommand = new Command<ShuvpiCommand, ShuvpiCommandContext>("shuvpi")
	.option(listenOption)
	...
	.action((command, context) => context.runShuvpi(command));
