...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as SHUVPI_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.SHUVPI_SESSION_ID = metadata.id;
						execution.env.SHUVPI_SESSION_FILE = sessionFile ?? "";
						execution.env.SHUVPI_PROVIDER = model.provider;
						execution.env.SHUVPI_MODEL = model.id;
						execution.env.SHUVPI_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
