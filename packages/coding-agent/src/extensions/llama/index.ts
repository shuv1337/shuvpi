...
export default function llamaExtension(shuvpi: ExtensionAPI): void {
	const provider = createLlamaProvider();
	shuvpi.registerProvider(provider.provider);
	...
	const syncCatalog = async (
		...
		const result = await ctx.modelRegistry.refresh({
			...
			// /llama already contacted the configured llama.cpp server, so keep this refresh live even in SHUVPI_OFFLINE.
			allowNetwork: true,
			signal,
		});
