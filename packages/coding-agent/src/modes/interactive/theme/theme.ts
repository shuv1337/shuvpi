...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		BUILTIN_THEMES = {
			...
			nightowl: JSON.parse(stripBom(fs.readFileSync(nightOwlPath, "utf-8"))) as ThemeJson,
			"nightowl-light": JSON.parse(stripBom(fs.readFileSync(nightOwlLightPath, "utf-8"))) as ThemeJson,
		};
	}
...
const THEME_KEY = Symbol.for("@shuv1337/shuvpi-coding-agent:theme");
const THEME_KEY_OLD = Symbol.for("@shuv1337/shuvpi-coding-agent:theme");
