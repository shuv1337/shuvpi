...
import type { Container, EditorComponent, TUI } from "@shuv1337/shuvpi-tui";
...
const DISCLAIMER =
	"This report goes to the Shuvpi developers (Earendil) and is not shared publicly. It includes your shuvpi version, operating system, the current model and provider configuration (without API keys), loaded extensions, settings, and provider error diagnostics from this session.";
...
export async function reportBug(context: BugReportContext, initialHint?: string): Promise<void> {
	...
	if (options.delivery === "upload" && process.env.SHUVPI_OFFLINE) {
