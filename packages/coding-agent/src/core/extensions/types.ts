...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@shuv1337/shuvpi-coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@shuv1337/shuvpi-coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown shuvpi and exit. Available in all contexts. */
	shutdown(): void;
 ...
 * belong to Shuvpi: it restores them after the handler returns, so a handler cannot drop
...
/**
 * Fired before each LLM call, after every `context` handler has run and Shuvpi has restored
...
export interface ExtensionAPI {
	...
	/**
	 * Run a registered extension command by name with a real ExtensionCommandContext.
	 * Does not call prompt()/sendUserMessage(), does not append a conversation entry,
	 * and does not trigger an agent/model turn.
	 */
...
/** Configuration for registering a provider via shuvpi.registerProvider(). */
...
export type ExtensionFactory = (shuvpi: ExtensionAPI) => void | Promise<void>;
...
export type RunCommandHandler = (name: string, args?: string) => Promise<void>;
...
/**
 * Action implementations for shuvpi.* API methods.
