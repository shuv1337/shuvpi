...
import { contentText, normalizeContext, type RetryPolicy, uuidv7 } from "@shuv1337/shuvpi-ai";
import type { Api, Model, Provider, SimpleStreamOptions } from "@shuv1337/shuvpi-ai/compat";
...
export const BUG_REPORT_CUSTOM_ENTRY_TYPE = "shuvpi.bug-report";
...
function collectEnvironment() {
	...
	return {
		version: VERSION,
		userAgent: getShuvpiUserAgent(VERSION),
		...
		shuvpiEnvironmentVariables: Object.keys(process.env)
...
export function bugReportArchiveFileName(id: string): string {
	return `shuvpi-bug-report-${id}.zip`;
}

const BUG_SUMMARY_SYSTEM_PROMPT = `You are helping a user file a bug report about shuvpi, the coding agent they are talking to. You will be shown the conversation transcript. Write a report for the shuvpi developers describing what the user was doing and what went wrong.
