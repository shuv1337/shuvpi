...
import { Agent, type AgentMessage, setDefaultStreamFn, type ThinkingLevel } from "@shuv1337/shuvpi-agent-core";
import type { ModelsSimpleStreamOptions } from "@shuv1337/shuvpi-ai";
import { clampThinkingLevel, type Message, type Model, streamSimple } from "@shuv1337/shuvpi-ai/compat";
...
import { applyFastModeToPayload } from "./fast-mode.ts";
...
// provider-agnostic and does not import shuvpi-ai/compat itself.
setDefaultStreamFn(streamSimple);

export interface CreateAgentSessionOptions {
	/** Working directory for project-local discovery. Default: process.cwd() */
	cwd?: string;
	/** Global config directory. Default: ~/.shuvpi/agent */
	agentDir?: string;
	 ...
	 *
	 * When omitted, Shuvpi uses the `defaultTools` setting for the initial built-in
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const transformProviderPayload = async (payload: unknown, payloadModel: Model<any>) => {
		const nextPayload = applyFastModeToPayload(
			payload,
			payloadModel,
			sessionRef.current?.isFastModeActiveForCurrentModel() ?? false,
		);
		const runner = extensionRunnerRef.current;
		if (!runner?.hasHandlers("before_provider_request")) return nextPayload;
