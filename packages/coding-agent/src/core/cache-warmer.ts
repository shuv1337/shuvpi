...
import { getProviderEnvValue } from "@shuv1337/shuvpi-ai/utils/provider-env";
...
export function getPromptCacheTtlMs(model: Model<Api>, options: SimpleStreamOptions | undefined): number | undefined {
	const retention =
		...
		(getProviderEnvValue("SHUVPI_CACHE_RETENTION", options?.env) === "long" ? "long" : "short");
...
export interface CacheWarmingDecision {
	...
	/** Shuvpi's decision: "warm" when `expectedSavings` is at least $0.05. */
	action: CacheWarmingAction;
}

/**
 * Fired before each refresh with shuvpi's decision filled in. Everything else an
...
export class CacheWarmer {
	...
	/** Lets extensions override `event.action`; failures fall back to shuvpi's decision. */
	...
	private async refresh(run: ActiveRun): Promise<void> {
		...
		try {
			...
			// Extension failures fall back to shuvpi's own decision.
		}
...
export function formatCacheWarmingStatus(status: CacheWarmingStatus, now = Date.now()): string {
	...
	// A decision is attached once shuvpi (or an extension) acted on it; "inactive"
