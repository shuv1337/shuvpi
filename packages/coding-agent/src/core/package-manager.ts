...
function isOfflineModeEnabled(): boolean {
	const value = process.env.SHUVPI_OFFLINE;
...
type SkillDiscoveryMode = "shuvpi" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "shuvpi" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "shuvpi");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside shuvpi and resolve shuvpi APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @shuv1337/shuvpi-* peers. Stale auto-installed shuvpi peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "shuvpi-extensions", private: true };
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			// Project extensions from .shuvpi/
			addResources(
				"extensions",
			...
			// Project skills from .shuvpi/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "shuvpi"),
				projectMetadata,
		...
		// User extensions from ~/.shuvpi/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.shuvpi/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "shuvpi"),
			userMetadata,
