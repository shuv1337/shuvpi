import type { Api, Model, ProviderHeaders } from "@shuv1337/shuvpi-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "Shuvpi",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "shuvpi-coding-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "shuvpi" };
}
