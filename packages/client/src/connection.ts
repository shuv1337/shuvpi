...
import { ShuvpiDisconnectedError, ShuvpiServerError, toDisconnectedError, toError } from "./errors.ts";
...
export class Connection {
	...
	constructor(options: ConnectionOptions) {
		...
		if (
			...
			throw new TypeError(`ShuvpiClient maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
		}
	}
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#lifecycle.state !== "disconnected") {
			return Promise.reject(new ShuvpiDisconnectedError(`ShuvpiClient is already ${this.#lifecycle.state}`));
		}
	...
	disconnect(reason: string | Error = "Client disconnected"): void {
		...
		this.#failAndClose(typeof reason === "string" ? new ShuvpiDisconnectedError(reason) : reason);
	}
	...
	send(frame: Uint8Array): void {
		...
		if (lifecycle.state !== "connected") throw new ShuvpiDisconnectedError();
	...
	#handleMessage(message: ServerMessage): void {
		...
		if (lifecycle.state === "connecting") {
			if (message.type === "hello_error") {
				this.#failAndClose(new ShuvpiServerError(message.error));
				return;
			}
	...
	#handleClose(): void {
		...
		let error: Error = new ShuvpiDisconnectedError("Byte transport closed");
		try {
