...
export class ShuvpiClient {
	readonly #options: ShuvpiClientOptions;
	...
	constructor(options: ShuvpiClientOptions) {
	...
	static async connect(options: ShuvpiClientOptions): Promise<ShuvpiClient> {
		const client = new ShuvpiClient(options);
		try {
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#disposed) return Promise.reject(new ShuvpiClientDisposedError());
	...
	async createSession(options: CreateSessionOptions = {}): Promise<ShuvpiSessionHandle> {
	...
	async attachSession(sessionId: string): Promise<ShuvpiSessionHandle> {
		return this.acquireSession(sessionId, { mode: "shared" });
	}

	async acquireSession(sessionId: string, options: AcquireSessionOptions): Promise<ShuvpiSessionHandle> {
		this.#assertNotDisposed();
	...
	#request<const TCommand extends Command>(command: TCommand): Promise<ResultForCommand<TCommand>> {
		if (this.#disposed) return Promise.reject(new ShuvpiClientDisposedError());
		if (!this.connected) return Promise.reject(new ShuvpiDisconnectedError());
	...
	#createSessionLease(sessionId: string, token: SessionLeaseToken): ShuvpiSessionHandle {
		...
		const assertActive = () => {
			this.#assertNotDisposed();
			if (!this.connected) throw new ShuvpiDisconnectedError();
			if (!isActive()) throw new ShuvpiSessionDetachedError(sessionId);
		};
	...
	#handleMessage(message: ResponseEnvelope | EventEnvelope): void {
		...
		if (!message.ok) {
			pending.reject(new ShuvpiServerError(message.error));
			return;
		}
	...
	#handleConnectionStateChange(change: ConnectionStateChange): void {
		if (change.state === "disconnected") {
			...
			this.#rejectPendingRequests(change.error ?? new ShuvpiDisconnectedError());
		}
	...
	dispose(): Promise<void> {
		...
		const error = new ShuvpiClientDisposedError();
	...
	#assertNotDisposed(): void {
		if (this.#disposed) throw new ShuvpiClientDisposedError();
	}
	...
	#reserveSessionLease(sessionId: string, mode: SessionLeaseMode): SessionLeaseToken {
		...
		if (mode === "exclusive" && count > 0) {
			throw new ShuvpiSessionOwnershipError(sessionId, `Session ${sessionId} already has an active lease`);
		}
		if (mode === "shared" && this.#exclusiveSessionLeases.has(sessionId)) {
			throw new ShuvpiSessionOwnershipError(sessionId, `Session ${sessionId} has an exclusive lease`);
		}
