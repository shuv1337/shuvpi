...
Transport-neutral client for the experimental Shuvpi service protocol.

```ts
import { Client, type ByteTransportFactory } from "@shuv1337/shuvpi-client";
...
```ts
import { Client } from "@shuv1337/shuvpi-client";
import { createUnixTransportFactory } from "@shuv1337/shuvpi-client/unix";

const client = new Client({
  serverId: "01234567-89ab-4def-8123-456789abcdef",
  transportFactory: createUnixTransportFactory({ path: "/tmp/shuvpi.sock" }),
});
...
```ts
import { discoverUnixServers } from "@shuv1337/shuvpi-client/unix";

const routes = await discoverUnixServers({ directory: "/run/user/1000/shuvpi" });
// [{ serverId: "...", path: "/run/user/1000/shuvpi/<serverId>.sock" }]
```
