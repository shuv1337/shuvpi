...
import { ShuvpiSessionDetachedError, ShuvpiSessionOwnershipError } from "../src/index.ts";
...
describe("ShuvpiClient", () => {
	test("keeps multiple session handles independent and enforces detach", async () => {
		...
		expect(second.attached).toBe(true);
		await expect(first.abort()).rejects.toBeInstanceOf(ShuvpiSessionDetachedError);
	});
	...
	test("rejects commands while releasing and restores an explicit detach after failure", async () => {
		...
		await expect(lease.abort()).rejects.toBeInstanceOf(ShuvpiSessionDetachedError);
