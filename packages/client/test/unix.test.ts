...
import { ShuvpiClient } from "../src/index.ts";
...
test("rejects invalid Unix transport options", () => {
	...
	expect(() => createUnixTransportFactory({ path: "/tmp/shuvpi.sock", maxPendingBytes: 0 })).toThrow(/positive/);
});

describe.runIf(process.platform !== "win32")("Unix-domain sockets", () => {
	test("ShuvpiClient exchanges fragmented framed messages over a real Unix socket", async () => {
		const directory = await mkdtemp(join(tmpdir(), "shuvpi-client-"));
		const socketPath = join(directory, "shuvpi.sock");
	...
	test("ShuvpiClient rejects a truncated final frame from a real Unix socket", async () => {
		const directory = await mkdtemp(join(tmpdir(), "shuvpi-client-"));
		const socketPath = join(directory, "shuvpi.sock");
		const sockets = new Set<Socket>();
		const server = createServer((socket) => {
