...
import { type ByteTransportFactory, ShuvpiClient, ShuvpiDisconnectedError } from "../src/index.ts";
import {
	attachSession,
	baseServerSnapshot,
	connectClient,
...
describe("ShuvpiClient", () => {
	...
	test("does not restore a connection after a snapshot listener disconnects during handshake", async () => {
		...
		await expect(client.connect()).rejects.toBeInstanceOf(ShuvpiDisconnectedError);
		expect(client.connectionState).toBe("disconnected");
	...
	test("rejects pending requests on close and reconnects through a fresh factory result", async () => {
		...
		first.close();
		await expect(pending).rejects.toBeInstanceOf(ShuvpiDisconnectedError);
		expect(client.connectionState).toBe("disconnected");
	...
	test("rejects pending requests on transport errors", async () => {
		...
		await expect(pending).rejects.toMatchObject({ name: "ShuvpiDisconnectedError", message: "read failed" });
		expect(client.connectionState).toBe("disconnected");
	});
