...
import type { ByteTransport, ByteTransportHandlers, ShuvpiSessionHandle } from "../src/index.ts";
...
export function createClient(server: MemoryByteServer): ShuvpiClient {
	return new ShuvpiClient({
		transportFactory: (handlers) => server.connect(handlers),
	});
}

export async function connectClient(server: MemoryByteServer): Promise<ShuvpiClient> {
