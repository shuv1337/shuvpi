...
import { classifyCaseStatus, readTaskObservation, SHUVPI_SESSION_SNAPSHOT_ARTIFACT } from "../src/report.ts";
...
async function writeTaskReport(assertion: ReportAssertion = {}) {
	const directory = await mkdtemp(join(tmpdir(), "shuvpi-eval-report-test-"));
	...
	return {
		...
		harness: {
			...
			run: {
				...
				artifacts: overrides?.artifacts ?? { runId: "run-1", [SHUVPI_SESSION_SNAPSHOT_ARTIFACT]: SESSION },
