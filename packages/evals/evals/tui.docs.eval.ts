...
import { createShuvpiDocumentationEvalHarness, type ShuvpiCodingAgentInput } from "../src/harness.ts";
...
async function inspectContextFooter(session: AgentSession, agentDir: string): Promise<TuiFooterOutput> {
	...
	const previousOffline = process.env.SHUVPI_OFFLINE;
	...
	try {
		...
		try {
			...
			try {
				...
				if (previousOffline === undefined) delete process.env.SHUVPI_OFFLINE;
				else process.env.SHUVPI_OFFLINE = previousOffline;
			}
		}
	}
}
...
const contextFooterJudge = createJudge<ShuvpiCodingAgentInput, TuiFooterOutput>(
	"ContextFooterJudge",
	async ({ output }) => {
			...
			await run([
				{
					type: "prompt",
					content:
						"Change the Shuvpi footer replacing the built-in context info, e.g. `42.2%/272k (auto)`, with a ten-cell progress bar to make it show ████░░░░░░ 42% instead. Clamp percentages to the 0-100 range. Just replace the context. Otherwise leave everything else the same.",
				},
				{ type: "reload" },
			]);
		});
