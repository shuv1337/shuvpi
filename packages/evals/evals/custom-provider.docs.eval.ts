...
describeEval("Add custom streaming provider", { harness, judges: [judge], judgeThreshold: null }, (it) => {
	it("implements a provider from an API fixture", async ({ run }) => {
		await run([
			{
				type: "prompt",
				content: `Configure this running Shuvpi installation with Acme Stream as a provider. Do not create a project package or modify project source. Its provider ID is ${STREAM_PROVIDER_ID}, its API is at ${server.origin()}, and its API documentation is in ./${DOCUMENTATION_PATH}. Read its credential from the ACME_STREAM_API_KEY environment variable.
