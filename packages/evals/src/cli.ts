...
function parseEvalCli(args: readonly string[], environment: NodeJS.ProcessEnv = process.env): EvalCliOptions {
	...
	if (cliSelectedModel) {
		...
		provider = environment.SHUVPI_PROVIDER?.trim() || undefined;
		model = environment.SHUVPI_MODEL?.trim() || undefined;
		if (Boolean(provider) !== Boolean(model))
			throw new Error("Set both SHUVPI_PROVIDER and SHUVPI_MODEL, or neither.");
	}

	const configuredRuns = (runsText ?? environment.SHUVPI_EVAL_RUNS_PER_VARIANT)?.trim();
...
if (!cli.provider || !cli.model)
	throw new Error("Set SHUVPI_PROVIDER and SHUVPI_MODEL, or pass --provider and --model.");
