...
import { contentText, InMemoryCredentialStore } from "@shuv1337/shuvpi-ai";
import { getCurrentSystemPrompt } from "@shuv1337/shuvpi-ai/utils/transcript";
import {
	type AgentSession,
...
import { SHUVPI_SESSION_SNAPSHOT_ARTIFACT } from "./report.ts";

type ShuvpiRunDiagnostics = {
...
export type ShuvpiCodingAgentModelSelection = {
	provider: string;
	id: string;
};

export type ShuvpiCodingAgentHarnessOptions = {
	name?: string;
	...
	expectedShuvpiDocumentation?: boolean;
};

export type ShuvpiCodingAgentHarnessWithOutput<TOutput extends JsonValue> = ShuvpiCodingAgentHarnessOptions & {
	output: (args: {
		response: string;
...
export function applyIsolatedEnvironment(home: string, agentDir: string): () => void {
	const overrides = { HOME: home, USERPROFILE: home, SHUVPI_CODING_AGENT_DIR: agentDir };
	const previous = new Map<string, string | undefined>();
	for (const name of Object.keys(process.env)) {
		if (!name.startsWith("SHUVPI_EVAL_")) continue;
...
function parseSandboxId(name: "SHUVPI_EVAL_SANDBOX_UID" | "SHUVPI_EVAL_SANDBOX_GID"): number | undefined {
...
function resolveSandboxIdentity(): SandboxIdentity | undefined {
	const uid = parseSandboxId("SHUVPI_EVAL_SANDBOX_UID");
	const gid = parseSandboxId("SHUVPI_EVAL_SANDBOX_GID");
	if (uid === undefined && gid === undefined) return undefined;
	if (uid === undefined || gid === undefined) {
		throw new Error("Set both SHUVPI_EVAL_SANDBOX_UID and SHUVPI_EVAL_SANDBOX_GID, or neither.");
	}
	return { uid, gid };
}
...
export function verifySystemPrompt(
	systemPrompt: string,
	options: Pick<ShuvpiCodingAgentHarnessOptions, "name" | "expectedShuvpiDocumentation">,
): string {
	if (options.expectedShuvpiDocumentation === undefined) return systemPrompt;
	if (!systemPrompt.includes("\n<rules>\n")) {
		throw new Error(`Shuvpi system prompt lost its rules in the ${options.name} eval variant.`);
	}
	const hasDocumentation = systemPrompt.includes("\n<docs>\nShuvpi documentation (read only");
	if (hasDocumentation !== options.expectedShuvpiDocumentation) {
		throw new Error(`Shuvpi system prompt does not match the ${options.name} eval variant.`);
	}
	return systemPrompt;
}
...
async function runShuvpiCodingAgent<TOutput extends JsonValue>(
	...
	const agentDir = join(isolatedHome, ".shuvpi", "agent");
	...
	if (options.transformSystemPrompt) {
		...
		extensionFactories.push({
			...
			hidden: true,
			factory: (shuvpi) => {
				shuvpi.on("before_agent_start", ({ systemPrompt }) => {
	...
	let runDiagnostics: ShuvpiRunDiagnostics | undefined;
	...
	try {
		...
		if (response === undefined) {
			throw new Error("Shuvpi eval input must include at least one prompt step.");
		}
		...
		if (sessionManager) {
			...
			if (!sessionPath || !existsSync(sessionPath)) {
				cleanupErrors.push(new Error("Shuvpi eval produced no session file."));
			} else {
				try {
	...
	if (!result) throw new Error("Shuvpi eval completed without a result.");
...
export function createShuvpiCodingAgentHarness<TOutput extends JsonValue>(
	options: ShuvpiCodingAgentHarnessOptions | ShuvpiCodingAgentHarnessWithOutput<TOutput> = {},
): Harness<ShuvpiCodingAgentInput, string | TOutput> {
...
export function resolveDocumentationVariant(
	value: string | undefined = process.env.SHUVPI_EVAL_VARIANT,
): DocumentationVariant {
	if (value === "without_docs" || value === "with_docs") return value;
	throw new TypeError('SHUVPI_EVAL_VARIANT must be "without_docs" or "with_docs".');
}

export function excludeShuvpiDocumentation(defaultPrompt: string): string {
	...
	if (documentationStart === -1) throw new Error("Default Shuvpi system prompt has no Shuvpi documentation section.");
	const documentationEnd = defaultPrompt.indexOf(documentationEndMarker, documentationStart);
	if (documentationEnd === -1)
		throw new Error("Default Shuvpi system prompt has no complete Shuvpi documentation section.");
	const cwdStart = defaultPrompt.lastIndexOf("\n<cwd>\n");
	if (cwdStart < documentationEnd) throw new Error("Default Shuvpi system prompt has no working-directory section.");
	return (
...
type DocumentationHarnessOptions = Omit<
	ShuvpiCodingAgentHarnessOptions,
	"name" | "transformSystemPrompt" | "expectedShuvpiDocumentation"
>;
type DocumentationHarnessWithOutput<TOutput extends JsonValue> = Omit<
	ShuvpiCodingAgentHarnessWithOutput<TOutput>,
	"name" | "transformSystemPrompt" | "expectedShuvpiDocumentation"
>;

export function createShuvpiDocumentationEvalHarness<TOutput extends JsonValue>(
	options: DocumentationHarnessWithOutput<TOutput>,
): Harness<ShuvpiCodingAgentInput, TOutput>;
export function createShuvpiDocumentationEvalHarness(
	options?: DocumentationHarnessOptions,
): Harness<ShuvpiCodingAgentInput, string>;
export function createShuvpiDocumentationEvalHarness<TOutput extends JsonValue>(
	options: DocumentationHarnessOptions | DocumentationHarnessWithOutput<TOutput> = {},
) {
	if (process.env.SHUVPI_EVAL_CONTAINER !== "1" || !resolveSandboxIdentity()) {
		throw new Error("Documentation evals must run in the isolated container sandbox.");
	}
	const variant = resolveDocumentationVariant();
	return createShuvpiCodingAgentHarness({
		...options,
		name: variant,
		tools: options.tools ?? [...DOCUMENTATION_EVAL_TOOLS],
		...(variant === "without_docs" ? { transformSystemPrompt: excludeShuvpiDocumentation } : {}),
		expectedShuvpiDocumentation: variant === "with_docs",
	});
}
