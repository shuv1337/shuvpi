...
import { createShuvpiCodingAgentHarness, type ShuvpiCodingAgentInput } from "./shuvpi-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasShuvpiDocs: boolean;
...
function createExtensionAuthoringHarness(name: string, transformSystemPrompt?: (defaultPrompt: string) => string) {
	return createShuvpiCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			const extensionPath = join(session.sessionManager.getCwd(), ".shuvpi", "extensions", "hello.ts");
			...
			return {
				...
				systemPromptHasShuvpiDocs: session.systemPrompt.includes("\nShuvpi documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Shuvpi system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Shuvpi system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<ShuvpiCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@shuv1337/shuvpi-coding-agent")) {
				failures.push("extension does not import the canonical @shuv1337/shuvpi-coding-agent package");
			}
...
const extensionHarnessTable = evalHarnessTable("Shuvpi extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Shuvpi extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create a Shuvpi extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Shuvpi eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasShuvpiDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
