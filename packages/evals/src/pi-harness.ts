...
function getRequiredModelSelection(): { provider: string; model: string } {
	const provider = process.env.SHUVPI_PROVIDER?.trim();
	const model = process.env.SHUVPI_MODEL?.trim();
	if (!provider || !model) {
		throw new Error("SHUVPI_PROVIDER and SHUVPI_MODEL must both be set for eval runs.");
	}
