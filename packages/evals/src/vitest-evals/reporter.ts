...
import { persistEvalArtifactReferences, SHUVPI_SESSION_SNAPSHOT_ARTIFACT } from "./artifacts.ts";
...
async function appendHarnessRunReport(test: TestCase): Promise<void> {
	const artifactDirectory = process.env.SHUVPI_EVAL_ARTIFACT_DIR?.trim();
	...
	const metadata = Object.fromEntries(
		Object.entries(run.artifacts ?? {}).filter(
			([name]) => name !== "runId" && name !== SHUVPI_SESSION_SNAPSHOT_ARTIFACT,
		),
	);
	const record = {
		schemaVersion: 1,
