...
export class TuiMainScreen extends TuiBase implements TUI {
	...
	protected doRender(): void {
		...
		const redrawLogDirectory = process.env.SHUVPI_TUI_DEBUG_REDRAW === "1" ? this.logDirectory : undefined;
		const logRedraw = (reason: string): void => {
			if (redrawLogDirectory === undefined) return;
			const logPath = path.join(redrawLogDirectory, "shuvpi-tui-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(this.logDirectory ?? os.tmpdir(), "shuvpi-tui-crash.log");
				const crashData = [
