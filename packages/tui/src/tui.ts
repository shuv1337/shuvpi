...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.SHUVPI_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.SHUVPI_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory =
			logDirectory ?? process.env.SHUVPI_CODING_AGENT_DIR ?? path.join(os.homedir(), ".shuvpi", "agent");
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.SHUVPI_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(this.logDirectory, "shuvpi-debug.log");
		...
		// Configurable via setClearOnShrink() or SHUVPI_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(this.logDirectory, "shuvpi-crash.log");
				const crashData = [
		...
		if (process.env.SHUVPI_TUI_DEBUG === "1") {
