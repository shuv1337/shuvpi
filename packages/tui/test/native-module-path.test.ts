...
describe("getNativeModuleCandidates", () => {
	it("resolves native helpers from the installed TUI package when the module is bundled elsewhere", () => {
		const packageRoot = resolve("virtual", "node_modules", "@earendil-works", "shuvpi-tui");
		const bundledModule = resolve("virtual", "shuvpi-coding-agent", "dist", "bundle", "chunks", "chunk.js");
		...
		const candidates = getNativeModuleCandidates(nativePath, {
			...
			resolvePackage: (specifier) => {
				assert.equal(specifier, "@shuv1337/shuvpi-tui");
	...
	it("keeps standalone binary fallbacks when the TUI package is unavailable", () => {
		const bundledModule = resolve("virtual", "shuvpi", "bundle", "chunks", "chunk.js");
		const execPath = resolve("virtual", "shuvpi", "shuvpi.exe");
