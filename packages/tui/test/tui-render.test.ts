...
describe("TUI debug logging", () => {
	it("writes redraw logs to the provided directory", async () => {
		const logDir = mkdtempSync(join(tmpdir(), "shuvpi-tui-log-"));
		try {
			await withEnv({ SHUVPI_DEBUG_REDRAW: "1" }, async () => {
				...
				assert.match(readFileSync(join(logDir, "shuvpi-debug.log"), "utf-8"), /fullRender: first render/);
				tui.stop();
			});
		} finally {
