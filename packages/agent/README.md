...
The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@shuv1337/shuvpi-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
Transport-neutral facet-service primitives live in `@shuv1337/shuvpi-chord`. The agent core does not export the service runtime.
