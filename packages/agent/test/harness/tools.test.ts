	...
	describe("bash", () => {
		...
		it("prepares command, cwd, and an explicit environment with the turn context", async () => {
			const env = new NodeExecutionEnv({
				cwd: createTempDir(),
				shellEnv: { SHUVPI_BASH_PREPARE_INHERITED: "inherited" },
			});
			...
			const tool = createBashTool<typeof context>({
				...
				prepare: async (execution, turnContext, callContext) => {
					...
					execution.env = { SHUVPI_BASH_PREPARE_EXPLICIT: "explicit" };
					execution.inheritEnv = false;
					execution.command += `\nprintf '%s:%s:%s:%s' "$prefix" "\${SHUVPI_BASH_PREPARE_INHERITED-}" "$SHUVPI_BASH_PREPARE_EXPLICIT" "$PWD"`;
				},
			});
