...
describe("NodeExecutionEnv", () => {
	...
	it.each([
		...
		["an empty override shadows the base value", { SHUVPI_SESSION_FILE: "" }, "x:"],
		[
		...
		async (_description, overrides, expectedSessionFile) => {
			...
			const env = new NodeExecutionEnv({
				cwd: root,
				shellEnv: {
					SHUVPI_SESSION_FILE: "/stale/parent.jsonl",
					SHUVPI_CODING_AGENT: "true",
					SHUVPI_NODE_ENV_PRESERVED_TEST: "preserved",
				},
			});
			const collected = await collectShellOutput(
				env,
				`printf '%s:%s|%s|%s' "\${SHUVPI_SESSION_FILE+x}" "\${SHUVPI_SESSION_FILE-}" "$SHUVPI_CODING_AGENT" "$SHUVPI_NODE_ENV_PRESERVED_TEST"`,
				{ env: overrides },
				BACKGROUND_CONTEXT,
			);
	...
	it("can replace rather than inherit the default shell environment", async () => {
		const root = createTempDir();
		const inheritedKey = "SHUVPI_NODE_ENV_INHERITED_TEST";
		const configuredKey = "SHUVPI_NODE_ENV_CONFIGURED_TEST";
		const explicitKey = "SHUVPI_NODE_ENV_EXPLICIT_TEST";
