import { createModels, fauxAssistantMessage, fauxProvider, type MutableModels } from "@shuv1337/shuvpi-ai";
...
describe("runtime structural drive", () => {
	...
	it("gives each split-turn provider request its own durable intent and usage row", async () => {
		...
		const requestIndices = attempts.flatMap((writes) =>
			writes.flatMap((write) =>
				write.kind === "value" &&
				write.op === "set" &&
				write.namespace === "shuvpi.op.state" &&
		...
		expect(
			attempts.flat().some((write) => write.kind === "list" && write.namespace === "shuvpi.pending.assistant_frame"),
		).toBe(false);
		expect(
	...
	it("moves an unsummarized navigation and cleans up in one terminal transaction", async () => {
		...
		expect(
			writes.some(
				(write) => write.kind === "value" && write.namespace === "shuvpi.op.state" && write.op === "delete",
			),
		).toBe(true);
		expect(writes.some((write) => write.kind === "value" && write.namespace === "shuvpi.result")).toBe(true);
		expect(writes.some((write) => write.kind === "value" && write.namespace === "shuvpi.lane.state")).toBe(true);
	});
