...
describe("runtime terminal cleanup mechanics", () => {
	it("deletes every operation-owned family and exact live frame list but preserves the lane inbox", async () => {
		...
		expect(writes.map(address)).toEqual([
			"value:delete:shuvpi.op.meta:run",
			"value:delete:shuvpi.op.state:run",
			"value:delete:shuvpi.op.tool_args:run:step:0",
			"value:delete:shuvpi.op.tool_memo:run:invocation:memo",
			"value:delete:shuvpi.op.preparation:run:task",
			"value:delete:shuvpi.pending.tool_output:run:invocation",
			"list:delete:shuvpi.pending.assistant_frame:run:response",
		]);
	...
	it("deletes staged tool outcomes and leaves completed results alone", async () => {
		...
		expect(writes.map(address)).toContain("value:delete:shuvpi.pending.entry:staged");
		expect(writes.map(address)).not.toContain("value:delete:shuvpi.pending.entry:placed");
	});
	...
	it.each([
		...
		expect(writes.map(address)).toEqual([
			`value:delete:shuvpi.op.meta:${operationId}`,
			`value:delete:shuvpi.op.state:${operationId}`,
			`value:delete:shuvpi.op.tool_args:${operationId}:step:0`,
			`value:delete:shuvpi.op.tool_memo:${operationId}:invocation:memo`,
			`value:delete:shuvpi.op.preparation:${operationId}:task`,
			`value:delete:shuvpi.pending.tool_output:${operationId}:invocation`,
		]);
	});
});
