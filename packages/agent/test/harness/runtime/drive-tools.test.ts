...
class ObservedMemoryStorage extends MemoryStorage {
	...
	override async commit(writes: Write[], context: Context) {
		...
		if (
			writes.some(
				(write) => write.kind === "value" && write.op === "set" && write.namespace === "shuvpi.op.tool_args",
			)
		) {
			this.observations.push("intent_commit");
		}
		const stagesOutcome = writes.some(
			(write) => write.kind === "value" && write.op === "set" && write.namespace === "shuvpi.pending.entry",
		);
		...
		if (
			...
			writes.some(
				(write) =>
					write.kind === "value" && write.op === "delete" && write.namespace === "shuvpi.pending.tool_output",
			)
		) {
