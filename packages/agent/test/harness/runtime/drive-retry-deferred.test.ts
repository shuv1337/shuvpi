...
describe("runtime deferred polling", () => {
	...
	it("replaces an unknown poll under fresh ids at the same poll number and deletes old frames", async () => {
			...
			.find((writes) =>
				writes.some(
					(write) =>
						write.kind === "list" &&
						write.op === "delete" &&
						write.namespace === "shuvpi.pending.assistant_frame" &&
		...
		const intentState = intent?.find(
			(write) => write.kind === "value" && write.op === "set" && write.namespace === "shuvpi.op.state",
		);
	...
	it("abandons unknown ids and frames into configuration failure when the captured model is unavailable", async () => {
		...
		expect(
			...
			"value:delete:shuvpi.op.meta",
			"value:delete:shuvpi.op.state",
			"list:delete:shuvpi.pending.assistant_frame",
			"value:set:shuvpi.result",
			"value:set:shuvpi.lane.state",
		]);
		expect(
			fixture.storage
				.getCommitAttempts()
