...
test("crash in generation@requesting → shuvpi.usage{interrupted}, retry, complete", async () => {
	const gate = new Gate();
	...
	await untilPhase(env, "shuvpi.generation", "requesting");
...
test("crash in generation@prepared (blocked in beforeRequest) → hooks rerun, no duplicate system entry", async () => {
	...
	await untilPhase(env, "shuvpi.generation", "prepared");
	await gate.arrivals(1);
	await env.crash();
	gate.open();
	...
	assert.equal(entries.filter((e) => e.kind === "shuvpi.system").length, 1); // step 1 never reruns
	assert.equal(entries.filter((e) => e.kind === "shuvpi.usage").length, 0); // prepared is not an in-flight effect: no failed attempt recorded
...
test("crash in generation@retrying → sleeps out untilMs, then retries", async () => {
	...
	onTestFinished(() => env.close());
	const a = await env.root.send({ content: "A" }, ctx);
	const t = await untilPhase(env, "shuvpi.generation", "retrying");
...
test("crash in tool@started with replay:safe → tool re-invoked; unsafe → synthetic interrupted result", async () => {
	for (const replay of ["safe", "unsafe"] as const) {
		...
		await untilPhase(env, "shuvpi.tool", "started");
		await gate.arrivals(1);
		await env.crash();
		...
		assert.equal((await env.input(a.id))?.status, "done");
		const result = (await env.entries()).find((e) => e.kind === "shuvpi.tool_result")!;
		if (replay === "safe") {
...
test("crash in tool before `started` (blocked in beforeTool) → hook reruns, tool runs once", async () => {
	...
	onTestFinished(() => env.close());
	const a = await env.root.send({ content: "tool:x" }, ctx);
	await untilPhase(env, "shuvpi.tool", undefined);
	await gate.arrivals(1);
	await env.crash();
	gate.open();
	const t2 = tool("x");
...
test("crash while post_tools is running (blocked in afterTools) → reruns from initial, continuation runs once", async () => {
	...
	assert.deepEqual(
		atOpen
			.filter((t) => t.kind !== "shuvpi.generation")
			.map((t) => `${t.kind.replace("shuvpi.", "")}:${t.status}`)
			.sort(),
	...
	assert.equal((await env.input(a.id))?.status, "done");
	assert.equal(kinds(await env.entries()), "user system assistant tool_result assistant");
	const pt = (await env.tasks()).find((t) => t.kind === "shuvpi.post_tools")!;
	assert.equal((await env.tasks()).filter((t) => t.kind === "shuvpi.generation" && t.id > pt.id).length, 1); // exactly one continuation (the original generation is retained: no pruning)
});
...
test("crash in collapse@summarizing → counted as a failed attempt, retried, summary lands", async () => {
	...
	await untilPhase(env, "shuvpi.collapse", "summarizing");
	...
	const summary = (await env.entries()).find((e) => e.kind === "shuvpi.summary")!;
...
test("crash in job@spawning / @running → host.status decides; unknown+rerun restarts with the same key", async () => {
	...
	const running = await untilPhase(env, "shuvpi.job", "running");
...
test("job with rerun:false and unknown after crash → failed/interrupted", async () => {
	...
	await untilPhase(env, "shuvpi.job", "running");
	await env.crash();
...
test("reopen with a marked task runs abort, not run", async () => {
	...
	onTestFinished(() => env.close());
	const a = await env.root.send({ content: "A" }, ctx);
	const t = await untilPhase(env, "shuvpi.generation", "requesting");
	await gate.arrivals(1);
