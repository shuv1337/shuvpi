...
const shape = (v: ConversationView) =>
	v.entries
		.map((e) => `${e.id}:${e.kind.replace("shuvpi.", "")}${e.head !== undefined ? `>${e.head}` : ""}`)
		.join(" ");
...
test("failed document commits do not break queued-input abort or later task-only watch updates", async () => {
	...
	const generation = (await env.tasks()).find(
		(task) => task.kind === "shuvpi.generation" && task.status !== "terminal",
	)!;
...
test("watch capture: fork-inherited head target, older head inside the range, and capture equals incremental fold", async () => {
	...
	const h1 = parentEntries.find((e) => e.kind === "shuvpi.handoff")!;
...
test("applyEnvelope folds head truncation ops while preserving display-only and older in-range head entries", () => {
	const base: ConversationView = {
		conversation: { id: 1 },
		entries: [
			{ id: 1, conversationId: 1, kind: "shuvpi.user" },
			{ id: 2, conversationId: 1, kind: "shuvpi.summary", head: 2 },
			{ id: 3, conversationId: 1, kind: "shuvpi.assistant" },
		],
		config: {},
		inbox: [],
		tasks: {},
	...
	const display = { id: 4, conversationId: 1, kind: "shuvpi.assistant", data: { reason: "aborted" } };
	const handoff = { id: 5, conversationId: 1, kind: "shuvpi.handoff", head: 3 };
	...
	const summary = { id: 6, conversationId: 1, kind: "shuvpi.summary", head: 4 };
