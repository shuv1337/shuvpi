...
test("registry: a plugin kind named shuvpi.* is rejected by defineTask and by open; a duplicate name is rejected; a forged core flag confers nothing", async () => {
	assert.throws(
		() =>
			defineTask({
	...
	const forged = { ...kinds.job, name: "shuvpi.generation", core: true };
...
test("registry: duplicate tool/section/entry names reject; unregister removes only the exact object registered", async () => {
	...
	assert.throws(() => env.h.registerEntryKind(defineEntry("shuvpi.mine")), /reserved/);
});
...
test("malicious casts: every core-only operation rejects at runtime for an ordinary task", async () => {
	...
	const probe = probeKind(
		"probe",
		{
			...
			createCoreByName: (tx) =>
				tx.createTask({ kind: "shuvpi.generation", conversationId: 1, input: { inputs: [] } }),
			...
			writeReservedKind: (tx) => tx.write(1, { kind: "shuvpi.assistant", model: [] }),
			otherSlot: (tx) => {
...
test("an ordinary runtime cannot abort a task in an unrelated conversation", async () => {
	...
	await gate.arrivals(1);
	target = (await env.tasks()).find((task) => task.kind === "shuvpi.generation" && task.status !== "terminal")!.id;
...
test("ToolDeclaration receives typed Static args; TypeBox 1.x validation rejects bad calls with a synthetic result", async () => {
	...
	assert.match(
		JSON.stringify((await env.entries()).filter((e) => e.kind === "shuvpi.tool_result").pop()!.model),
		/n=42 tags=1/,
	);
	await (await env.root.send({ content: "bad" }, ctx)).wait(ctx);
	const bad = (await env.entries()).filter((e) => e.kind === "shuvpi.tool_result").pop()!;
	...
	const tasks = (await env.tasks()).filter((x) => x.kind === "shuvpi.tool");
