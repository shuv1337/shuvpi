...
test("active transcript capture preserves inherited head targets, inner heads, display-only entries, and edits verbatim", async () => {
	...
	await storage.commit(
		[
			...
			{ type: "entry", entry: { id: 3, conversationId: 1, kind: "shuvpi.summary", head: 2 } },
			{
				type: "entry",
				entry: { id: 4, conversationId: 1, kind: "shuvpi.assistant", data: { reason: "aborted" } },
			},
			{
				type: "entry",
			...
			{ type: "entry", entry: { id: 11, conversationId: 10, kind: "shuvpi.handoff", head: 2 } },
		],
		ctx,
	);
	const harness = await Harness.open(storage, { models: fake({ respond: echoScript }) }, ctx);
...
test("active transcript capture has no fixed page cap and crosses the head boundary on an older page", async () => {
	...
	writes.push({ type: "entry", entry: { id: 603, conversationId: 1, kind: "shuvpi.summary", head: 100 } });
