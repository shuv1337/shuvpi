...
describe("bound value addresses", () => {
	it("binds immutable scalar and list addresses with validated components", () => {
		...
		expect(value<unknown>("shuvpi.application")).toEqual({ kind: "value", namespace: "shuvpi.application", key: "" });
	});
...
describe("built-in durable addresses", () => {
	it("uses the exact reserved namespaces, keys, kinds, and value types", () => {
		...
		expect([
			...
			{ kind: "value", namespace: "shuvpi.branch.tip", key: "review" },
			{ kind: "value", namespace: "shuvpi.lane.config", key: "review" },
			{ kind: "value", namespace: "shuvpi.lane.state", key: "review" },
			{ kind: "value", namespace: "shuvpi.result", key: "operation" },
			{ kind: "value", namespace: "shuvpi.op.meta", key: "operation" },
			{ kind: "value", namespace: "shuvpi.op.state", key: "operation" },
			{ kind: "value", namespace: "shuvpi.op.tool_args", key: "operation:step:2" },
			{ kind: "value", namespace: "shuvpi.op.tool_memo", key: "operation:invocation:name" },
			{ kind: "value", namespace: "shuvpi.op.preparation", key: "operation:task" },
			{ kind: "value", namespace: "shuvpi.pending.entry", key: "entry" },
			{ kind: "value", namespace: "shuvpi.pending.tool_output", key: "operation:invocation" },
			{ kind: "list", namespace: "shuvpi.pending.assistant_frame", key: "operation:response" },
			{ kind: "value", namespace: "shuvpi.session.name", key: "" },
			{ kind: "value", namespace: "shuvpi.entry.label", key: "entry" },
		]);
	});
	...
	it("exports exactly the five documented scan-prefix constructors", () => {
		...
		expect(prefixes).toEqual([
			{ kind: "value", namespace: "shuvpi.branch.tip", key: "" },
			{ kind: "value", namespace: "shuvpi.op.tool_args", key: "operation:" },
			{ kind: "value", namespace: "shuvpi.op.tool_memo", key: "operation:invocation:" },
			{ kind: "value", namespace: "shuvpi.op.preparation", key: "operation:" },
			{ kind: "value", namespace: "shuvpi.pending.tool_output", key: "operation:" },
		]);
