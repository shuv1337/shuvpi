...
# Shuvpi Observability Design Notes

## Goal

Make `packages/ai` and `packages/agent`/harness observable without depending on OpenTelemetry, Sentry, or any APM vendor.

Shuvpi should emit stable, structured lifecycle events. External listeners can convert those events into OTel spans, Sentry spans, logs, metrics, or custom telemetry.
...
```text
traceId=t1 spanId=s1 parent=-  name=shuvpi.agent.prompt
traceId=t1 spanId=s2 parent=s1 name=shuvpi.agent.turn
traceId=t1 spanId=s3 parent=s2 name=shuvpi.ai.provider.request
traceId=t1 spanId=s4 parent=s2 name=shuvpi.agent.tool_call
traceId=t1 spanId=s5 parent=s4 name=shuvpi.session.append_entry
```
...
```ts
await Promise.all([
  runWithShuvpiContext({ userId: "alice" }, () => harness.prompt("A")),
  runWithShuvpiContext({ userId: "bob" }, () => harness.prompt("B")),
]);
```

Deep code can then read the correct current context for the active async chain.

Shuvpi must run in Node, Bun, browser, workers, and other JS runtimes, so ALS cannot be the core abstraction. It should be a runtime adapter.

## Core design

Shuvpi owns a small runtime-agnostic observability abstraction:

```ts
export interface ShuvpiObservabilityContext {
  traceId?: string;
  currentSpanId?: string;
  userContext?: Record<string, unknown>;
}

export interface ShuvpiObservabilityEvent {
...
export interface ShuvpiObservability {
  getContext(): ShuvpiObservabilityContext | undefined;
  runWithContext<T>(context: ShuvpiObservabilityContext, fn: () => T): T;
  emit(event: ShuvpiObservabilityEvent): void;
...
```ts
export function configureShuvpiObservability(observability: ShuvpiObservability): void;
export function subscribeShuvpiObservability(listener: (event: ShuvpiObservabilityEvent) => void): () => void;
export function runWithShuvpiContext<T>(userContext: Record<string, unknown>, fn: () => T): T;
...
channel("shuvpi.observability").publish(event);
```

Subscribers can create OTel/Sentry spans without monkey-patching shuvpi.

## What shuvpi emits

Shuvpi emits what happened. It does not create OTel/Sentry spans directly.
...
shuvpi.agent.prompt
shuvpi.agent.skill
shuvpi.agent.prompt_template
shuvpi.agent.compaction
shuvpi.agent.branch_navigation
shuvpi.agent.session.append_entry
shuvpi.ai.provider.request
```
...
```text
shuvpi.agent.turn
shuvpi.agent.tool_call
shuvpi.agent.queue_update
shuvpi.ai.provider.retry
shuvpi.ai.provider.first_token
shuvpi.ai.provider.usage
shuvpi.session.read
...
Observability must never affect shuvpi execution.
...
{
  type: "start",
  name: "shuvpi.ai.provider.request",
  traceId: "t1",
  spanId: "s3",
  parentSpanId: "s1",
  context: {
...
```text
packages/ai
  emits shuvpi.ai.* events

packages/agent
  emits shuvpi.agent.* / shuvpi.session.* events
```
...
packages/otel
  subscribes to shuvpi events and creates OpenTelemetry spans
```

## Thesis

Shuvpi defines a stable, safe event contract. Adapters define where events go.
