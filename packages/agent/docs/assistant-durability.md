...
- `AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` from `@shuv1337/shuvpi-ai`;
...
4. Reuse shuvpi-ai's canonical frame conversion and reduction semantics.
...
Create one shuvpi-ai encoder per provider stream and feed it every event in order:
...
Do not define a second harness frame codec or reducer. Persistence stores the exported shuvpi-ai value directly; hydration calls `reduceAssistantMessageFrames()`.
...
Idle forks never copy lists in the `shuvpi.pending.assistant_frame` address family. Precise rewrites and migrations page frame lists and preserve element sequences when retaining them.
...
3. Every stored element is an exported shuvpi-ai `AssistantMessageFrame`.
