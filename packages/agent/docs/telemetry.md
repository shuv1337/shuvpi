...
This work must reuse `@shuv1337/shuvpi-telemetry`. It must not introduce another span abstraction.
...
Current propagation preserves context through callbacks and gives `before_tool` and `after_tool` handlers a child context derived from `shuvpi.harness.hook`. Extending that span behavior to every hook type remains work; handlers without an installed hook span currently receive the operation context directly.

Within the harness process, events preserve the context that caused each event, and buffered event watchers store `{ event, context }` rather than only `event`. Starting `shuvpi.harness.event_handler` from that event context and passing its child context to each listener remains work. Event registration itself is host-local configuration and has no operation parent.

Session mutation callbacks and commits receive the same explicit invocation context. Starting `shuvpi.session.write` from the committing invocation and passing its child context through the storage commit remains work.
...
`TODO_CONTEXT` remains a temporary migration marker, not a semantic root. Current uses cluster at unresolved transport and worker boundaries that cannot yet reconstruct a caller context, notably Shuvpi protocol request ingress and worker RPC ingress. `BACKGROUND_CONTEXT` means intentionally start without a caller.
