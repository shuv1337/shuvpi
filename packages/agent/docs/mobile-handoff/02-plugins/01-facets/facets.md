...
export const SessionStatusReporting =
  defineService<SessionStatusReporting>("shuvpi.session-status-reporting");

export interface SessionStatusView {
  readonly state: State<Record<string, SessionStatus>>;
}
export const SessionStatusView =
  defineService<SessionStatusView>("shuvpi.session-status");
```
...
| invocation | route |
| --- | --- |
| `shuvpi` (bare, in a directory) | ask server to create a session for cwd |
| `shuvpi --resume` | invoke the picker command at startup |
| `shuvpi --session <id>` | attach directly |
...
```ts
// contract.ts
export const TranscriptState = defineState<TranscriptTail>("shuvpi.transcript.tail");
```
...
```ts
export const Lane = defineKeyedService<LaneView>("shuvpi.lane");   // key = lane name

export const laneSession = defineFacet({
  id: "@shuvpi/lane:session",
  uses:     [Harness],
  provides: [Lane],
  observes: [],
...
export const BrowseState = defineState<BrowseView>("shuvpi.browse.view");
...
export const FileBrowsing = definePeerService<FileBrowsing>("shuvpi.file-browsing");
```
...
export const fileBrowsingServer = defineFacet({
  id: "@shuvpi/file-browsing:server",
...
export const ConnectedPeers =
  defineService<{ readonly state: State<Record<PeerId, Principal>> }>("shuvpi.peers");
```
...
const Tui = defineService<TuiHost>("shuvpi.local.tui", { rpc: false });
```
...
```text
fresh working copy
→ built-in providers      (@shuvpi/providers-builtin)
→ remote catalogue        (@shuvpi/providers-catalog)
→ models.json transform   (@shuvpi/providers-models-json)
→ auth/availability mark  (@shuvpi/auth)
→ validated state
```
...
```ts
import { object, array, string, int, oneOf, literal } from "@shuvpi/schema";
...
`@shuvpi/schema` re-exports TypeBox constructors as free functions so declarations stay
...
```
POST /v1/call/shuvpi.transcript/page
...
```
GET /v1/state/shuvpi.lane/main/snapshot
