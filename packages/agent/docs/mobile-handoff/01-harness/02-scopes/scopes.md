...
export const pendingToolOutput = (operationId: string, invocationId: string) =>
  list<WireOp[]>("shuvpi.pending.tool_output", `${operationId}:${invocationId}`, operationId);

export const pendingAssistantOutput = (operationId: string, entryId: string) =>
  list<WireOp[]>("shuvpi.pending.assistant_output", `${operationId}:${entryId}`, operationId);

export const operationToolMemo = (operationId: string, invocationId: string, name: string) =>
  value<JsonValue>("shuvpi.op.tool_memo", `${operationId}:${invocationId}:${name}`, operationId);

// unchanged — no scopeId, so Session by inference
export const laneStateValue = (lane: string) => value<DurableLaneState>("shuvpi.lane.state", lane);
```
...
Scoping everything under `shuvpi.op.*` on the grounds that it dies with the operation
...
| | addresses |
| --- | --- |
| **ephemeral (sidecar)** | `shuvpi.pending.tool_output`, `shuvpi.pending.assistant_output`, `shuvpi.op.tool_memo` |
| **session (main log)** | `shuvpi.lane.state`, `shuvpi.op.state`, `shuvpi.op.meta`, `shuvpi.result`, `shuvpi.pending.entry`, `shuvpi.branch.tip`, `shuvpi.op.tool_args`, `shuvpi.op.preparation` |
...
```jsonc
// today — 1250 bytes
{"kind":"value","op":"set","seq":7,"namespace":"shuvpi.op.state","key":"01a04cf6-…","value":{"at":"starting","control":{…},"settings":{…},"latestAssistantEntryId":null}}
{"kind":"value","op":"set","seq":8,"namespace":"shuvpi.lane.state","key":"main","value":{"currentOperationId":"01a04cf6-…",…}}
{"kind":"value","op":"set","seq":9,"namespace":"shuvpi.op.state","key":"01a04cf6-…","value":{"at":"checkpoint",…}}
{"kind":"value","op":"set","seq":12,"namespace":"shuvpi.op.state","key":"01a04cf6-…","value":{"at":"assistant.ready",…}}
```

```jsonc
// with both dictionaries — 547 bytes
["@",0,"shuvpi.op.state","01a04cf6-…"]
["v",0,7,[["r",{"at":"starting","control":{…},"settings":{…},"latestAssistantEntryId":null}]]]
["@",1,"shuvpi.lane.state","main"]
