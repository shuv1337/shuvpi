...
All other current built-ins remain session-scoped. In particular, `shuvpi.op.state`, `shuvpi.op.meta`, `shuvpi.op.tool_args`, `shuvpi.op.preparation`, and `shuvpi.pending.entry` remain in the main scope because their writes coordinate atomically with lane/operation state.
