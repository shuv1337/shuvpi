...
shuvpi already does wire-is-delta one layer down. `PiMessagesEvent` — the serialized form
a shuvpi-messages backend sends — has no `partial`:
...
`shuvpi-messages.ts` then rehydrates: it holds a local `partial`, mutates it per event
...
`message_update` from the shuvpi-ai event directly. They are out of scope here.
...
### 5.1 shuvpi-ai frames stay at the shuvpi-ai boundary; Chord ops cross replication boundaries

`AssistantMessageFrame` is shuvpi-ai's semantic delta vocabulary (`text_delta`, `text_end`, …). [Delta tracking §6](../01-delta/delta.md#6-there-is-no-frame-type) defines no second frame wrapper: in-process replication carries `Op[]`, and a wire adapter carries encoded `WireOp[]`. Shuvpi-ai frames stop at the fold; Chord ops cross the replication boundary.

`AssistantMessageFrame` is shuvpi-ai's own delta vocabulary and stays. What changes is
