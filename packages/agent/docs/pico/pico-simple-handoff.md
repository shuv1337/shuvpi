...
active turn graph: the live tasks among the fixed `shuvpi.generation`, `shuvpi.tool` and `shuvpi.post_tools` kinds. Other
...
```ts
import type { Context } from "@shuv1337/shuvpi-chord";
```
...
`ToolCall` are provider-neutral shuvpi-ai types. Current shuvpi-ai has no `SystemMessage`; typed `shuvpi.system`
...
Built-in kind strings are `shuvpi.user`, `shuvpi.assistant`, `shuvpi.tool_result`, `shuvpi.system`, `shuvpi.notice`,
`shuvpi.summary`, `shuvpi.handoff` and `shuvpi.reset`. Ready foundation witnesses are:
...
message. Reset has no model. `shuvpi.system` is reserved but has no ready typed model witness. Stored entry-kind
...
- Ordinary edits cannot target managed `shuvpi.system` entries. The system-preparation operation is the only
...
5. Provider/model normalization in shuvpi-ai.
...
Namespaces beginning `shuvpi.` are protected. Public/plugin constructors reject them unless created with an
...
The fixed `shuvpi.generation`, `shuvpi.tool`, `shuvpi.post_tools` and `shuvpi.collapse` definitions use `defineCoreTask`.
`shuvpi.job` uses ordinary `defineTask` unless a later concrete requirement proves it needs a narrow privileged
surface. Core definitions are installed internally and cannot be registered, replaced or removed. Ordinary
plugin definitions cannot use a protected `shuvpi.*` name and never receive transcript/admission authority.
...
`shuvpi.output` base append occur in the same `Write[]`.
...
- Several creations/terminalizations use the prospective post-commit live reference set. No ref may target a
  retired output.
- Every protected `list.append` to `defineList({type:"shared",id}, "shuvpi.output")` is an exact nonempty
...
The protected `shuvpi.inbox` address is a conversation-scoped sticky (`rewind:false`) list bound to exactly one
...
`shuvpi.generation`, `shuvpi.tool` or `shuvpi.post_tools`, regardless of foreground/background. The predicate is evaluated
...
Fresh built-in `shuvpi.tool` cleanup reads its durable child records. It marks each child task with
...
removed. Ordinary registration rejects every protected `shuvpi.*` task name.

Stable core names and roles are:

```text
generation  shuvpi.generation   defineCoreTask; turn task; transcript/admission authority
tool        shuvpi.tool         defineCoreTask; turn task; transcript/admission authority
postTools   shuvpi.post_tools   defineCoreTask; turn task; transcript/admission authority
collapse    shuvpi.collapse     defineCoreTask; not a turn task; transcript/admission authority
job         shuvpi.job          defineTask;     not a turn task; ordinary task authority
```
...
The fixed `shuvpi.tool` `CoreTaskKind` owns `CoreTaskRuntime.commit` and its transaction capabilities. An
ordinary `ToolDefinition` receives neither. Its eventual adapter is a small `shuvpi.tool`-mediated capability over the
...
- Its invocation ID is the stable `shuvpi.tool` task ID. Its output is the task's `TaskOutput`. Durable
...
The `shuvpi.tool` adapter, not ordinary tool code, resolves timeout/background publication. An ordinary tool may
return a child ID after local observation timeout, but the `shuvpi.tool` terminal closure rereads that child on
...
Pico sends shuvpi-ai `{messages}` only; top-level `systemPrompt` and `tools` are absent. A typed
...
- Pico declarations introduce and explicitly spell no `any`; imported shuvpi-ai types are exempt;
...
- fixed turn-task classification by the live `shuvpi.generation`/`shuvpi.tool`/`shuvpi.post_tools` set directly in each
...
- **Provider generation/system integration:** verify landed shuvpi-ai messages-only behavior and define complete
...
- **Typed system sections:** define the concrete draft/persistence API and preparation staleness retry code
  after shuvpi-ai verification; section 14.4 fixes required semantics.
...
boundaries include Chord and shuvpi-ai.

Use erasable TypeScript syntax. Pico declarations must not introduce or explicitly spell `any`; imported
shuvpi-ai declarations are exempt and their durable values receive strict-JSON boundary validation. Check
