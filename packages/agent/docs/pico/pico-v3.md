...
`handoff` stores its message and starts a context; managed `system` entries store the exact shuvpi-ai
`SystemMessage` prepared at their transcript position. They record canonical prepared instructions,
not proof that a provider received them. Shuvpi-ai owns provider translation and best-effort cache
...
assistant cut before all its results, request-local shuvpi-ai transformation supplies missing results;
...
Every accepted input has an `inputId`, the id of its `shuvpi.inbox` list element (§8.1). A caller may
also supply an opaque request id. Acceptance atomically stores a session value from that key to the
new identity:

```text
400  append shuvpi.inbox item                         inputId = 400
...
```ts
const sessionName = sessionValue<string>("shuvpi.session.name");
...
```ts
import { AssistantMessageFrameEncoder, type AssistantMessageFrame } from "@shuv1337/shuvpi-ai";
...
const generationKind = defineTaskKind<GenerationState>()({
  kind: "generation",
  config: {
    model:         conversationValue<ModelId>("shuvpi.model", { rewind: true }),
    thinking:      conversationValue<ThinkingLevel>("shuvpi.thinking", { rewind: true }),
    selectedTools: conversationValue<string[]>("shuvpi.tools.selected", { rewind: true }),
    profile:       conversationValue<string>("shuvpi.prompt.profile", { rewind: true }),
    budgetMs:      conversationValue<number>("shuvpi.tool.budget", { rewind: false }),
  },
  ...
});
...
shuvpi-ai may return `stopReason: "aborted"`. The kind handles its domain outcome normally; if a durable
...
import { createContextKey, withAbortSignal, withContextValue } from "@shuv1337/shuvpi-chord/context";
...
Queued input is a conversation-scoped sticky list, `shuvpi.inbox`. Its list element id is the stable
...
namespace `shuvpi.inputResult` and key `String(id)`, typed as `InputResult`. It is ordinary stored
...
Fold effective shuvpi-ai system-message tool fields from all projected messages, including declarations
...
`baseline:true` is pico metadata, not a magic reset understood by shuvpi-ai.
...
**Shuvpi-ai owns provider translation.** Pico passes only `{ messages }`, with neither top-level
`systemPrompt` nor `tools`, even when there are no selected tools. Both fields absent selects the
agreed shuvpi-ai system-message mode; explicit empty strings/arrays are not absence. Adapters translate
...
The shuvpi-ai `SystemMessage` shape comes from [PR #9116](https://github.com/earendil-works/pi/pull/9116);
...
therefore unknown to fresh abort cleanup. The record is a session list, `shuvpi.usage`, one element per
attempt (conversation, task, model, tokens, cost), appended in the settlement commit together with an update of the session
value `shuvpi.usage.totals`, so stats are a point read and never a fold. Tools and jobs append the same
...
<harness>
[warn] output truncated: 2,000 of 51,204 lines shown; full output at /tmp/shuvpi/out-4421.log
</harness>
```
...
interface SessionView {
  ...
  readonly values: ReadonlyMap<Address, JsonValue>;     // session-scoped: shuvpi.usage.totals, shuvpi.session.name, ...
  readonly faulted: boolean;
  readonly readAt: Id;
}
type SessionEvent =
