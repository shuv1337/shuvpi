...
Harden the implementation in `harness-v3.zip` without reverting its core design. Return a new standalone zip for review. Do not integrate it into the shuvpi repository yet; the accepted implementation will later be incorporated under `packages/agent/src/harness/pico3/`.
...
| §4.4 entries / §1 `shuvpi.*` | `shuvpi.*` entry names reject for non-core callers except the allow-listed `shuvpi.notice` (`plugins.md` §4.4) |
...
| §16 `shuvpi.plugin` handlers map | folded into custom task kinds; `shuvpi.plugin` may stay as a convenience kind but is not the plugin mechanism |
...
1. **Fixed core.** `shuvpi.generation`, `shuvpi.tool`, `shuvpi.post_tools`, `shuvpi.collapse`, and the Harness methods built on them are fixed and unreplaceable. `shuvpi.job` and `shuvpi.plugin` are built-ins too, though they use ordinary-task authority. Plugins cannot register any `shuvpi.*` task kind or replace a built-in.
...
- Reject plugin task names beginning with `shuvpi.`.
...
Tests: attempts to replace `shuvpi.generation`, forge core authority, use a stale token, and unsubscribe twice.
...
4. display-only entries, including aborted/error assistants, `shuvpi.usage`, and model-less plugin entries;
...
- `Stored<T> = JsonRepresentation<T>` for imported shuvpi-ai values;
...
- managed `shuvpi.system` baseline/delta chronology;
