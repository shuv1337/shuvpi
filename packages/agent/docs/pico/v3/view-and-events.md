...
interface ConversationView {
  ...
  /** Present while a shuvpi.collapse task is live. */
...
interface TurnView {
  ...
  generation?: GenerationStatus;        // present while a shuvpi.generation is live
...
interface ToolSlot {
  ...
  entry?: Id;                           // the shuvpi.tool_result entry once landed
}
...
- Older head entries inside the range stay (a renderer may mark them). Display-only
  entries (`shuvpi.assistant` with `stopReason` `error`/`aborted`, `shuvpi.usage`, model-less
...
frame to `turn.message` in place (same reducer as shuvpi-ai's frame reducer).
...
| moment | ops | events |
|---|---|---|
| `send`, idle | `entries +shuvpi.user`; `turn = { inputs, tools: [] }`; (older queued items placed first: `inbox` splice, `entries +…`) | `input.placed`, `entry.added`, `turn.started` |
...
| `prepared` commit | optional `entries +shuvpi.system`; `turn.tools = []`; `turn.message` deleted; `turn.generation = { stage: "requesting", attempt }` | `entry.added` (system) |
...
| retry decision | `entries +shuvpi.usage`; `turn.message` deleted; `turn.generation = { stage: "retrying", … }` | `entry.added`, `generation.retrying` |
| deferred | `turn.generation = { stage: "deferred", pollAt }` | `generation.deferred` |
| response with tool calls (closure) | `entries +shuvpi.assistant`; `turn.message` deleted; `turn.generation` deleted; `turn.tools = slots(status:"pending")`; (threshold: `compaction = {…}`) | `entry.added`, `generation.completed{toolCalls:n}`, (`compaction.started`). Slots are `pending`; `tool.started` fires at each tool's `started` checkpoint (§6.3) |
| final answer, no triggers, no continuation (closure) | `entries +shuvpi.assistant`; `turn` deleted; boundary placements (`inbox` splices, `entries +writes`) | `entry.added`, `generation.completed`, `turn.ended{done, answer}`, `input.placed`/`entry.added` for placed writes |
| final answer, `onYield` continuation | `entries +shuvpi.assistant`, `entries +shuvpi.user{continuation}`; `turn.generation = { stage: "preparing" }` | `entry.added`×2, `generation.completed`; **no** `turn.ended` (same group continues) |
| final answer with queued followUp/steer triggers | as "no triggers", then `entries +shuvpi.user`, `turn = { inputs: triggers, tools: [] }`, `turn.generation = { stage:"preparing" }` | `turn.ended{done}`, `input.placed`, `entry.added`, `turn.started` |
| terminal error / aborted-by-provider (closure) | `entries +shuvpi.assistant` (display-only; `model` absent, message in `data`); group resolved `unanswered/failed`; **final boundary runs**: placed writes, selected triggers; `turn` deleted, or replaced by a successor turn for triggers | `entry.added`, `generation.failed{provider|retries_exhausted, entry}`, `turn.ended{unanswered, failed}`, then `input.placed`/`entry.added`/`turn.started` as in "with triggers" |
| `no_model`, overflow with nothing collapsible | as above; overflow additionally `entries +shuvpi.notice` (placed immediately: the closing task counts as gone). `no_model` is not special-cased: a successor for queued triggers fails fast and resolves its own group | `generation.failed`, `turn.ended{unanswered, failed}`, `entry.added`, … |
...
| abort (fresh abort closure) | optional `entries +shuvpi.assistant` (display-only partial, only if content was streamed); `turn` deleted | (`entry.added`), `turn.ended{unanswered, aborted}` |
...
| result (closure) | `entries +shuvpi.tool_result`; `turn.tools[i].status = done|error`; `turn.tools[i].entry` | `entry.added`, `tool.finished{isError, control}`, (`warning` if truncated) |
...
| abort closure | `entries +shuvpi.tool_result(aborted)`; `turn.tools[i].status = "aborted"` | `entry.added`, `tool.aborted` |
...
| moment | ops | events |
|---|---|---|
| continuation (default) | boundary placements (`inbox` splices, `entries +shuvpi.user` for steer, `+writes`); `turn.inputs += triggers`; `turn.generation = { stage: "preparing" }` | `input.placed`…, `entry.added`…; no turn event |
| synthesized missing results (orphaned/aborted tool) | `entries +shuvpi.tool_result` per missing call | `entry.added`, `warning{source:"post_tools"}` |
...
| `handoff` | `entries` truncated + `+shuvpi.handoff` (head); `turn` deleted; placements; successor turn | `head.moved`, `entry.added`, `turn.ended{done}`, … |
...
| summary lands (closure) | `entries` truncated to `head` + `+shuvpi.summary`; `compaction` deleted | `head.moved`, `entry.added`, `compaction.finished{summary}` |
...
| job notice (`notify: true`) | `entries +shuvpi.notice` (immediately if idle, else via `inbox`) | `entry.added` or `input.queued{write}` |
...
| `ThreadItem::UserMessage` | `shuvpi.user` entry |
| `ThreadItem::AgentMessage` + `item/agentMessage/delta` | `turn.message` (streaming) → `shuvpi.assistant` entry; ops on `["turn","message",…]` |
| `ThreadItem::Reasoning` + deltas | thinking blocks inside `turn.message.content` / the assistant entry |
| `ThreadItem::CommandExecution { status, aggregatedOutput, exitCode }` + `item/commandExecution/outputDelta` | `turn.tools[i]` (`status`, `output`) → `shuvpi.tool_result` entry |
...
| `ThreadItem::ContextCompaction` | `compaction` (live) + `shuvpi.summary` entry + `compaction.finished` |
...
| `thread/tokenUsage/updated` | `usage` on the assistant entry; `shuvpi.usage` entries for failed attempts |
...
| `process/outputDelta`, `process/exited` | `tasks[id].status` for `shuvpi.job` |
