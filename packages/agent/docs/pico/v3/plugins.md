...
h.namespace<T extends NamespaceShape>(ns: string, defaults: T, opts?: { view?: (slice: T) => JsonValue }): Namespace<T>
                                                               // token with idempotent `unregister()`; unique, not `shuvpi.*`; no `view` ⇒ nothing in the view
...
defineEntry<E>(kind: string): EntryKind<E>                                      // stateless; throws on `shuvpi.*` (§4.4)
...
inside `shuvpi.tool`, whose `started{call}` checkpoint is the effect evidence.
...
Crash before the memo: `shuvpi.tool` recovers `started` (safe), re-invokes, no memo,
...
core-only. `defineEntry` throws on a `shuvpi.*` name and `c.write`/`tx.write` reject
`shuvpi.*` at runtime for non-core callers even after a cast, with one allow-listed
exception: `shuvpi.notice` (one user message, optional `data`, no `head`/`edits`),
...
Subagents (`api.conversation`) and processes (`shuvpi.job` via `api.task(kinds.job, …)`)
...
```ts
// session worker
export const Harness = defineService<HarnessHandle>("shuvpi.harness", { local: true });   // the in-process handle from §3; never remote
...
export const ApprovalDialogs = defineService<ApprovalDialog>("shuvpi.approval-dialog");   // keyed `${conversationId}:${callId}`
...
- Entries: `defineEntry("shuvpi.x")` throws; `write` of any `shuvpi.*` except `shuvpi.notice`
  rejects at runtime after a cast; `shuvpi.notice` with `head`/`edits` rejects.
