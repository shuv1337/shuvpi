...
- **Core kind**: one of Pico's four privileged built-in task kinds (`shuvpi.generation`, `shuvpi.tool`,
  `shuvpi.post_tools`, `shuvpi.collapse`). Core kinds have direct append and boundary authority; ordinary kinds have
  only the boundary-safe passive `write`.
- **Turn task**: a live `shuvpi.generation`, `shuvpi.tool` or `shuvpi.post_tools`. A conversation with a live turn task
...
- **Managed entry**: a `shuvpi.system` entry written by system preparation (section 12).
...
import type { JsonRepresentation } from "@shuv1337/shuvpi-chord";
type Stored<T> = JsonRepresentation<T>;   // the durable form of any imported shuvpi-ai type, including unions with primitive arms (UserInput)
```

shuvpi-ai's `AssistantMessage`, `ToolCall`, `Usage`, `DeferredHandle` and friends are interfaces: they have no
...
(`InputOutcome.answer.message`, hook payloads) hand out the shuvpi-ai type; only durable positions use `Stored<T>`.
Every durable position that carries a shuvpi-ai type uses `Stored<T>`: `Entry.model` and the entry witnesses
...
`Prep.retry`/`Base.retry`, and the `retry` config token. Public surfaces stay shuvpi-ai typed (`SendInput.content:
...
interface Entry {
  ...
  readonly model?: readonly Stored<Message>[];   // shuvpi-ai messages this entry contributes, in durable form
...
|---|---|---|---|
| `shuvpi.user` | `[UserMessage]` | | `{ continuation: true; from: Id }` on yield continuations only |
| `shuvpi.assistant` | `[AssistantMessage]` | | `AssistantEntryData` (attempt count; usage lives on the message) |
| `shuvpi.tool_result` | `[ToolResultMessage]` | | `ToolResultData` |
| `shuvpi.system` | `[SystemMessage]` or `[]` | | `SystemEntryData` (section 12) |
| `shuvpi.notice` | `[UserMessage]` | | optional |
| `shuvpi.usage` | `[]` | | `UsageEntryData = { attempt: number; usage?: Stored<Usage>; error: string }` (failed generation attempt) |
| `shuvpi.summary` | `[UserMessage]` | yes | `{ through: Id }` |
| `shuvpi.handoff` | `[UserMessage]` | yes | |
| `shuvpi.reset` | `[]` | yes | |
...
- Edits target earlier visible entries. Edits never target managed `shuvpi.system` entries, except the omission
...
// external constructors reject every `shuvpi.*` namespace at runtime
...
The namespace prefix `shuvpi.` is reserved for built-ins: external constructors reject it, so a plugin cannot
define `shuvpi.anything`. Within it there are exactly two sets: (1) the fixed kinds' config bundle definitions
...
they already have their own field and event. (2) The internals `shuvpi.inbox`, `shuvpi.input_result`, `shuvpi.request`,
and `shuvpi.output` are protected: generic read/write/watch/`rootValues` reject them and they surface only through
`view.inbox`, `InputHandle` and `view.taskOutputs`. Plugin and
application definitions (any non-`shuvpi.` namespace) are public and appear in a watch only when requested. There
...
```ts
// the bundles the fixed kinds attach as `<kind>.config` (definition objects; the internal constructor is allowed to use shuvpi.* names)
const generationConfig = {
  model:         conversationValue<ModelRef>("shuvpi.model", { rewind: true }),                    // no default -> no_model
  thinkingLevel: conversationValue<ThinkingLevel>("shuvpi.thinking", { rewind: true, default: "off" }),
  selectedTools: conversationValue<readonly string[]>("shuvpi.tools.selected", { rewind: true, default: [] }),
  profile:       conversationValue<string>("shuvpi.prompt.profile", { rewind: true, default: "default" }),
  retry:         conversationValue<Stored<RetryPolicy>>("shuvpi.generation.retry", { rewind: false,
                   default: { enabled: true, maxRetries: 3, baseDelayMs: 2000, maxAgentDelayMs: 60_000 } }),   // shuvpi-ai RetryPolicy; coding-agent defaults
} satisfies ConfigBundle;
const postToolsConfig = {
  steeringMode:  conversationValue<"all" | "one-at-a-time">("shuvpi.queue.steering", { rewind: false, default: "one-at-a-time" }),
  followUpMode:  conversationValue<"all" | "one-at-a-time">("shuvpi.queue.follow_up", { rewind: false, default: "one-at-a-time" }),
} satisfies ConfigBundle;
...
const collapseConfig = {
  threshold:     conversationValue<number>("shuvpi.collapse.threshold", { rewind: true, default: 0 }),   // context tokens; 0 = never
  keepRecent:    conversationValue<number>("shuvpi.collapse.keep_recent", { rewind: true, default: 20_000 }),   // tokens retained after `through`
} satisfies ConfigBundle;
...
delta to the protected shared list built by Pico's internal address constructor (`shuvpi.output`).
...
- Runtime facade: `output.read(ctx)`, `output.mutate(state => { ... }, ctx)`, `output.replace(value, ctx)`.
  ...
  flush creates a `shuvpi.output` list element, consumes a `Seq` and emits a `task_output` event; a `mutate` whose
...
| address scope: conversation address must be in writer's subtree; task address must be writer's own; protected `shuvpi.*` internals rejected for non-internal writers | capability |
...
- One `shuvpi.output` append per tracker flush, with provenance from a live task referencing that output.
...
- cancellation reach: conversation abort and `shuvpi.tool` abort follow `owns`;
...
task; the only exception is the `shuvpi.tool` descendant resolver of section 11.1 for later sibling tool calls.
...
checks `shuvpi.request` on the line before minting anything; if a receipt for that key exists (of either kind),
...
protected namespace. Preparation applies it while this conversation has no *local* managed `shuvpi.system` entry
...
Inheriting the parent's `shuvpi.system` entries and then sending without that delta would present the main agent's
...
`answer.message` is `answer.entry.model[0]` handed out as the shuvpi-ai `AssistantMessage` type (the stored
...
- `shuvpi.inbox`: sticky conversation list of `QueuedInput`. The element ID is the input ID; ascending IDs are
  admission order. Not inherited by forks. Reads return one complete current collection; no backend replays
  history per read.
- `shuvpi.input_result`: sticky session value keyed by input ID → `StoredInputResult`.
- `shuvpi.request`: sticky session value keyed by `requestId` → `Acceptance` (tagged `kind`; `Harness.input` ignores `"write"`).
...
`busy(c)` = at least one live `shuvpi.generation`, `shuvpi.tool` or `shuvpi.post_tools` directly in conversation `c`,
foreground or background. Evaluated against committed live tasks plus the current batch's `Write[]`.
Ordinary tasks and `shuvpi.collapse` never make a conversation busy. Reason: collapse runs speculatively next
...
send when not busy:
  ...
  input as shuvpi.user entries; create one shuvpi.generation { inputs: [...placed ids] }; results -> placed;
...
- **post-tools** (in `shuvpi.post_tools` closure): selected = all queued writes + steer per `steeringMode`; place
...
- **final answer** (in `shuvpi.generation` closure when no tool calls): the closure has already appended the
...
- **yield continuation** (section 10.1): when an `onYield` hook asks to continue and no steer/followUp is
  ...
  exchange); appends a `shuvpi.user` entry `{ model: [{ role: "user", content: <continue text>, timestamp }], data:
...
is `{}`. shuvpi-ai's `event.partial` is one shared object the provider keeps mutating after `start`, so it must
never be inserted into or retained by the tracker. Generation runs every provider event through shuvpi-ai's
...
shuvpi-ai's frame reducer. It never buffers prior frames, never re-reduces the sequence, never keeps a second
`AssistantMessage`, and never `replace`s the whole message per token: one frame, one incremental `mutate`, one
Chord delta. shuvpi-ai's `reduceAssistantMessageFrames` is a test oracle only (the final tracker value must equal
...
1. Read `generationKind.config` at the current position (`model` absent → `failed/no_model`). Run system
   preparation (section 12), which may append a managed `shuvpi.system` entry and returns its ID. Commit
...
2. Derive the immutable request snapshot from the cutoff. Run `beforeRequest` hooks on a private copy
   ...
   *final* transformed request: (a) overflow check, `estimate(final) > model.contextWindow - model.maxTokens` →
   ...
   `shuvpi.tool`. Commit `requesting` with a fresh `requestKey`, that `tools`, and `attempt` (1 on the first pass;
...
3. Every terminal provider `AssistantMessage` (stop, length, toolUse, error, aborted, or the result of a
   ...
   `policy = checkpoint.retry`. Classify with shuvpi-ai's
   `isRetryableAssistantError(message)`; if retryable, `policy.enabled` and `attempt <= policy.maxRetries`:
   one commit buffers a data-only `shuvpi.usage` entry `{ attempt, usage, error }` (no `model`; records the failed
...
3b. Terminal non-retryable outcome: a provider `stopReason: "aborted"` without a Pico mark (the provider ended
   ...
   same path: the closure appends the error/aborted `AssistantMessage` once as a display-only `shuvpi.assistant`
...
4. Successful response (`stop`, `length`, `toolUse`): `afterResponse` has already run (step 3; the assistant
   *entry* does not exist yet, hooks never run inside or after the closure). For a final answer (no tool
   ...
   continuation boundary (section 9.5) with a `shuvpi.user` entry carrying the returned text; the successor
   generation keeps the same `inputs`. No `continue` → final-answer boundary.
5. Tool calls: closure buffers the assistant entry, one `shuvpi.tool` per call and one
   `shuvpi.post_tools{after: tools, inputs, assistant, tools}`. Every call gets a `shuvpi.tool`, including one naming a
   tool that was not offered; `shuvpi.tool` decides (10.2) using the generation's own terminal checkpoint as the
...
like step 3: if `retry.enabled` and `attempt <= retry.maxRetries`, one commit appends `shuvpi.usage{ attempt, error:
...
resume polling with the stored handle. Usage accounting: each failed attempt's usage is one `shuvpi.usage` entry committed with its
`retrying` checkpoint; the successful attempt's usage lives once, on the final `shuvpi.assistant` entry's
`AssistantMessage.usage`. Nothing is counted twice. Missing usage is unknown, not zero.

Size estimation: `estimate(messages) = estimateContextTokens(messages).tokens` from shuvpi-ai, applied to the
projected request messages (including the messages-only system/tool declarations once the shuvpi-ai PR lands).
...
snapshot, and always before `requesting` is committed or the provider is called (shuvpi-ai `Model` fields; no
separate cap in v1): compute `through` (below). If a collapsible prefix exists, the
closure creates a **foreground** `shuvpi.collapse C{reason: "overflow", through}` and a replacement
`shuvpi.generation G{after: [C], inputs}`, transfers input ownership to `G`, and terminalizes `failed/overflow`
without resolving inputs. `G` reads `C`'s outcome first: completed → normal execution against the new head;
failed/declined/aborted/orphaned → `G`'s closure `write`s a `shuvpi.notice` ("context too large; compaction
...
`shuvpi.post_tools` (tool calls); `no_model`, terminal provider error/abort, and interrupted `requesting` with no
...
**foreground** `shuvpi.collapse C{reason: "threshold", through}`; the generation completes normally. Automatic
...
checkpoint). The task must be a completed `shuvpi.generation` whose result names this assistant entry, and its last
...
`shuvpi.tool_result` `isError: true, "tool not offered"` without consulting the registry. Then look up the tool by
...
stale `Promise.race`); buffer exactly one `shuvpi.tool_result` entry whose `model` message is the bounded output
...
subagent queues. Closure appends an aborted `shuvpi.tool_result` through `CoreAbortTx.entry`.

### 10.3 `shuvpi.post_tools`
...
5. If `handoff`: append `shuvpi.handoff{ head: "self", model: [user handoff message] }` **directly** through
...
6. If `handoff` or `terminate`: resolve the active group `done` (answer: assistant) and run the final-answer
   ...
   fresh context. Otherwise: post-tools boundary and `shuvpi.generation{ inputs }` as the continuation with the group
   plus placed steer.
7. Terminal snapshot.
...
Abort: `shuvpi.post_tools` owns the active input group while it is live, so its fresh abort closure resolves every
...
(someone else collapsed). Else append `shuvpi.summary { data: {through}, model: [user summary], head: first entry
...
output into `output.mutate`. On exit: if `notify`, `tx.write` a `shuvpi.notice` for this occurrence in the same
...
The behaviour of the built-in `shuvpi.tool` kind (10.2) and of every tool *result* is normative. The exact
...
// what a tool produces (public, shuvpi-ai typed content)
...
Reason: a tool is plugin code; the transcript and the task are Pico's. The `shuvpi.tool` kind owns the tool's
...
withdrawn), how the tool reaches the `shuvpi.tool` task's `TaskOutput` for streaming progress, durable memos, the
...
- It is a small `shuvpi.tool`-mediated surface over the current invocation: the tool's identity is the stable
  `shuvpi.tool` task ID, its output is that task's `TaskOutput`, and durable strict-JSON memos are protected
  state scoped to that task, retired with it.
- A keyed child-task operation, if included, uses one internal `shuvpi.tool` transaction to create the child and
...
- A keyed subagent operation, if included, applies a `ConversationSeed` (section 8.1) in one internal
  `shuvpi.tool` transaction: `createConversation` (owner = the tool task), the seeded config/sticky/section
...
- Returned task handles expose wait/abort, never commit. A local wait timeout is observation only: it returns
  "timed out" without marking the child. The `shuvpi.tool` terminal closure rereads the child on the line: a
...
- `abortWithTool: false` means only that enclosing `shuvpi.tool` abort does not mark that child; it is not a
...
Until this decision is appended here, no ordinary tool can be executed: `shuvpi.tool` (10.2) is implemented and
tested against an internal test executor that returns `ToolResult`s, and every built-in tool (including `bash`
and `subagent`) waits for the API. WP10 delivers `shuvpi.tool` to that boundary and no further.
...
Pico sends shuvpi-ai `{ messages }` only. System instructions and the tool loadout are `SystemMessage`s inside
`messages` at their historical positions. shuvpi-ai owns native translation and best-effort cache preservation.
This depends on the shuvpi-ai system-message API (`SystemMessage` with `toolsAdded`/`toolsRemoved`, messages-only
...
```ts
// shuvpi-ai (PR #9116); reproduced here for reading only, imported in code. After the PR, shuvpi-ai's `Message` union includes it,
// which `Entry.model: readonly Stored<Message>[]` and `SystemEntry extends Entry` rely on: the WP1 compile matrix passes only
// against a shuvpi-ai that has landed it (a hard dependency, section 18).
...
its deltas. This uses the indexed `shuvpi.system` kind scan and never depends on model heads or edits; a
...
interface HarnessOpenOptions<R extends Partial<Settings> = {}> {
  readonly models: Models;                        // shuvpi-ai; generationKind.config.model resolves against it
  ...
  readonly processHost?: ProcessHost;             // section 10.5; absent -> every shuvpi.job fails { reason: "spawn", detail: "no process host" } at execute
}
type Report =
...
`reset` is a `write` of `shuvpi.reset` or `shuvpi.handoff{head: "self"}`: appended immediately when idle, queued when
...
`register` rejects an existing name and any `shuvpi.*` name; `replace` requires an existing ordinary name;
...
4a. Validate every live fixed built-in task (`shuvpi.generation`, `shuvpi.tool`, `shuvpi.post_tools`, `shuvpi.collapse` and
   the ordinary-authority but fixed `shuvpi.job`) against its fixed input/checkpoint schema. Any mismatch fails
...
interface ConversationWatchOptions extends WatchOptions {
  readonly values?: readonly (Value<JsonValue> | UnboundValue<JsonValue>)[];   // plugin/application values; `shuvpi.*` rejects (built-in config has view.config; internals are protected)
...
config and protected `shuvpi.*` reject). A later source-conversation write emits nothing for an existing fork.
...
never appear as `config` events. Initial state is read from `view.config`. Protected state (`shuvpi.inbox`,
`shuvpi.input_result`, `shuvpi.request`, `shuvpi.output`, scratch, conversation-record seed metadata) is never exposed through `config` or
...
the existing Chord facet split, already used by `packages/coding-agent/examples/plugins/shuvpi-example-plugin`
...
- Protected internals: external `conversationValue("shuvpi.inbox")` rejects at construction; a hand-built `shuvpi.inbox`
...
- No `processHost` → `shuvpi.job` fails `spawn` before any checkpoint.
...
- Yield continuation: same `inputs` carried, results stay `placed`; transcript order is assistant, then placed
  writes, then the `shuvpi.user` continuation; queued trigger arriving between hook and closure wins and the yield
...
- `shuvpi.tool` abort marks children with `abortWithTool: true` only; subagent queues preserved.
...
- Recover from each generation phase; failed attempts produce `shuvpi.usage` entries, the final attempt's usage is
  on `shuvpi.assistant` only; recovery from `requesting` with retries left records `shuvpi.usage{ error: "interrupted" }`
...
- Tool control: `terminate` ends the turn; `handoff` ends it and appends `shuvpi.handoff` *before* the final
...
- Lists: requested list captured with complete elements; append/remove/clear in one commit → one `list` event
  with ops in write order, folded before the listener; element IDs stable; `shuvpi.inbox` and built-in config
...
- Watch options: the same plugin address requested twice appears once in the view and yields one `value`
  event; a built-in config token or `shuvpi.inbox` in `values` rejects rather than deduplicating into `config`.
...
| 9 | system | sections, draft, managed entries, canonical fold, baseline/delta, staleness retry (section 12); depends on shuvpi-ai `SystemMessage` having landed |
| 10 | generation + tool + post_tools | 10.1–10.3, `ToolResult`/`ToolDeclaration`, hooks of 11.2; `shuvpi.tool` runs an internal test executor, no ordinary tool API; faux provider only. Uses shuvpi-ai's landed `SystemMessage`/messages-only adapters; no parallel representation |
...
- Location: `packages/agent/src/harness/pico/`. No imports from `harness/runtime`, `harness/session`,
  `agent-harness.ts` or earlier Pico prototypes. Chord and shuvpi-ai are allowed. Read reusable leaf code before
  copying, then own the copy.
- Erasable TypeScript only. No `any` in Pico declarations (imported shuvpi-ai types exempt).
...
`ToolCall`, `Tool`, `Models`, `Usage`, `DeferredHandle`, `RetryPolicy`, `isRetryableAssistantError`, `retryDelayMs`, `estimateContextTokens`, `AssistantMessageFrameEncoder` from shuvpi-ai (`reduceAssistantMessageFrames` only in tests, as the oracle); `JsonRepresentation` from Chord (as `Stored<T>`, section 1); `ThinkingLevel`
...
type UsageEntry = EntryBase & { readonly data: UsageEntryData };   // shuvpi.usage, data-only
...
| `shuvpi.job` ordinary "unless a requirement proves otherwise" | stays ordinary | its notice is a `write` from the terminal closure, which ordinary kinds now have |
...
| protected = four names (`shuvpi.inbox`, `shuvpi.input_result`, `shuvpi.request`, `shuvpi.output`) | unchanged; all `shuvpi.*` reserved for built-ins; config tokens exported; section seeds live on the `conversation.create` record, not an address | restore the v1 rule; no new protected address |
...
| `onYield` and `before_collapse` semantics in historical docs | specified in 10.1, 10.4, 9.5; continuation entry kind is `shuvpi.user` | decision, not inherited |
| `defineStateTask` in section 5.1 | section 16, package 15 | unchanged contract, moved and condensed |
| section 14.4 system sections + usage-guide contract | section 12, unchanged design | user decision: the settled design stands, shuvpi-ai PR will land |
