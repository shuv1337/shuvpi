...
transcript order, stored model arrays concatenated, then shuvpi-ai tool results ordered by call index.
...
reject; new attempts clear scratch; persisted assistant frames use the shuvpi-ai compact frame encoder,
...
removes unwanted remaining declarations; `baseline: true` is not a shuvpi-ai tool-map reset. Never emit
...
- Request-local overrides leave stored state unchanged, remain messages-only and determine actual
  offered-tool validation; adapter fixtures are gated on the shuvpi-ai prerequisite in group 20.
...
`shuvpi.inbox` as a conversation sticky list whose element id is `inputId` and whose value holds mode,
...
`report` and `usage`, the usage ledger (`shuvpi.usage` + totals).
...
## 20. Clients and shuvpi-ai integration prerequisite
...
Target: shuvpi-ai enters system-message mode when BOTH `context.systemPrompt` and `context.tools` are
...
belongs to shuvpi-ai: unsupported mid-conversation changes become user messages bracketed with `<system>`
...
- `@shuv1337/shuvpi-chord` Context types and `@shuv1337/shuvpi-chord/context` helpers (1, 6, 8, 19)
- `@shuv1337/shuvpi-chord/delta` (15, 16: preview/watch only; not storage)
- `@shuv1337/shuvpi-ai`: `faux` provider for tests, `utils/estimate` for thresholds; SystemMessage and
