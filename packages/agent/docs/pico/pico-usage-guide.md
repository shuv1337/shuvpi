...
Durable agent harness for shuvpi: one session file, any number of conversations, every piece of work
...
```typescript
import type { Call } from '@shuv1337/shuvpi-agent';
import { BACKGROUND_CONTEXT, withCancel } from '@shuv1337/shuvpi-chord/context';
...
```bash
npm install @shuv1337/shuvpi-agent
```

## Quick Start

```typescript
import { Harness, JsonlStorage, systemSections, type Call } from '@shuv1337/shuvpi-agent';
import { BACKGROUND_CONTEXT } from '@shuv1337/shuvpi-chord/context';
import { readTool, writeTool, bashTool } from '@shuv1337/shuvpi-agent/tools';
import { generationKind } from '@shuv1337/shuvpi-agent/kinds';
...
| option | what it is |
|---|---|
| `models` | a shuvpi-ai `Models` collection |
| `tools` | what the model may call, besides the built-in `subagent` and `job` tools; a `ToolRegistry` or an array |
| `kinds` | ordinary plugin entry and task kinds, added beside the fixed built-ins; protected `shuvpi.*` names reject |
...
// {
//   model:         conversationValue<ModelRef>('shuvpi.model', { rewind: true }),
...
Pico sends shuvpi-ai only `{ messages }`: no parallel top-level `systemPrompt` or `tools`. Managed system
entries contain the exact rendered shuvpi-ai messages, including complete tool additions/removals. Shuvpi-ai
...
The target shuvpi-ai system-message API and messages-only adapter behavior are integration prerequisites
...
const baseline: SystemEntry = {
  ...
  data: { baseline: true, sections: [
    { key: 'shuvpi.identity', action: 'set',
...
For unsupported provider/model combinations, shuvpi-ai translates system messages to `<system>`-bracketed
...
`baseline:true` is pico metadata, not a reset command understood by shuvpi-ai.
...
admission appends and removes that element in one commit. `shuvpi.inbox` is a conversation-scoped sticky,
...
const name     = sessionValue<string>('shuvpi.session.name');
...
```typescript
import { Type, type Tool } from '@shuv1337/shuvpi-agent';
...
commit authority. The built-in `shuvpi.tool` adapter may eventually expose narrow keyed memo, child-task and
...
```text
shuvpi.tool task T owns task output O
