...
A complete finalized result becomes durable immediately in `shuvpi.pending.entry`; the call becomes `outcome_ready`; placement happens later when every earlier source position is complete or ready.
...
The mutation verifies the same operation, turn, source position, and invocation remain `effect_pending`. It does not rewrite `shuvpi.op.state`. A late checkpoint after settlement returns without committing.
...
3. Every `outcome_ready` call has exactly one complete matching `shuvpi.pending.entry` value.
...
```text
operationToolMemo(operationId, invocationId, name) → value("shuvpi.op.tool_memo", ...)
pendingToolOutput(operationId, invocationId)      → value("shuvpi.pending.tool_output", ...)
pendingEntry(resultEntryId)                       → value("shuvpi.pending.entry", ...)
```
