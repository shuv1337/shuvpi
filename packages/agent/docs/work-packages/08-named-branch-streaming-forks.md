...
- `shuvpi.op.*`, `shuvpi.pending.*`, `shuvpi.result` → exclude, both scopes.
- `shuvpi.session.name` → copy.
- `shuvpi.entry.label` → copy iff the keyed entry is copied.
- `shuvpi.branch.tip`, `shuvpi.lane.config`, `shuvpi.lane.state` → structured lane actions; the scope-specific rules (branch keeps only the named lane and rewrites its tip; lane state is always replaced with fresh idle state; tree keeps all) live in one shared driver consumed by all backends.
- The exact namespace `shuvpi` and any other `shuvpi.*` namespace → the fork **fails**, but only when current surviving state exists at fork time (a current scalar row or a surviving list element). Historical writes later replaced or deleted are absent from current state on every backend and must not alone fail a JSONL fork — behavior is backend-equivalent. Introducing a new built-in namespace without declaring its fork semantics must break fork tests, not silently copy or drop state.
- Neither `shuvpi` nor `shuvpi.*` → application: copy on tree, exclude on branch. The only built-in list namespace (`shuvpi.pending.assistant_frame`) excludes; application lists follow the application rule.
...
- Namespace fork knowledge is repeated across `fork.ts`, `values.md` prose, and conformance assertions; nothing forces a new `shuvpi.*` namespace to declare fork semantics.
...
2. The closed classifier/driver of §1.2, exported from core and consumed by all three backends; unknown reserved namespaces (exact `shuvpi` or undeclared `shuvpi.*`) fail the fork only when current surviving state exists, identically on every backend.
...
- current surviving state in an unknown reserved namespace (exact `shuvpi` or an undeclared `shuvpi.*` scalar or surviving list element) fails the fork on every backend; the same namespace **set then deleted** before the fork fails no backend — including JSONL, where the dead history remains as physical lines and the disk fold must classify it as absent. Construct both cases via raw committed writes.
...
- copying usage rows, `shuvpi.result`, or any open-operation state under any option;
...
- one closed core classifier owns every namespace disposition; unknown reserved namespaces (exact `shuvpi` or undeclared `shuvpi.*`) fail forks exactly when current surviving state exists, equivalently on all backends;
