...
1. **Core and application namespaces.** Every core address uses the exact documented `shuvpi.*` namespace. Applications use their own non-reserved namespaces through the same public `value()` / `list()` constructors. `fact.custom` and its API are deleted, not renamed to a built-in custom namespace.
2. **Forks.** Generic forks copy only explicitly handled core addresses: Branch tip/lane configuration plus fresh lane state, session name, and labels whose targets copy. They copy no `shuvpi.op.*`, `shuvpi.pending.*`, list, ledger, or arbitrary application address. A later application feature must add an address-specific fork policy before relying on copied application state.
...
- no registry, catalog, privilege constructor, global value map, or runtime `shuvpi.*` gate.
