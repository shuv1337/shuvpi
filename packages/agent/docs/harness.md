...
**Storage** (Part 1) exposes atomic transactions and queries over three durable forms. `shuvpi.op.meta` is written once per operation; `shuvpi.op.state` is replaced after each transition with the complete current state; tool checkpoints of bounded progress are auxiliary and never prove effect completion. The terminal transaction deletes operation-owned values/lists and writes immutable `shuvpi.result/{operationId}`. No partial transaction is ever visible.
...
*Every payload is in an entry, a bound value/list, or the ledger; there is no third place.* An entry is the complete conversation record: placement and payload in one row. A `Value<T>` holds only its current value; a `ValueList<T>` holds immutable elements ordered by write sequence, deletable only whole. Complete content that durably exists before tree placement — queued input, deferred writes, finalized out-of-order tool results — waits in `shuvpi.pending.entry` and becomes an entry in the transaction that places it; tool progress may occupy `shuvpi.pending.tool_output` only while its effect is uncertain; streamed assistant frames occupy `shuvpi.pending.assistant_frame` only while their response is effect-pending (§3.7). Per-backend projections (branch index, search, stats) are rebuildable and carry no authority.
...
A user posts in a channel with 400 entries of history; the application creates a lane anchored at the channel's tip and calls `lane.prompt(...)`. The normative write order (each `TX[...]` is one atomic commit): acceptance is hook-free and starts no task or effect; the intent mints response/usage ids before anything is sent; streamed events append compact frames without blocking the stream (§3.7); settlement commits response, usage, next state, and frame-list deletion together; tool calls follow intent → effect → outcome settlement, materializing in assistant source order; the terminal transaction deletes operation values/lists and writes `shuvpi.result/O`:

```text
TX[ insert entry n1 (user msg), upsert shuvpi.branch.tip = n1,
    upsert shuvpi.op.meta/O, upsert shuvpi.op.state/O = starting,
    upsert shuvpi.lane.state = { currentOperationId: O } ]
… first drive owns real work; before_drive then before_run …
TX[ insert injected messages if any, upsert shuvpi.branch.tip when needed,
    upsert shuvpi.op.state/O = checkpoint need_assistant ]
TX[ upsert shuvpi.op.state/O = assistant ready (config snapshot) ]
TX[ upsert shuvpi.op.state/O = effect_pending (reserves response n2, usage u1) ]
… provider streams …                                  ← the uncertain window
TX[ append shuvpi.pending.assistant_frame/O:n2 += frame ]    ← zero or one per non-terminal
                                                        event, enqueued without awaiting
TX[ insert entry n2, insert usage u1, upsert shuvpi.branch.tip = n2,
    delete list shuvpi.pending.assistant_frame/O:n2,
    upsert shuvpi.op.state/O = tools (result id n3 reserved) ]
TX[ upsert shuvpi.op.tool_args/O:s1:0, upsert shuvpi.op.state/O = call 0 effect_pending ]
… tool runs; selected bounded updates may replace shuvpi.pending.tool_output/O:n3 …
TX[ upsert shuvpi.pending.entry/n3 = finalized tool result,
    delete shuvpi.pending.tool_output/O:n3, upsert shuvpi.op.state/O = call 0 outcome_ready ]
TX[ insert entry n3, delete shuvpi.pending.entry/n3, upsert shuvpi.branch.tip = n3,
    upsert shuvpi.op.state/O = checkpoint ]
… second turn: ready · intent · stream · settle (n4, u2) …
TX[ delete shuvpi.op.meta/O, shuvpi.op.state/O, shuvpi.op.tool_args/O:*,
    set shuvpi.result/O = { operationId: O, kind: "run", status: "completed",
                        fromTipId, tipId: n4, startedAt, endedAt },
    upsert shuvpi.lane.state = { currentOperationId: null,
...
```text
TX[ insert entry n2 (assistant, 2 calls), insert usage u1, upsert shuvpi.branch.tip = n2,
    upsert shuvpi.op.state/O = tools (result ids n3, n4 reserved) ]
TX[ upsert shuvpi.op.tool_args/O:s1:0, upsert shuvpi.op.state/O = call 0 effect_pending,
                                                    replay: "never" ]
… tool deletes files; live updates u1 … u19 …
TX[ upsert shuvpi.pending.tool_output/O:n3 = bounded update u1 ]
… live updates u2 … u19 …  ← CRASH
```

On restart, `shuvpi.op.state` says `calls[0].status = "effect_pending", replay = "never"`, so the deletion is not re-run. A later drive reconciles the orphan per §4.5: latest durable checkpoint content plus an explicit interruption warning, staged as a synthetic error under the reserved id, then materialized normally:

```text
TX[ upsert shuvpi.pending.entry/n3 = synthetic interrupted result containing u1,
    delete shuvpi.pending.tool_output/O:n3, upsert shuvpi.op.state/O = call 0 outcome_ready ]
TX[ insert entry n3, delete shuvpi.pending.entry/n3, upsert shuvpi.branch.tip = n3,
    upsert shuvpi.op.state/O = call 0 completed ]
```
...
- Namespace `shuvpi` and every `shuvpi.*` namespace are reserved for built-ins by contract; every built-in namespace starts with `shuvpi.`. Application use of `shuvpi.*` is a trusted-programming defect; constructors perform no ownership check — exact constructor tests, not runtime privilege checks, enforce the convention.
...
| ----------------------------------------------- | ----- | -------------------------------------------------------- | -------------------------------- | ------------------------------------ |
| `branchTip(lane)`                               | value | `shuvpi.branch.tip`, lane                                    | entry id or `null`               | where this lane appends next         |
| `laneConfig(lane)`                              | value | `shuvpi.lane.config`, lane                                   | `LaneConfiguration`              | total lane configuration             |
| `laneState(lane)`                               | value | `shuvpi.lane.state`, lane                                    | `LaneState` (§3.3)               | current/last operation ids and inbox |
| `operationResult(opId)`                         | value | `shuvpi.result`, operation id                                | `OperationResultRecord` (§3.13)  | immutable terminal observation      |
| `operationMeta(opId)`                           | value | `shuvpi.op.meta`, operation id                               | `OperationMeta` (§3.1)           | acceptance data; written once        |
| `operationState(opId)`                          | value | `shuvpi.op.state`, operation id                              | `OperationState` (§3.2)          | total durable restart point          |
| `operationToolArgs(opId, stepId, sourceIndex)`  | value | `shuvpi.op.tool_args`, `{opId}:{stepId}:{sourceIndex}`       | effective arguments              | written once at clearance            |
| `operationToolMemo(opId, invocationId, name)`   | value | `shuvpi.op.tool_memo`, `{opId}:{invocationId}:{name}`        | `JsonValue`                      | invocation-scoped durable memo       |
| `operationPreparation(opId, taskId)`            | value | `shuvpi.op.preparation`, `{opId}:{taskId}`                   | `DurableStructuralPreparation`   | structural preparation               |
| `pendingEntry(entryId)`                         | value | `shuvpi.pending.entry`, reserved entry id                    | `PendingEntry`                   | complete content awaiting placement  |
| `pendingToolOutput(opId, invocationId)`         | value | `shuvpi.pending.tool_output`, `{opId}:{invocationId}`        | `AgentToolResult<unknown>`       | latest bounded progress checkpoint   |
| `pendingAssistantFrames(opId, responseEntryId)` | list  | `shuvpi.pending.assistant_frame`, `{opId}:{responseEntryId}` | `AssistantMessageFrame` elements | committed stream-frame prefix        |
| `sessionName`                                   | value | `shuvpi.session.name`, empty key                             | string                           | session name                         |
| `entryLabel(entryId)`                           | value | `shuvpi.entry.label`, entry id                               | string                           | entry label                          |
...
|---|---|---|
| `branchTipInventoryPrefix()` | `shuvpi.branch.tip` | `""` (all lanes) |
| `operationToolArgsPrefix(opId, stepId?)` | `shuvpi.op.tool_args` | `{opId}:` or `{opId}:{stepId}:` |
| `operationToolMemoPrefix(opId, invocationId?)` | `shuvpi.op.tool_memo` | `{opId}:` or `{opId}:{invocationId}:` |
| `operationPreparationPrefix(opId)` | `shuvpi.op.preparation` | `{opId}:` |
| `pendingToolOutputPrefix(opId)` | `shuvpi.pending.tool_output` | `{opId}:` |
...
```text
shuvpi.lane.*  shuvpi.session.*  shuvpi.entry.*   session-lived semantic values
shuvpi.result                             immutable lane-lived records, one per terminal operation
shuvpi.op.*                               operation-lived; deleted no later than the terminal transaction (§3.13)
shuvpi.pending.entry                      until placement, cancellation, or owning-operation cleanup
shuvpi.pending.tool_output                only while its invocation is effect-pending
shuvpi.pending.assistant_frame            only while its response is effect-pending
```

- `shuvpi.op.meta` and `shuvpi.op.preparation` are written exactly once; `shuvpi.op.tool_args` once per call. Invocation memos die when the invocation reaches `outcome_ready`. Every `shuvpi.op.*` value is deleted no later than the terminal transaction.
...
- `shuvpi.result` records are written once by terminal transactions, never updated or deleted by the runtime, and never read by recovery.
...
**Born placed** — assistant responses and direct appends to an idle lane; content and placement arrive in one transaction (`TX[ insert entry, upsert shuvpi.branch.tip ]`).
...
```text
t0  TX[ upsert shuvpi.pending.entry/e_q1 = { type: "message", payload: <200KB message> },
        S(next){ ...inbox.steer += "e_q1" } ]
t1  TX[ insert e_q1 (parent e_a3), delete shuvpi.pending.entry/e_q1,
        upsert shuvpi.branch.tip/main = "e_q1", S(next){ ...inbox.steer -= "e_q1" } ]
```
...
**Content first — finalized parallel tool outcomes.** A tool result id begins as a plain reserved string in `shuvpi.op.state`. When execution and `after_tool` finish, the complete final `ToolResultMessage` is staged in `pendingEntry(resultEntryId)` and the call becomes `outcome_ready`; it enters the tree only when every earlier source position is ready (`t0`: stage + `outcome_ready`; `t1`: insert after the earlier result + delete pending + `completed`). Effects settle in completion order while entries materialize in assistant source order. Crash before `t0`: uncertain effect; after `t0`: never re-executed; after `t1`: immutable entry.
...
Consequences: a queued or outcome-ready item is invisible to tree queries but visible through its owning state and `pendingEntry(id)`; queue placement/cancellation and outcome-ready materialization delete `shuvpi.pending.entry` atomically with their state change; a reserved tool-result id moves through `string only → shuvpi.pending.entry → immutable entry` with no two representations coexisting at a commit boundary; queued input pays the deliberate double-write (§1.8), and finalized tool outcomes stage once before placement when source ordering requires it — the extra write that prevents completed parallel effects from replaying after a crash.

## 2.3 Branches and AgentLanes

A `Branch` is data for one named path through the tree; it exists exactly when its tip value `shuvpi.branch.tip/{name}` exists (entry id or `null`). A Branch owns only its tip, branch-relative queries, and direct append — a raw append always inserts at the current tip and moves the tip in one Session mutation. It has no model, queues, operation state, hooks, or execution policy.

A configured `AgentLane` is a Branch plus total agent state: `shuvpi.lane.config/{name}` (`LaneConfiguration = { model: { provider, modelId }, thinkingLevel, activeToolNames }`), `shuvpi.lane.state/{name}` (`LaneState`, §3.3), and one `shuvpi.result/{operationId}` per terminal operation.
...
**Both scopes** copy the session name and labels only for copied entries. They exclude the usage ledger, `shuvpi.result`, every `shuvpi.op.*`, and every `shuvpi.pending.*` value/list, including pending entries, tool checkpoints, and assistant frames. Destination usage starts at zero and `messageCount` counts copied message entries. Copied entries retain ids.
...
One closed core policy classifies every namespace. Session name copies; labels depend on copied-entry membership; branch/lane values are reconstructed coherently; operation, pending, and result namespaces exclude; application namespaces follow scope. Exact namespace `shuvpi` and every otherwise-undeclared `shuvpi.*` namespace reject only when current surviving scalar or list state exists. Replaced or deleted history is absent and cannot by itself reject a fork.
...
|---|---|---|---|
| call _i_ `planned` | clearance passed (`before_tool`, lookup, arg validation) | `TX[ upsert shuvpi.op.tool_args/O:{stepId}:{i} = effective args, S(call i = effect_pending, replay) ]` | dispatch |
| call _i_ `effect_pending` | tool calls `onUpdate(partial, { checkpoint:true })` | `TX[ upsert shuvpi.pending.tool_output/O:{resultEntryId} = partial ]` after invocation fencing; state unchanged | `effect_pending` |
| call _i_ `effect_pending` | effect settled; latest update delivery and latest checkpoint write awaited; `after_tool` applied | `TX[ upsert shuvpi.pending.entry/{resultEntryId} = finalized result, delete shuvpi.pending.tool_output/O:{resultEntryId}, delete shuvpi.op.tool_memo/O:{resultEntryId}:*, S(call i = outcome_ready, terminate) ]`, with post-commit `tool_end` | `outcome_ready` |
| call _i_ `planned` | unknown tool / invalid args / `before_tool` blocks or throws / control cancelled | `TX[ upsert shuvpi.pending.entry/{resultEntryId} = complete synthetic result, S(call i = outcome_ready, terminate) ]`, with post-commit `tool_start` followed by `tool_end`; no effect intent | `outcome_ready` |
| source-ready prefix | first non-completed calls are `outcome_ready` | `TX[ insert result entries in source order, delete their shuvpi.pending.entry values, insert reported usage, upsert shuvpi.branch.tip, S(calls = completed / next checkpoint) ]` | `completed` or checkpoint |
...
Every terminal path performs one universal suffix in the same transaction as its final business writes: procedure-specific entries/usage/tip writes → delete all operation-owned `shuvpi.op.*` and pending progress/frame/outcome addresses → set `operationResult(operationId)` exactly once → set `laneState{ currentOperationId: null, lastOperationId: operationId, inbox: preservedCurrentInbox }`. This is the implementation's normative write order. The old §3.13 prose listed result before cleanup, while its worked trace and source used cleanup first; this resolves that contradiction in favor of source and the trace.
...
Queue application and final structural commits remain atomic (Part 3): a crash before one sees the prior complete state, after one the next. A crash after durable abort activates reconciliation; a crash after terminal cleanup sees an idle lane and its immutable `shuvpi.result`.
...
**T1 — declared, largely unimplemented.** `src/harness/telemetry.ts` and the generated `docs/telemetry-schema.md` declare the span vocabulary below, but production starts only `shuvpi.harness.hook`, and only for registered `before_tool`/`after_tool` handlers. AI options propagate `telemetryContext`, but no provider path starts `shuvpi.ai.request`, and no tool-effect span is emitted anywhere. Server request ingress has request-ID cancellation signaling but no trace carrier and no client/server RPC spans. T1 must first reconcile whether every declared span is wanted, then implement or remove; RPC trace propagation and an exporter are separate follow-ups. The declared spans:

```text
shuvpi.harness.run | compaction | navigation
shuvpi.harness.checkpoint | turn | step | tool | hook | sleep | event_handler
shuvpi.session.write
shuvpi.ai.request
```

Specified span semantics for the implementation T1 commissions: operation, step, tool, hook, event, and write parents follow the actual async procedure nesting; sleep spans permit run, compaction, navigation, turn, and checkpoint parents; `stepId`/`taskId` correlate retries and recovery. Every provider request/fetch/cancel uses `shuvpi.ai.request`; each real or safely replayed phase-two tool effect uses one tool span. Every storage transaction uses one `shuvpi.session.write` whose start attributes include `shuvpi.session.item_count` and `shuvpi.session.item_kinds` (`entry`, `usage`, `value`, `list`); list appends/deletes are never reported as value replacements; a calling procedure may supply its lane/operation ids and storage never infers them from payloads; end attributes include first and last committed sequence. Tool-checkpoint, invocation-memo, and assistant-frame commits are ordinary value/list writes under this span and emit no additional tool- or provider-effect span; address namespaces may be attributes, but snapshot and frame content never enters telemetry. No span is emitted when a mutation returns without committing; synthetic settlements and blocked/invalid tools emit no provider/tool-effect span.
...
**Migrations are total.** Value conversion is a field mapping; a state-machine shape change is more — an old `shuvpi.op.state` mid-phase may have no field-by-field equivalent in the new machine. A vN→vN+1 migration translates every stored value/list: lane/semantic values, `shuvpi.pending.entry`, optional `shuvpi.pending.tool_output`, invocation memos, and open operations' `shuvpi.op.meta`/`shuvpi.op.state` included (a migration adding `outcome_ready`, for example, must distinguish staged finalized tool results from still-uncertain effects). The author of a state-machine change writes the mapping for every reachable old state in the same change; a state with no natural successor maps to an explicit safe choice — no force-settle path or silent partial escape hatch. This is tractable because migration runs at open under exclusive host-assigned ownership over quiescent state: no task running, no effect in flight, every `shuvpi.op.state` exactly what some transaction committed — a pure function over a small, fully enumerable, fully typed set of values.
...
14. Acceptance must observe `currentOperationId === null`, commits no `Drive`, and returns before any hook/provider/tool/timer work begins. Run acceptance commits payload-free `starting`; only its consuming command may apply `before_run` output and replace it with `checkpoint`. A supplied operation id obeys §1.2 and is the exact id written to `shuvpi.op.meta`, events, and its eventual `shuvpi.result` record.
15. A reserved id may exist only with the content its intent named. Queued-content ids begin in `shuvpi.pending.entry`; settlement-family ids begin as strings in `shuvpi.op.state`. A tool-result id may then move through `string only → outcome-ready shuvpi.pending.entry → immutable entry`; no two representations coexist at a commit boundary (§2.2). An effect-pending response id may additionally key its auxiliary frame list (§3.7); frames are observation, not a content representation, and die with settlement.
16. Only terminal transitions construct `OperationResultRecord`. Exactly one immutable `shuvpi.result/{operationId}` is retained per terminal operation; older records remain readable after later operations, and recovery never reads any record.
...
29. Every outcome-ready call has exactly one matching finalized `shuvpi.pending.entry`, no immutable result entry, no invocation memos, and no tool-output checkpoint. Outcome-ready and completed calls never execute again.
...
|---|---|
| **Pending entry** | Complete unplaced content in `shuvpi.pending.entry` until placement/cancellation/cleanup (§2.2). |
| **Inbox** | Lane-owned globally ordered tagged queue (§3.11). |
| **Result record** | Immutable `shuvpi.result/{operationId}` terminal disposition (§3.13). |
...
| **Tool checkpoint** | Optional bounded complete live-update snapshot in `shuvpi.pending.tool_output`; auxiliary, never completion authority (§3.8). |
| **Assistant frame** | Compact replayable shuvpi-ai stream frame in `shuvpi.pending.assistant_frame`; auxiliary, never completion authority (§3.7). |
