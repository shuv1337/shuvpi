...
Assumed knowledge: shuvpi-ai (`Models`, `streamSimple`, tool definitions, thinking levels, deferred
...
AssistantMessageFrame, DeferredHandle, Usage and Models reuse shuvpi-ai; AgentMessage/ThinkingLevel and
...
// usage(task, conv, usage) appends to the main session list shuvpi.usage; no special storage verb
```
...
```ts
const model = conversationValue<ModelIdentity>("shuvpi.model");
const thinkingLevel = conversationValue<ThinkingLevel>("shuvpi.thinking");
const activeTools = conversationValue<string[]>("shuvpi.active-tools");
...
const entryLabel = (id: number) => sessionValue<string>("shuvpi.entry.label", String(id));
const requestInput = (requestId: string) => sessionValue<number>("shuvpi.request-input", requestId);
...
(scope, kind, namespace, key) tuples name the same address within the receiver's scope. `shuvpi` and
`shuvpi.*` are reserved for core. Constructing the same address with incompatible value types is a
...
const frames = list<AssistantMessageFrame>("shuvpi.pending.frames", "", work);
const memo = value<JsonValue>("shuvpi.pending.memo", "step", work);
...
Shuvpi/application concerns, not Chord concepts.
...
```ts
const model = conversationValue<ModelIdentity>("shuvpi.model");
const thinking = conversationValue<ThinkingLevel>("shuvpi.thinking");
const tools = conversationValue<string[]>("shuvpi.active-tools");
...
**Telemetry** uses shuvpi's callback `TelemetryContext` (typed schemas, no second contract). Spans and
