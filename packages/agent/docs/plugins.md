...
`@shuv1337/shuvpi-chord`. This document specifies how Shuvpi composes that runtime with Shuvpi-owned
...
→ @shuvpi/providers-builtin:session provides shuvpi.models/singleton

env.use(Models)
→ @shuvpi/model-selection:tui requires shuvpi.models/singleton

env.provideMany(QuestionDialogs)
→ @shuvpi/question:session provides shuvpi.question-dialog/keyed

env.observe(QuestionDialogs, handler)
→ @shuvpi/question:tui requires shuvpi.question-dialog/keyed
```
...
const Agent = defineService<AgentFacetScope>("shuvpi.local.agent", { local: true });
const Providers = defineService<ProviderContributionRegistry>("shuvpi.local.providers", { local: true });
const Tools = defineService<ToolContributionRegistry>("shuvpi.local.tools", { local: true });
```
...
const Tui = defineService<TuiHost>("shuvpi.local.tui", { local: true });
```
...
export function createAgentControllerRuntimeFacet(lane: AgentLane) {
	return defineFacet({
		id: "@shuvpi/agent-controller-runtime",
		setup(env) {
...
const Accounts = defineService<Accounts>("shuvpi.accounts");
```
...
```text
fresh ProviderDraft
→ built-in provider contribution        (@shuvpi/providers-builtin)
→ remote catalogue contribution         (@shuvpi/providers-catalog)
→ models.json transformation            (@shuvpi/providers-models-json)
→ authentication/availability marking   (@shuvpi/auth)
...
const Fleet = defineService<FleetFacetScope>("shuvpi.local.fleet", { local: true });
```
...
const QuestionDialogs = defineService<QuestionDialogs>("shuvpi.question-dialog");
```
		...
		tools.add((draft) => {
			draft.set("question", {
				...
				async execute(_toolCallId, params, _onUpdate, _toolContext, invocation, context) {
					...
					const memoName = "shuvpi.question.answer";
...
const DiffReviewManager = defineService<DiffReviewManager>("shuvpi.diff-review-manager");
const DiffReviews = defineService<DiffReviews>("shuvpi.diff-review");
```
