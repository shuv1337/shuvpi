...
boundary. Shuvpi owns the concrete wire envelope, routing, attachment state, and error adapters
...
const SessionDirectory = defineService<SessionDirectory>("shuvpi.session-directory");
const SessionManagement = defineService<SessionManagement>("shuvpi.session-management");
```
