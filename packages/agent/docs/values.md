...
await session.readList(workspaceEvents("shuvpi"), { limit: 100 }, context);
```
...
- namespace `shuvpi` and every `shuvpi.*` namespace are reserved for built-ins by contract;
...
export const branchTip = (lane: string) =>
  value<string | null>("shuvpi.branch.tip", lane);

export const operationState = (operationId: string) =>
  value<OperationState>("shuvpi.op.state", operationId);
...
export const pendingAssistantFrames = (
  ...
  "shuvpi.pending.assistant_frame",
...
export const laneConfig = (lane: string) =>
  value<LaneConfiguration>("shuvpi.lane.config", lane);
export const laneState = (lane: string) =>
  value<LaneState>("shuvpi.lane.state", lane);
export const operationResult = (operationId: string) =>
  value<OperationResultRecord>("shuvpi.result", operationId);

/** Used only by scanValues() to enumerate Branch names. */
export const branchTipInventoryPrefix = () =>
  value<string | null>("shuvpi.branch.tip");

export const operationMeta = (operationId: string) =>
  value<OperationMeta>("shuvpi.op.meta", operationId);
export const operationState = (operationId: string) =>
  value<OperationState>("shuvpi.op.state", operationId);
...
export const operationPreparationPrefix = (operationId: string) =>
  value<DurableStructuralPreparation>("shuvpi.op.preparation", `${operationId}:`);

export const pendingEntry = (entryId: string) =>
  value<PendingEntry>("shuvpi.pending.entry", entryId);
export const pendingToolOutput = (operationId: string, invocationId: string) =>
  value<AgentToolResult<unknown>>(
    "shuvpi.pending.tool_output",
...
export const pendingToolOutputPrefix = (operationId: string) =>
  value<AgentToolResult<unknown>>("shuvpi.pending.tool_output", `${operationId}:`);

export const sessionName = value<string>("shuvpi.session.name");
export const entryLabel = (entryId: string) => value<string>("shuvpi.entry.label", entryId);
```

`OperationMeta` is immutable acceptance metadata stored at `shuvpi.op.meta`. The process-local `Operation` projection is `{ meta: OperationMeta, state: OperationState }`, assembled from the separate metadata and state values; it is never stored at one address.
...
`AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` come from `@shuv1337/shuvpi-ai`. Do not define a second frame codec or reducer.
...
```jsonl
{"kind":"list","op":"append","seq":41,"namespace":"shuvpi.pending.assistant_frame","key":"O:R","value":{"type":"text_delta","contentIndex":0,"delta":"hi"}}
{"kind":"list","op":"delete","seq":52,"namespace":"shuvpi.pending.assistant_frame","key":"O:R"}
```
...
- operation-owned `shuvpi.op.*` scalar values are not copied into an idle fork;
- immutable `shuvpi.result` operation records are not copied by forks;
- `shuvpi.pending.entry`, `shuvpi.pending.tool_output`, and `shuvpi.pending.assistant_frame` values/lists are not copied;
...
3. Namespace `shuvpi` and every `shuvpi.*` are reserved by contract; every built-in namespace starts with `shuvpi.`, and application use is a trusted-programming defect.
...
- built-in address constructors produce exact `shuvpi.branch.tip`, `shuvpi.lane.*`, `shuvpi.op.*`, `shuvpi.pending.*`, `shuvpi.session.name`, and `shuvpi.entry.label` namespace/key/kind triples;
- every built-in namespace starts with `shuvpi.`, while application fixtures use non-reserved namespaces;
- `branchTipInventoryPrefix()` binds the empty-key `shuvpi.branch.tip` inventory prefix and is used only to enumerate Branches through `scanValues`;
