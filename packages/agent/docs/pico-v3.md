...
accept(input, requestId="client-42"), ONE command:
  read session value (shuvpi.request-input, "client-42")
  if present: resolve the existing acceptance
  otherwise:
    create accepted input and its required task/queue changes
    set session value (shuvpi.request-input, "client-42") = input ID
...
const frames = list<Frame>("shuvpi.pending.frames", { scope: work });
const checkpoint = value<Checkpoint>("shuvpi.pending.checkpoint", { scope: work });
```
...
provider model/usage types reuse the existing agent/shuvpi-ai boundary. Initialization is the typed
