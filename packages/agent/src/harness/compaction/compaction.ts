...
/**
 * Normalize a message timestamp to the epoch milliseconds an entry requires.
 *
 * Custom agent message types contributed through `CustomAgentMessages` may declare a string
 * timestamp, so fall back to the owning entry's timestamp when the value is not epoch millis.
 */
function toEntryTimestamp(timestamp: string | number, fallback: number): number {
...
export function prepareCompaction(
	...
	if (prevCompactionIndex >= 0) {
		...
		const virtualRetainedEntries: Entry[] = prevCompaction.retainedTail.map((message, index) => ({
			...
			// Custom agent messages may carry an ISO timestamp; entries always store epoch milliseconds.
			timestamp: toEntryTimestamp(message.timestamp, prevCompaction.timestamp),
			message,
		}));
