...
export class HookRegistry implements Hooks {
	...
	private invokeToolRegistration(
		...
		return startHarnessSpan(
			"shuvpi.harness.hook",
			{
				"shuvpi.lane.name": event.lane,
				"shuvpi.operation.id": event.runId,
				"shuvpi.hook.name": name,
				...(registration.id === undefined ? {} : { "shuvpi.hook.registration_id": registration.id }),
			},
			async (span, spanContext) => {
				try {
					...
					span.setAttributes({ "shuvpi.hook.outcome": blocked ? "blocked" : "completed" });
					return result;
				} catch (error) {
					span.setAttributes({ "shuvpi.hook.outcome": "failed" });
