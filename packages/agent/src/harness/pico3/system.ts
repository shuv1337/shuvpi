...
export type SystemEntry = Entry & { kind: "shuvpi.system"; model: [] | [Stored<SystemMessage>]; data: SystemEntryData };
...
export async function planManagedEntry(
	...
	if (needBaseline) {
		...
		const edits: ContextEdit[] = entries
			.filter((e: Entry) => e.kind === "shuvpi.system" && e.id > (snapshot.newestHead ?? 0))
