...
import { AssistantMessageFrameEncoder, isRetryableAssistantError } from "@shuv1337/shuvpi-ai";
import { estimateContextTokens } from "@shuv1337/shuvpi-ai/utils/estimate";
...
/** shuvpi-ai drops system messages and aborted/error assistant messages before sending; estimate what is sent. */
	...
	async initial(task, rt, ctx) {
		...
		return {
			next: async (tx, current) => {
				...
				const system = plan === undefined ? null : tx.appendEntry(c, { kind: "shuvpi.system", ...plan });
...
async function classify(task: G, cp: Prep, message: AssistantMessage, rt: Rt, ctx: Context): Promise<S> {
	...
	if (calls.length > 0) {
		return {
				...
				const tools = calls.map((call, index) =>
					tx.createTask({
						kind: "shuvpi.tool",
						input: { assistant, call: toStored(call), offered: cp.tools, index },
					}),
				);
				const postTools = tx.createTask({
					kind: "shuvpi.post_tools",
					after: tools,
					input: { inputs: task.input.inputs, assistant, tools },
				});
				if (collapseThrough !== undefined)
					tx.createTask({ kind: "shuvpi.collapse", input: { reason: "threshold", through: collapseThrough } });
	...
	return {
			...
			if (continueText !== undefined && triggers.length === 0 && !terminated) {
				...
				const successor = tx.createTask({ kind: "shuvpi.generation", input: { inputs: task.input.inputs } });
			...
			if (triggers.length > 0) {
				const successor = tx.createTask({ kind: "shuvpi.generation", input: { inputs: triggers } });
...
async function settleFailedTurn(
	...
	if (triggers.length > 0) {
		tx.createTask({ kind: "shuvpi.generation", conversationId: current.conversationId, input: { inputs: triggers } });
		tx.emit({ type: "turn.started", inputs: triggers });
	}
}
...
async function thresholdCollapseThrough(
	...
	if ((await tx.tasks({ conversationId, kind: "shuvpi.collapse", status: ["pending", "running"] })).length > 0)
		return undefined;
...
function overflow(task: G, rt: Rt): S {
	return {
			...
			const collapse = tx.createTask({ kind: "shuvpi.collapse", input: { reason: "overflow", through } });
			tx.createTask({ kind: "shuvpi.generation", after: [collapse], input: { inputs: task.input.inputs } });
			tx.emit({
				type: "generation.failed",
				taskId: current.id,
				reason: "overflow",
