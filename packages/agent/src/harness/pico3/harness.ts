...
import { withAbortSignal } from "@shuv1337/shuvpi-chord/context";
...
export class Harness<Ks extends readonly AnyKind[] = []> {
	...
	private constructor(storage: Storage, options: HarnessOptions<Ks>, ctx: Context) {
		...
		for (const k of options.taskKinds ?? []) {
			if (k.name.startsWith("shuvpi."))
				throw new Error(`task kind "${k.name}": names beginning with "shuvpi." are reserved`);
	...
	async suspend(ctx: Context): Promise<void> {
		...
		const tools = [...this.session.liveTasks.values()].filter((task) => task.kind === "shuvpi.tool");
		if (tools.length > 0) {
	...
	registerTaskKind<K extends AnyKind>(kind: K): () => void {
		if (kind.name.startsWith("shuvpi."))
			throw new Error(`task kind "${kind.name}": names beginning with "shuvpi." are reserved`);
	...
	namespace<T extends JsonObject>(
		...
		if (!/^[a-z][a-z0-9_.-]*$/i.test(id) || id.startsWith("shuvpi.")) throw new Error(`invalid namespace "${id}"`);
	...
	registerEntryKind(kind: EntryKind): () => void {
		if (kind.kind.startsWith("shuvpi."))
			throw new Error(`entry kind "${kind.kind}": names beginning with "shuvpi." are reserved`);
