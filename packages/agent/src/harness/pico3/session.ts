import { type Context, isJsonValue } from "@shuv1337/shuvpi-chord";
import { createContextKey, withContextValue, withoutAbortSignal } from "@shuv1337/shuvpi-chord/context";
import { isBase, type Op, type Tracker, track } from "@shuv1337/shuvpi-chord/delta";
...
const CORE_KINDS = new Set(["shuvpi.generation", "shuvpi.tool", "shuvpi.post_tools", "shuvpi.collapse"]);
...
class TxImpl implements CoreTx {
	...
	async write(conversationId: Id, a: NewEntry | EntryKind, b?: object): Promise<Id> {
		...
		if (!this.core) {
			...
			if (entry.kind.startsWith("shuvpi.") && entry.kind !== "shuvpi.notice")
				throw new Forbidden(`write: kind ${entry.kind} is reserved`);
			if (entry.kind === "shuvpi.notice" && (entry.model?.length !== 1 || entry.model[0]?.role !== "user")) {
				throw new Forbidden("write: shuvpi.notice requires exactly one user model message");
			}
		}
	...
	private createTaskInternal(spec: TaskSpec): Id {
		...
		if (spec.kind === "shuvpi.generation" && this.hasLiveKind(conversationId, spec.kind))
			throw new GenerationInProgress(conversationId);
		if (spec.kind === "shuvpi.collapse" && this.hasLiveKind(conversationId, spec.kind))
	...
	async send(conversationId: Id, input: SendInput): Promise<Id> {
		...
		this.createTaskInternal({ kind: "shuvpi.generation", conversationId, input: { inputs } });
