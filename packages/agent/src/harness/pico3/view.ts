import { applyImmutable, type Op, type Tracker, track } from "@shuv1337/shuvpi-chord/delta";
...
export class ViewManager {
	...
	private turn(conversationId: Id, sticky: StickyState): { turn?: TurnView } {
		...
		const generation = turnTasks.find((task) => task.kind === "shuvpi.generation");
		const postTools = turnTasks.find((task) => task.kind === "shuvpi.post_tools");
	...
	private compaction(conversationId: Id): { compaction?: ConversationView["compaction"] } {
		const task = [...this.session.liveTasks.values()].find(
			(candidate) => candidate.conversationId === conversationId && candidate.kind === "shuvpi.collapse",
		);
