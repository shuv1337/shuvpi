...
import type { JsonValue as ChordJsonValue, Context, JsonRepresentation } from "@shuv1337/shuvpi-chord";
import type { Op } from "@shuv1337/shuvpi-chord/delta";
...
/** Coerce an in-memory value (e.g. a shuvpi-ai message) to its stored representation. */
 ...
 * Use for imported shuvpi-ai types at durable positions.
 */
...
/**
 * System instructions and the tool loadout ride inside `messages` (shuvpi-ai PR #9116, shimmed
...
export function defineEntry<E extends Entry>(kind: string): EntryKind<E> {
	if (kind.startsWith("shuvpi.")) throw new Error(`entry kind names beginning with "shuvpi." are reserved: ${kind}`);
...
/** Author an ordinary kind. Names may not begin with `shuvpi.`. */
...
>(definition: Kind<I, C, R, F, A, H, Cfg, S>): Kind<I, C, R, F, A, H, Cfg, S> {
	if (definition.name.startsWith("shuvpi."))
		throw new Error(`task kind names beginning with "shuvpi." are reserved: ${definition.name}`);
