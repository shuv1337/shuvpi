import type { AssistantMessageFrame } from "@shuv1337/shuvpi-ai";
...
export const branchTip = (branch: string) => value<string | null>("shuvpi.branch.tip", branch);
export const branchTipInventoryPrefix = () => value<string | null>("shuvpi.branch.tip");
export const laneConfig = (lane: string) => value<LaneConfiguration>("shuvpi.lane.config", lane);
export const laneState = (lane: string) => value<LaneState>("shuvpi.lane.state", lane);
export const operationResult = (operationId: string) => value<OperationResultRecord>("shuvpi.result", operationId);

export const operationMeta = (operationId: string) => value<OperationMeta>("shuvpi.op.meta", operationId);
export const operationState = (operationId: string) => value<OperationState>("shuvpi.op.state", operationId);
export const operationToolArgs = (operationId: string, stepId: string, sourceIndex: number) =>
	value<Record<string, JsonValue>>("shuvpi.op.tool_args", `${operationId}:${stepId}:${sourceIndex}`);
...
export const operationPreparation = (operationId: string, taskId: string) =>
	value<DurableStructuralPreparation>("shuvpi.op.preparation", `${operationId}:${taskId}`);
...
export const pendingEntry = (entryId: string) => value<PendingEntry>("shuvpi.pending.entry", entryId);
export const pendingToolOutput = (operationId: string, invocationId: string) =>
	value<AgentToolResult<unknown>>("shuvpi.pending.tool_output", `${operationId}:${invocationId}`);
export const pendingAssistantFrames = (operationId: string, responseEntryId: string) =>
	list<AssistantMessageFrame>("shuvpi.pending.assistant_frame", `${operationId}:${responseEntryId}`);
